"""
Round 4 v02 — softer VELVETFRUIT MR, HYDROGEL untouched.

Round-4 ADF (1000-tick rolling): VELVETFRUIT rejects 5% null in only
4% of windows (vs HYDROGEL's 11%) — closer to a random walk than the
round-3 series. Keep the same MM lattice but cut MR aggressiveness on
VELVETFRUIT to avoid loading positions on weak signals:

  - mr_gain  2000 → 1000   (half the position-target slope vs σ)
  - take_max_pay  -2 → -4  (only cross when offer is 4+ ticks below fair)
  - quote_size  30 → 15    (smaller resting flow when MR signal is weak)

HYDROGEL params and the VEV anchor-divergence pipeline are unchanged.
"""
