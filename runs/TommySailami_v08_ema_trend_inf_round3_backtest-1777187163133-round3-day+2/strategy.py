"""Round 3 v08 — v07 + EMA trend skew + amplified informed-flow following.

Two additions on top of v07:
  EMA trend skew on HP/VFE position target
    Maintain a fast price EMA (alpha=EMA_ALPHA, ~50-tick half-life). Add
    TREND_GAIN * (mid - ema) to the Kalman target. Positive trend (price
    above EMA) biases the strategy long → don't fight the trend; negative
    biases short. Caps the over-aggressive Kalman MR target during strong
    drifts.

  Closer informed-flow following on VFE
    INFORMED_GAIN_S 2 -> 5 (3.5x amplification)
    INFORMED_DECAY 0.998 -> 0.99 (~5x faster decay so we exit positions
    sooner once the signal has played out — captures the +4.5 mid move
    over 500 ticks rather than holding indefinitely).

Three new tunables (EMA_ALPHA, TREND_GAIN, plus the gain/decay update).

Strategy stack (top to bottom):
  Kalman MR (HP/VFE) -> EMA trend bias -> informed-flow bias -> P3 MM
  VEV options: BS-with-smile + dual-anchor MR + Stoikov skew + IV scalp
"""
...
# v07 + v08 additions
INFORMED_SIZE_VFE = 11    # qty threshold for VFE big-trade informed signal
INFORMED_GAIN_S = 5       # v08: amplified from 2 (closer informed-trader following)
INFORMED_DECAY = 0.99     # v08: faster decay (0.998 -> 0.99 = exit ~5x faster)
EMA_ALPHA = 0.014         # v08: ~50-tick half-life price EMA per product
TREND_GAIN = 3            # v08: lots of position bias per tick of mid-EMA gap
