"""
Anchor-divergence multi-product market maker — VALIDATED variant.

`diverge_threshold` per product is the rounded 95th-percentile of measured
`|mid − anchor|`. Walk-forward (train on 2 days, test on 3rd) confirmed:
  - OOS total = +341,126   vs   IS total = +314,262    (OOS / IS = 108%)
  - Per-day OOS within ±10% of IS on D0 and D1; D2 outperforms IS by 59%
  - Cross-fold thresholds vary <25% — signal is stable across days

Pipeline (see make_orders):
...
Same code shape as zscore_aggressive.py — only the per-product
diverge_threshold values differ.
"""
...
def update_pnl_and_check_stop(cfg, state, scratch, position, fair):
    ...
    so the caller can suppress all new orders for this product."""
...
# diverge_threshold = strict 95th-percentile of |mid − anchor| from round-3
# historical data. Walk-forward validation (train on 2 days, test on 3rd)
# confirmed the signal generalizes (OOS > IS by 8.5%) at these thresholds.
PRODUCTS = [
    {"product": "HYDROGEL_PACK", "position_limit": 200, "aggressor_lambda": -0.010,
     "diverge_threshold": 55, "max_diverge_position": 195, "stop_loss_pnl": -4000},
    {"product": "VELVETFRUIT_EXTRACT", "position_limit": 200, "aggressor_lambda": 0.012,
     "diverge_threshold": 28, "max_diverge_position": 195},
    {"product": "VEV_4000", "position_limit": 300, "quote_size": 30, "baseline_vol": 0.5,
     "aggressor_lambda": 0.015, "diverge_threshold": 28, "max_diverge_position": 295},
    {"product": "VEV_4500", "position_limit": 300, "quote_size": 30, "baseline_vol": 0.5,
     "diverge_threshold": 28, "max_diverge_position": 295},
