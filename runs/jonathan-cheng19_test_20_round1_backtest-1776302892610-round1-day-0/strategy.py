...
class Trader:
    """
    Test_20 — inventory-skewed take thresholds + make edges.

    Mean +90,498 over 1000 MC sessions.

    OSMIUM: three-phase (take/clear/make) at static fair=10000.
      - Take: threshold skewed by inventory. When long, harder to buy more
        (tw_ask widens) and easier to sell (tw_bid narrows to 0 at max pos).
        Aggressively sheds overhang on favorable terms.
      - Clear: at fair±2 for excess inventory.
      - Make: penny-inside outer levels with inventory-skewed edges (long →
        tighter ask, wider bid).
    PEPPER: price-insensitive buy-and-hold to +80. Near theoretical max.
    """
