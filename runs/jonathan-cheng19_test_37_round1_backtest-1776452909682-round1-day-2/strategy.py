...
class Trader:
    """
    Test_37 — Drift-aware asymmetric TAKE on osmium.

    Test_36 used static fair=10000 for TAKE (safe vs MC drift) and Kalman
    for MAKE. That got real avg 97,502 (osm 18,056). Pepper is already
    near cap (79,381/80,000). The remaining 2,500 to reach 100k must
    come from osmium.

    Observation: on real round1 data, mean osmium price is 10000.21 —
    essentially stationary. Kalman fair is a good directional signal
    even on real. Making TAKE aware of Kalman direction (without
    crossing static floor) captures +EV fills we miss in Test_36
    while still rejecting adverse MC drift.

    Asymmetric TAKE rule:
      take_fair_buy  = max(STATIC, kalman_fair)   # buy more when drift up
      take_fair_sell = min(STATIC, kalman_fair)   # sell more when drift down

    If kalman=10001 then we buy at 9998 (vs 9997 in Test_36). We require
    tw=3 below the *more optimistic* of the two fairs so we only take
    when BOTH estimators agree there's edge. Net effect: slightly more
    aggressive TAKE when Kalman confirms direction, identical TAKE when
    Kalman disagrees (falls back to static).

    Additionally: CLEAR uses Kalman fair with variable width:
    - aggressive (cw=1) when |pos| >= 50 (reduce risk)
    - standard (cw=2) otherwise
    ...
    def _osmium(self, d, pos, td):
        ...
        # Asymmetric take fair: use the more optimistic of static/Kalman
        # for each side. Buying benefits from upward Kalman drift;
        # selling from downward. But we never undercut static buffer.
        ...
        # Adaptive clear width: tighten when heavily positioned
