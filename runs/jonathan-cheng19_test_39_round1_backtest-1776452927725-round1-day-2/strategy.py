...
class Trader:
    """
    Test_39 — Test_37 with MAKE_EDGE=2 (tighter make quotes).

    Test_37 real 97,811 / MC 91,565. Test_38's stacked changes didn't
    help. Next lever: MAKE_EDGE. At edge=3 we post at fair±3 minimum,
    with outer bots at fair±8 we post at fair±7 (big spread). Dropping
    MAKE_EDGE to 2 lowers the floor: fair±2 minimum, fair±7 max when
    outer bot present. Should capture more fills on days when book is
    narrower.

    The `pos <= vol_lim` guard still keeps us from posting too tight
    when inventory-heavy.

    Everything else unchanged from Test_37.
    """
