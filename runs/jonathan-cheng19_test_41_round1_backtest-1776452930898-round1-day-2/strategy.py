...
class Trader:
    """
    Test_41 — Test_40 + variance-adaptive TAKE_WIDTH.

    Kalman steady-state variance for our process is ≈ 1.04
    (solving v² = σ_ν·(v + σ_η²) with σ_ν²=0.141, σ_η²=6.656).
    Initial var = σ_η² = 6.656. It converges to steady state in ~10
    ticks.

    When Kalman is confident (low variance), the fair estimate is
    precise and we can safely take at narrower spreads — narrower
    TAKE buffer captures more +EV fills. When var is high (early
    session or post-shock), wider buffer protects us.

    Adaptive rule:
      var < 1.5 (converged) → tw = 2
      var ≥ 1.5 (transient) → tw = 3

    This is essentially a Bayesian confidence-adaptive take threshold:
    the tighter our posterior, the more aggressively we can trade.

    Everything else unchanged from Test_40.
    """
    ...
    OSM_TAKE_WIDTH_NARROW = 2
    OSM_TAKE_WIDTH_WIDE = 3
    OSM_VAR_THRESH = 1.5
    OSM_CLEAR_WIDTH = 2
    ...
    def _kalman_update(self, depth, td):
        if not depth.buy_orders or not depth.sell_orders:
            return td.get("_osm_kf_fair", self.OSM_FAIR_STATIC), self.OSM_OBS_VAR
        ...
        return kf["fair"], kf["var"]

    def _osmium(self, d, pos, td):
        if not d.buy_orders or not d.sell_orders:
            return []
        make_fair, kf_var = self._kalman_update(d, td)
        take_fair_buy = max(self.OSM_FAIR_STATIC, make_fair)
        take_fair_sell = min(self.OSM_FAIR_STATIC, make_fair)

        # Variance-adaptive TAKE buffer
        base_tw = self.OSM_TAKE_WIDTH_NARROW if kf_var < self.OSM_VAR_THRESH else self.OSM_TAKE_WIDTH_WIDE
        ...
        tw_ask = max(self.OSM_MIN_TW, base_tw + skew)
        tw_bid = max(self.OSM_MIN_TW, base_tw - skew)
