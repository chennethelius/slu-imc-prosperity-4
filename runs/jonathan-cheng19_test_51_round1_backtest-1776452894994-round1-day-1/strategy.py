...
class Trader:
    """
    Test_51 — Test_40 baseline with candidate parameter triple from
    prior search: STATIC=10002, SKEW_UNIT=14, TAKE_WIDTH=4.

    Prior analysis recorded +724 vs Test_40 on a single holdout
    session. That was ~1.5σ given an estimated OOS noise stdev of
    ~500, so insufficient to promote. This variant isolates the
    triple for high-power validation (500-session MC + day-0
    walk-forward sub-windows).

    Rationale for each knob:
      * STATIC=10002 — shifts the static anchor slightly above mid
        (observed mid mean ≈ 10001 on day 0). take_fair_buy uses
        max(STATIC, kalman), so this biases buys to activate at a
        SLIGHTLY higher threshold, capturing more ask-side flow.
      * SKEW_UNIT=14 — tighter inventory skew (was 24). Each 14
        units of position flips the skew by 1 tick, so the quote
        pulls back from the direction we're already long/short
        faster. Reduces carry risk.
      * TAKE_WIDTH=4 — stricter TAKE gate (was 3). Avoids marginal
        adverse-selection takes.

    Everything else identical to Test_40.
    """
