...
class Trader:
    """
    Test_55 — collapse CLEAR_WIDTH_TIGHT.

    vs Test_54: derive the tightened clear width from CLEAR_WIDTH via
    a boolean shift,
        cw = OSM_CLEAR_WIDTH - (abs(pos) >= OSM_CLEAR_TIGHT_POS)
    which yields 2 normally, 1 when position is large — identical to
    the two-constant form. Removes OSM_CLEAR_WIDTH_TIGHT.

    Params 14 → 13. No other changes; behavior byte-identical.
    """
