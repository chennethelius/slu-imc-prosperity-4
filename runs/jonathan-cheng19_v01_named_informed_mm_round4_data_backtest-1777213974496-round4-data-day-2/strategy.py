"""Round 4 v01 — aggressive HP MM + Mark-14/38 informed-trader follow signal.
...
def hp_orders(state, store):
    ...
    # 1. Informed-trader follow signal — sets position target
    sig = update_hp_informed_signal(
        store.setdefault("_hp_inf_store", {}),
        state.market_trades.get("HYDROGEL_PACK", []),
    )
    target = max(-HP_LIMIT, min(HP_LIMIT, int(round(INFORMED_GAIN_HP * sig))))

    # 2. Aggressive cross to chase target — only when we have strong conviction.
    #    Pay up to 5 ticks beyond fair when chasing (alpha is ~20 ticks so 5 is fine).
    ...
    if delta > 0:
        for ask in sorted(depth.sell_orders):
            ...
            qty = min(avail, delta - bv, HP_LIMIT - position - bv)
            if qty <= 0: break
        ...
        for bid in sorted(depth.buy_orders, reverse=True):
            ...
            qty = min(avail, need - sv, HP_LIMIT + position - sv)
            if qty <= 0: break
