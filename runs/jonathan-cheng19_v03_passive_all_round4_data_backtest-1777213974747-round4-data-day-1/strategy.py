...
def vfe_orders(state, store):
    """v03: VFE switched to passive MM with dynamic fair = current mid.

    The Kalman MR with VFE_FAIR_STATIC=5275 was an OVERFIT — round-4 day-3
    had VFE drop 63 ticks from 5295 to 5232. Holding longs against that
    drop bled massively. Pure passive MM around live mid avoids the bet.
    """
    depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
    if not depth or not depth.buy_orders or not depth.sell_orders:
        return []
    bb = max(depth.buy_orders); ba = min(depth.sell_orders)
    fair = (bb + ba) / 2.0  # dynamic fair = live mid
    fair_int = round(fair)

    position = state.position.get("VELVETFRUIT_EXTRACT", 0)
    orders, bv, sv = [], 0, 0

    # 1. Fair-value taking — cross any ask < fair, any bid > fair (both sides
    #    of the touch are profitable when above/below fair).
    for ask in sorted(depth.sell_orders):
    ...
    # 2. Position-reducing at fair_int.
    pos_after = position + bv - sv
    ...
    # 3. Always-on passive MM at best_bid+1 / best_ask-1 (capped at fair±1).
