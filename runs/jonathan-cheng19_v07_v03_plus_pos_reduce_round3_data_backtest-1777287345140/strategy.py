...
INFORMED_GAIN_S = 2       # bias added to Kalman target per signal unit
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            depth = state.order_depths.get(cfg["product"])
            target_bias = 0
            if cfg["product"] == "VELVETFRUIT_EXTRACT" and depth and depth.buy_orders and depth.sell_orders:
                vfe_bid_ = max(depth.buy_orders); vfe_ask_ = min(depth.sell_orders)
