"""Round 3 v10 — v09 with full IMC position limits (200/300 like hybrid.py).

Same logic as v09; only the position_limit values change (100 -> 200 for HP/VFE,
100 -> 300 for VEV_*) to match Kevin's hybrid.py and Test_1.py — both of which
target IMC's actual round-3 caps. On slu CI (rust backtester defaults unknown
products to 100) v10 will trigger wholesale order rejections once positions
build past 100; on IMC simulation the strategy can fully utilise inventory.

Test_1 at limit=200/300 historical = 601,742 (vs 157k at limit=100) → 4x edge
from limit-utilisation alone.
"""

# === v09 docstring follows ===
"""Round 3 v09 — hybrid.py's tighter VEV thresholds + v07 informed-flow + v08 EMA trend.

Integrates teammate Kevin's hybrid.py with our own informed-flow + EMA-trend
work:
  - Adopt hybrid.py's TIGHTER per-strike divergence thresholds for VEV — Kevin
    halved them (5500: 3->2, 5400: 5->4, 5300: 10->7, 5200: 14->10, ...). More
    frequent fade trading on options.
  - Keep v07's full pipeline (Kalman MR + Stoikov skew + dual-anchor MR + IV
    scalp + exit-at-fair) under the tighter thresholds.
  - Add v07's VFE informed-flow signal (size>=11 print bias on Kalman target).
  - Add v08's EMA trend skew on HP/VFE (TREND_GAIN * (mid - ema)).
  - Position limits stay at 100 for slu-CI compatibility (rust backtester
    default = 100; orders that would breach are wholesale-rejected).

Tunables: (TAKE_WIDTH, ANCHOR_WARMUP, DIVERGE_TAKE_SIZE, FLOW_DECAY,
SKEW_PER_LOT_INV, FLOW_AGREE_K, FLOW_DISAGREE_K, EXIT_AT_FAIR, FLOW_REVERSAL,
MIN_INVENTORY_FOR_EXIT, IV_SCALP_K, INFORMED_SIZE_VFE, INFORMED_GAIN_S,
INFORMED_DECAY, EMA_ALPHA, TREND_GAIN). All values inherited from prior
versions — no new tunables introduced. The 8 sigma_K constants are calibrated.
"""

# === v07 docstring follows ===
"""Round 3 v07 — v03 + position-reducing-at-fair + VFE informed-flow signal.

Synthesis of the experimentation in v04-v06:
    v04-v06 (clean P3 from-scratch) underperform v03 because the assumption
    "HP fair = 10,000" doesn't hold day-by-day — mid drifts to 9958 on day 0,
    so any (fair - mid) targeting accumulates losing longs. v03's Kalman-MR
    adapts to the actual reversion level and is the right engine.

    But v03 was missing two things that DID generalise from the from-scratch
    work:
      (1) position-reducing-at-fair on every product (Chris's third P3
          optimization — take orders priced at int(fair) that move us toward
          zero, freeing inventory for higher-margin fills next tick).
      (2) the VFE informed-flow signal (size>=11 prints reliably predict
          +4.5 mid move over 500 ticks; mined from the trade CSV).

v07 adds both to v03 with smallest effective doses:
    POSITION_REDUCE_AT_FAIR = True (3rd optimization, no parameter)
    INFORMED_GAIN_S = 2 (small, modulates the Kalman target on VFE only)

Everything else (Kalman-MR, BS-with-smile, Stoikov skew, dual-anchor MR,
IV scalping, exit-at-fair) is identical to v03.
"""

# === v03 docstring follows for reference ===
...
# v07 + v08 additions inherited
INFORMED_SIZE_VFE = 11
INFORMED_GAIN_S = 2
INFORMED_DECAY = 0.998
EMA_ALPHA = 0.014         # ~50-tick half-life price EMA per Kalman product
TREND_GAIN = 3            # lots of position bias per tick of mid-EMA gap
...
# =========================================================================
# Informed-flow signal on VFE big trades (size>=11)
# =========================================================================


def update_ema_trend(scratch, mid):
    """Update price EMA, return signed (mid - EMA) trend signal."""
    ema = scratch.get("_ema", mid)
...
KALMAN_MR_PRODUCTS = [
    {"product": "HYDROGEL_PACK", "position_limit": 200, "k_ss": 0.02,
     "fair_static": 10030, "mr_gain": 2000, "sigma_init": 30.0,
     "take_max_pay": -6, "quote_edge": 3, "quote_size": 30},
    {"product": "VELVETFRUIT_EXTRACT", "position_limit": 200, "k_ss": 0.02,
...
def kalman_mr_orders(cfg, depth, position, scratch, target_bias=0):
    ...
    # NOTE: position-reducing-at-fair removed here — it conflicts with the
    # Kalman position TARGET (would sell exactly the inventory the target
    # is asking us to hold). Pos_red is only valid in pure-MM mode.
    return orders
...
VEV_PRODUCTS = [
    # Adopted from Kevin's hybrid.py — tighter divergence thresholds for more
    # frequent options-fade trading (5500: 3->2, 5400: 5->4, 5300: 10->7, etc.)
    {"product": "VEV_4000", "strike": 4000, "position_limit": 300, "quote_size": 30, "diverge_threshold": 18, "max_diverge_position": 295},
    {"product": "VEV_4500", "strike": 4500, "position_limit": 300, "quote_size": 30, "diverge_threshold": 18, "max_diverge_position": 295},
    {"product": "VEV_5000", "strike": 5000, "position_limit": 300, "quote_size": 30, "diverge_threshold": 15, "max_diverge_position": 295},
    {"product": "VEV_5100", "strike": 5100, "position_limit": 300, "quote_size": 30, "diverge_threshold": 13, "max_diverge_position": 295},
    {"product": "VEV_5200", "strike": 5200, "position_limit": 300, "quote_size": 30, "diverge_threshold": 10, "max_diverge_position": 295},
    {"product": "VEV_5300", "strike": 5300, "position_limit": 300, "quote_size": 30, "diverge_threshold": 7,  "max_diverge_position": 295},
    {"product": "VEV_5400", "strike": 5400, "position_limit": 300, "quote_size": 30, "diverge_threshold": 4,  "max_diverge_position": 295},
    {"product": "VEV_5500", "strike": 5500, "position_limit": 300, "quote_size": 30, "diverge_threshold": 2,  "max_diverge_position": 295},
]
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            depth = state.order_depths.get(cfg["product"])
            target_bias = 0
            scratch_for_prod = store.setdefault(cfg["product"], {})
            if depth and depth.buy_orders and depth.sell_orders:
                bid_ = max(depth.buy_orders); ask_ = min(depth.sell_orders)
                mid_ = (bid_ + ask_) / 2.0
                # EMA trend bias on every Kalman product
                trend = update_ema_trend(scratch_for_prod, mid_)
                target_bias += int(round(TREND_GAIN * trend))
                # VFE-only informed-flow bias
