"""Round 3 v13 — v12 + HP informed-flow + LOWERED VFE threshold for 15x events.

Empirical re-mining of trade CSV (3 days):
  HYDROGEL_PACK qty distribution: 2-6 (uniform). qty>=3 has 778 events with
    +1.5 mid move over 500t — usable informed signal we missed in v12.
  VELVETFRUIT_EXTRACT qty>=11 (v12 threshold): 49 events, +4.5 mid move.
    qty>=6: 736 events, +1.46 mid move. ~15x more signal events with similar
    per-event predictive power (3-4x more total alpha).

v13 adds two changes vs v12:
  HP: new informed-flow target bias (qty>=3, gain=2)
  VFE: threshold 11 -> 6 (more events) AND gain 10 -> 4 (per-event smaller)

Per-event gain reduced because larger sample means more potential noise per
print; total signal stays comparable. Empirically tuned for inventory-risk
balance.
...
# v13: dual informed-flow signals on HP and VFE
INFORMED_SIZE_VFE = 6     # lowered from 11 -> 736 events / 3 days, +1.46 mid in 500t
INFORMED_GAIN_VFE = 4     # smaller gain than v12's 10 — more events => smaller per-event push
INFORMED_SIZE_HP = 3      # 778 events at this threshold, +1.5 mid in 500t
INFORMED_GAIN_HP = 2      # smaller signal magnitude on HP, smaller gain
...
def update_informed_signal(store, market_trades, bid, ask, size_thresh):
    """Generic informed-flow signal: decayed signed-volume of trades >= threshold."""
    sig = store.get("_inf", 0.0) * INFORMED_DECAY
    for t in market_trades or []:
        if t.quantity < size_thresh:
            continue
        if t.price >= ask:
            sig += t.quantity
        elif t.price <= bid:
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            ...
            if depth and depth.buy_orders and depth.sell_orders:
                bb_ = max(depth.buy_orders); ba_ = min(depth.sell_orders)
                if sym == "VELVETFRUIT_EXTRACT":
                    sig = update_informed_signal(
                        store.setdefault("_inf_vfe", {}),
                        state.market_trades.get(sym, []),
                        bb_, ba_, INFORMED_SIZE_VFE,
                    )
                    target_bias = int(round(INFORMED_GAIN_VFE * sig))
                elif sym == "HYDROGEL_PACK":
                    sig = update_informed_signal(
                        store.setdefault("_inf_hp", {}),
                        state.market_trades.get(sym, []),
                        bb_, ba_, INFORMED_SIZE_HP,
                    )
                    target_bias = int(round(INFORMED_GAIN_HP * sig))
