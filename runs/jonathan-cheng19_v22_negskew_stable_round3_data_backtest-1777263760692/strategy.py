"""Round 3 v22 — v16 + reduced spread-paying + informed-flow exit-at-fair.

Goal: maintain v16's ~720k historical PnL while improving Sharpe and
reducing realised drawdown. NEG-SKEW design — avoid the "buying above
mid / selling below mid" expense that drives MTM swings.

Two structural changes vs v16 (no parameter tweaks):

1. TIGHTER spread-paying via take_max_pay
     HP : -6 -> -3   (only cross when offer >=3 ticks below fair, not 6)
     VFE: -2 -> -1   (only cross when offer >=1 tick below fair)
   Stops the strategy from paying half-spread on every Kalman target chase.
   The Kalman MR still works via penny-jumping fills.

2. INFORMED-FLOW EXIT-AT-FAIR on VFE
     If position > 0 and informed-flow signal turns negative (informed
     sellers arrived, expect price down), and best_bid >= int(fair), cross
     the bid to exit at fair (no spread paid). Symmetric for short.
   Captures the "unload at fair when expecting profitable trades coming"
   rule; uses our validated VFE size>=11 informed-flow signal.

Everything else (Kalman MR with HP mr_gain=1000, VFE mr_gain=2000, hybrid's
tight VEV thresholds, informed-flow target bias gain=10) is verbatim v16.
...
KALMAN_MR_PRODUCTS = [
    {
        ...
        "mr_gain": 1000,
        "sigma_init": 30.0,
        "take_max_pay": -3,         # v22: -6 -> -3 (less spread paid on Kalman chase)
        "quote_edge": 3,
        "quote_size": 30,
    },
    {
        ...
        "mr_gain": 2000,
        "sigma_init": 15.0,
        "take_max_pay": -1,         # v22: -2 -> -1 (less spread paid on VFE chase)
        "quote_edge": 1,
        "quote_size": 30,
    },
]
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            ...
            target_bias = 0
            exit_orders = []
            position_cfg = state.position.get(sym, 0)
            ...
            if sym == "VELVETFRUIT_EXTRACT" and depth and depth.buy_orders and depth.sell_orders:
                ...
                # v22: informed-flow EXIT-AT-FAIR
                # If long + informed sellers (sig < -threshold), unload at fair
                # to avoid the predicted drop. Symmetric for short.
                fair_int = round(cfg["fair_static"])
                EXIT_FLOW_THRESH = 5.0
                if position_cfg > 0 and sig < -EXIT_FLOW_THRESH and vfe_bid_ >= fair_int:
                    avail = depth.buy_orders[vfe_bid_]
                    qty = min(avail, position_cfg)
                    if qty > 0:
                        exit_orders.append(Order(sym, vfe_bid_, -qty))
                        position_cfg -= qty
                elif position_cfg < 0 and sig > EXIT_FLOW_THRESH and vfe_ask_ <= fair_int:
                    avail = -depth.sell_orders[vfe_ask_]
                    qty = min(avail, -position_cfg)
                    if qty > 0:
                        exit_orders.append(Order(sym, vfe_ask_, qty))
                        position_cfg += qty

            ors = kalman_mr_orders(cfg, depth, position_cfg,
                                   store.setdefault(sym, {}),
                                   target_bias=target_bias)
            if exit_orders or ors:
                orders[sym] = exit_orders + (ors or [])
