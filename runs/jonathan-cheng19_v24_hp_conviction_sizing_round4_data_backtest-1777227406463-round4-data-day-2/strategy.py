"""Round 4 v24 — conviction-weighted volume sizing (Picard's principle).

"Volume should reflect the strength of your conviction. Nothing more.
 Nothing less."

The structural lesson: take SIZE proportional to signal strength, not
binary fire-and-load. Three orthogonal signal sources combine into a
single 0..1 conviction score; take quantity = TAKE_MAX * conviction.

Conviction = weighted combination of:

  Z-score magnitude (0..2):
       z = (mid − HP_FAIR) / stdev_ewma
       z_str = clamp(|z| − Z_MIN, 0, Z_MAX − Z_MIN) / (Z_MAX − Z_MIN)
       z_str = 0 when |z| < Z_MIN (deviation too small to matter)
       z_str = 1 when |z| ≥ Z_MAX (extreme deviation, full conviction)

  EMA confluence agreement (0..1):
       short = ema_fast − ema_slow
       medium = ema_slow − ema_vslow
       ema_str = 0 if sign(short) ≠ sign(medium) (whipsaw, no edge)
       ema_str = clamp(|short| / EMA_FULL, 0, 1) when aligned

  Informed-flow strength (0..1):
       net_informed = sum(buys − sells) by Mark-* counterparties, last K
       inf_str = clamp(|net| / INF_FULL, 0, 1)

Direction filter: signal direction = sign(HP_FAIR − mid) — i.e. buy when
cheap, sell when rich (mean reversion). EMA and informed signals must
agree with this direction to contribute their strength; if they oppose,
their strength is set to zero (no edge to trust).

Final conviction:
   conviction = w_z * z_str + w_ema * ema_str + w_inf * inf_str
   weights tuned so all three can together reach 1.0.

Volume:
   take_qty = round(HP_TAKE_MAX * conviction)
   conviction = 0  → no take
   conviction = 1  → full HP_TAKE_MAX commitment

Per-tick budget caps the burst rate (graduated entry); soft inventory cap
caps total directional exposure. Mean-reversion alpha gets harvested with
size proportional to the prize.

Chess-grandmaster discipline: weak signal → small stake or pass; strong
signal with confirmation → larger stake; conflicting signals → no stake.
"""
...
HP_Z_MIN = 0.7              # below this, no conviction
HP_Z_MAX = 2.0              # at/above this, full z-strength

# EMA confluence
HP_EMA_FAST_ALPHA = 0.30
HP_EMA_SLOW_ALPHA = 0.05
HP_EMA_VSLOW_ALPHA = 0.02
HP_EMA_FULL = 1.5           # short-EMA diff at which ema_str saturates

# Informed flow
HP_INFORMED_LOOKBACK = 8
HP_INFORMED_FULL = 30        # net flow at which inf_str saturates

# Conviction blend (sum to 1.0)
HP_W_Z = 0.50
HP_W_EMA = 0.30
HP_W_INF = 0.20

# Volume scaling
HP_TAKE_MAX = 80             # max per-tick conviction-weighted take
HP_TAKE_SOFT_CAP_PCT = 0.50  # max directional position
HP_TAKE_OFFSET = 4           # max ticks beyond HP_FAIR we'll lift/hit
...
def _conviction(mid, market_trades, td):
    ...
    direction = +1 if dev < 0 else -1   # buy if cheap, sell if rich
    ...
    # EMA contributes only when:
    #   - short and medium share sign (confluence — true momentum, not whipsaw)
    #   - that sign matches our MR direction (ema confirms reversal)
    ...
    if market_trades:
        for t in market_trades[-HP_INFORMED_LOOKBACK:]:
            if (t.buyer or "").startswith("Mark"):
                net_inf += int(t.quantity)
            if (t.seller or "").startswith("Mark"):
                net_inf -= int(t.quantity)
    ...
    if z_str == 0.0:
        # No deviation → no take, regardless of momentum/flow
        return 0, 0.0
