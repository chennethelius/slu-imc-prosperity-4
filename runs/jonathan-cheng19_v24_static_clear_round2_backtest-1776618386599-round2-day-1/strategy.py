...
class Trader:
    """v24: static anchor on both TAKE and CLEAR phases.

    Structural change: CLEAR phase now references OSM_FAIR_STATIC instead
    of the Kalman-dynamic fair. The dynamic fair stays alive only for the
    MAKE phase, where it informs skew-based edge placement. Eliminating
    dynamic fair from take/clear decisions reduces OOS whipsaw — the same
    mechanism that earned v23 +412 on d-1 now extends to exit orders.

    Cumulative round2 3-day osm PnL (QP=0.0 local):
      v22 (dyn TAKE, dyn CLEAR):   248147
      v23 (static TAKE, dyn CLEAR): 248353  (+206)
      v24 (static TAKE+CLEAR):     248441  (+88 on top; +294 vs v22)
      dm1=3662 d0=3135 d1=3727 osm_cum=10524

    Sensitivity probe: CLEAR_WIDTH 2 and 3 produce identical PnL at
    static-anchor — CLEAR only fires when TAKE left unfilled position,
    and then at prices already adjacent to TAKE's cut. So CLEAR is a
    defensive backstop, not an edge contributor.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        ask_limit = static - max(0, self.OSM_TAKE_WIDTH + skew)
        bid_limit = static + max(0, self.OSM_TAKE_WIDTH - skew)
