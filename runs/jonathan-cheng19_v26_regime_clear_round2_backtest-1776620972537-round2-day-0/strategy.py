...
class Trader:
    """v26: regime-aware CLEAR on osmium — functional iteration over v22.

    Structural delta vs v22: the CLEAR phase now checks whether the
    Kalman fair is above or below OSM_FAIR_STATIC (=10001, osmium's
    mean-reversion center) before unwinding. If inventory direction
    already aligns with the expected reversion — e.g., long while fair
    is below 10001, short while fair is above 10001 — we skip CLEAR
    and let MAKE handle it. When direction is adverse to expected
    reversion, we clear as v22 did.

    First-principles motivation:
    - v22 wins CI because it accumulates inventory during favorable
      drift and captures marked-to-market gain at close.
    - v22's CLEAR at fair±cw unwinds inventory indiscriminately,
      occasionally dumping favorable-direction holdings.
    - Regime gate preserves favorable inventory, clears adverse.
    - Zero new parameters — STATIC is the same 10001 already in v22.

    Trade-off: paused clearing on favorable side means slower unwind
    there. If fair keeps drifting away from 10001 (rare, osmium is
    mean-reverting), inventory risk grows. Hard position limit (80)
    is the ultimate safety net; skew-in-MAKE still pressures fairer
    quotes.
    """
