"""Round 3 v25 — v16 with worst-mid as the Kalman observation (P3 winner trick).

CHRIS'S P3 INSIGHT (verbatim from his round-1 walkthrough video):
   "The way they actually calculate the price... it's the mid price of the
   worst bid and the worst ask, right? It's consistently that... we tested
   and verified."

The IMC server's mark-to-market fair value is the midpoint of the OUTERMOST
book levels — NOT the best-bid/best-ask micro-price the strategy was using.
Outer levels are the "stable wall" set by symmetric MM bots that don't move
often, so their midpoint is the stable estimate of true value.

v16 used the volatile micro-price (best-bid/best-ask volume-weighted mid) as
the Kalman observation. v25 swaps it for worst-mid:

  worst_bid = min(depth.buy_orders)        # deepest bid
  worst_ask = max(depth.sell_orders)       # deepest ask
  worst_mid = (worst_bid + worst_ask) / 2  # IMC's actual fair-value formula

Single line change. The Kalman MR now mean-reverts to a level that matches
how IMC computes PnL — every adverse MTM swing the strategy was bleeding
through was a worst-mid divergence the strategy didn't track.

Also: the fair_static anchors (HP=10030, VFE=5275) were hand-tuned biases
to compensate for the micro-vs-worst-mid mismatch. With v25 we can keep them
or simplify to true means; testing both.
...
def kalman_mr_orders(cfg, depth, position, scratch, target_bias=0):
    ...
    # v25: worst-mid as the Kalman observation — matches IMC's MTM fair value.
    # Falls back to best-bid/best-ask mid when level-2 absent.
    worst_bid = min(depth.buy_orders)
    worst_ask = max(depth.sell_orders)
    worst_mid = (worst_bid + worst_ask) / 2.0

    # Kalman-track fair on the WORST-mid (stable outer-wall) instead of
    # the volatile micro-price.
    k_ss = cfg["k_ss"]
    fair = scratch.get("_f", worst_mid)
    innov = worst_mid - fair
    ...
    # v26: target uses worst_mid too (matches IMC's MTM fair-value formula).
    anchor = cfg["fair_static"]
    target = max(-limit, min(limit,
        round(cfg["mr_gain"] * (anchor - worst_mid) / sigma) + target_bias))
