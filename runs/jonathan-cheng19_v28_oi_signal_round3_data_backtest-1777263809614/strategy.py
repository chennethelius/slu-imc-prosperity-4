"""Round 3 v28 — v27 + VFE order-book imbalance (OI) signal.

Mined alpha (3 days, all-tick aggregate):
   VFE order-book imbalance oi = (bv1 - av1) / (bv1 + av1) at top-of-book.
   Top-decile (oi > +0.53):  fwd_500_tick mean = +2.195  (n=2942)
   Mid-band  (oi -0.53..+0): fwd_500_tick mean = +0.150  (n=17867)
   Bottom-decile (oi < -0.47): fwd_500_tick mean = +0.931  (negative side
                  signal noisier — keep symmetric application but expect
                  asymmetric realized fill).

The signal: extreme top-of-book volume imbalance precedes mid-price moves
in the same direction. Mean shift +2 ticks at +1 oi → ~+10 ticks fully
saturated.

v28 adds an OI-based bias to the VFE Kalman target, in the same channel
as v27's informed-flow signal. Small gain to avoid dominating Kalman MR:

   target_bias_oi = OI_GAIN * oi_ema   (clamp to ±OI_BIAS_CAP)

Two new constants only (OI_GAIN, OI_DECAY). The OI_BIAS_CAP keeps the
addition small relative to v27's existing biases.
...
# v28: VFE order-book imbalance (OI) signal — top-decile predicts +2.2 mid in 500t
OI_GAIN = 8           # lots of bias per unit oi_ema (oi_ema in [-1, +1])
OI_DECAY = 0.95       # ~20-tick half-life — OI is a fast-decaying microstructure signal
OI_BIAS_CAP = 12      # cap absolute OI bias to keep within Kalman dominance
...
def update_oi_signal(store, depth):
    """v28: VFE top-of-book order-imbalance EMA in [-1, +1]."""
    bb = max(depth.buy_orders); ba = min(depth.sell_orders)
    bv = depth.buy_orders[bb]; av = -depth.sell_orders[ba]
    oi_inst = (bv - av) / (bv + av) if (bv + av) > 0 else 0.0
    oi_ema = store.get("_oi", oi_inst) * OI_DECAY + (1 - OI_DECAY) * oi_inst
    store["_oi"] = oi_ema
    return oi_ema
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            ...
            if (cfg["product"] == "VELVETFRUIT_EXTRACT"
                ...
                # v28: order-book imbalance signal
                oi = update_oi_signal(store.setdefault("_oi_store", {}), depth)
                oi_bias = max(-OI_BIAS_CAP, min(OI_BIAS_CAP, int(round(OI_GAIN * oi))))
                target_bias += oi_bias
