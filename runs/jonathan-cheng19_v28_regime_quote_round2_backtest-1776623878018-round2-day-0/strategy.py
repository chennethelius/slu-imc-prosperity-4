...
class Trader:
    """v28: regime-aware MAKE quote placement — extends v27.

    Progression of regime-aware gating:
      v26: CLEAR phase skipped on favorable regime (CI +127 over v22)
      v27: MAKE skew zeroed on favorable regime    (CI +46 over v26)
      v28: MAKE quote JOINS BBO (not jumps it) on unwind side when
           regime favors holding current inventory.

    Current v27 MAKE places orders at bbbf+1 (inside best bid) and
    baaf-1 (inside best ask) to jump queue. In regime-favorable
    direction we don't want to unwind — moving the unwind-side quote
    OUT to the BBO line itself (bbbf / baaf) reduces its fill rate
    while still posting liquidity.

    Structural logic:
      sell_price = baaf   if pos > 0 AND fair < STATIC  else  baaf - 1
      buy_price  = bbbf   if pos < 0 AND fair > STATIC  else  bbbf + 1

    Zero new parameters. Uses the same STATIC=10001 regime threshold.
    Accumulation side keeps queue-jumping behavior (bbbf+1 bid when
    long-favorable, baaf-1 ask when short-favorable).
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        if baaf is not None and bbbf is not None:
            ...
            buy_price = bbbf if (pos < 0 and short_favorable) else bbbf + 1
            sell_price = baaf if (pos > 0 and long_favorable) else baaf - 1

            buy_q = lim - pos - bv
            if buy_q > 0:
                orders.append(Order("ASH_COATED_OSMIUM", buy_price, buy_q))
            sell_q = lim + pos - sv
            if sell_q > 0:
                orders.append(Order("ASH_COATED_OSMIUM", sell_price, -sell_q))
        return orders
