"""Round 3 v29 — combined-best: v19 base + v22 exit-at-fair + tighter HP take.

Synthesis of the top dashboard performers:
   v19 (top CI: 716k) — HP mr_gain=1000, VFE mr_gain=3000, informed-flow
   v16 (#2:    714k) — same as v19 but VFE mr=2000
   hybrid (Kevin)    — tight VEV thresholds (best PnL extraction)
   v22 (Sharpe arm) — exit-at-fair on flow reversal (preserves PnL with
                       lower drawdown if signal fires meaningfully)

v29 stacks the validated alphas:
   HP  mr_gain = 1000  (v16/v19)
   VFE mr_gain = 3000  (v19 — top CI scorer)
   VEV thresholds: hybrid's tight (Kevin)
   VFE informed-flow: gain=10, decay=0.998 (v12)
   v22 add: HP take_max_pay -6 -> -3 (less spread paid on Kalman chase)
   v22 add: exit-at-fair on VFE when |informed-flow| > 5 lots and bid >= int(fair)

No tanh/circuit-breaker (over-restrictive, lost 555k in v20). No delta hedge
(fights Kalman, lost 30k in v23). No OI signal (zero net effect in v28). No
EMA trend skew (zero net in v08).
...
KALMAN_MR_PRODUCTS = [
    {
        ...
        "mr_gain": 1000,
        "sigma_init": 30.0,
        "take_max_pay": -3,         # v29: -6 -> -3 (less spread paid on Kalman chase)
        "quote_edge": 3,
        "quote_size": 30,
    },
    {
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for cfg in KALMAN_MR_PRODUCTS:
            ...
            target_bias = 0
            exit_orders = []
            position_eff = state.position.get(sym, 0)
            ...
            if (sym == "VELVETFRUIT_EXTRACT"
                ...
                # v29: exit-at-fair when flow signals a reversal (informed
                # sellers while we're long, etc). Conservative thresholds.
                fair_int = round(cfg["fair_static"])
                EXIT_FLOW_THRESH = 5.0
                if position_eff > 0 and sig < -EXIT_FLOW_THRESH and vfe_bid_ >= fair_int:
                    avail = depth.buy_orders[vfe_bid_]
                    qty = min(avail, position_eff)
                    if qty > 0:
                        exit_orders.append(Order(sym, vfe_bid_, -qty))
                        position_eff -= qty
                elif position_eff < 0 and sig > EXIT_FLOW_THRESH and vfe_ask_ <= fair_int:
            ...
            ors = kalman_mr_orders(cfg, depth, position_eff,
