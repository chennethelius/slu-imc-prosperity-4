...
class Trader:
    """v29: regime-aware TAKE skew — unifies regime gate across all 3 phases.

    Principle trajectory:
      v22: all three phases (TAKE/CLEAR/MAKE) pressure unwind via skew.
      v26: CLEAR gated by regime — skip unwind when inventory aligns with
           mean-reversion drift (fair vs STATIC=10001).          (+127 CI)
      v27: MAKE gated by regime — drop skew when favorable.      (+46 CI)
      v29: TAKE gated by regime — drop skew when favorable.

    v22 TAKE:
      ask_limit = max(STATIC, fair) - max(0, TAKE_WIDTH + skew)
      bid_limit = min(STATIC, fair) + max(0, TAKE_WIDTH - skew)
      When long, the +skew on ask_limit narrows buying AND the -skew on
      bid_limit widens selling — both push inventory toward zero.

    v29: if (long AND fair < STATIC) OR (short AND fair > STATIC) — we're
    already aligned with expected mean-reversion, so forcing TAKE to unwind
    is actively destructive (sells long position cheap while fair rises).
    In that case use symmetric TAKE (skew=0); otherwise v22 behavior.

    Zero new parameters. Same favorable_inv predicate as v26/v27, now
    applied to the third and final phase. No tuning, only structure.

    Risk: if Kalman fair is noisy and swings across STATIC within a tick,
    the regime flip might cause inconsistency between the TAKE gate
    (evaluated pre-TAKE on `pos`) and CLEAR/MAKE gates (pos_after, etc).
    Each phase uses its own correct state, so they're logically coherent.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        pre_favorable = (pos > 0 and long_favorable) or (pos < 0 and short_favorable)
        if pre_favorable:
            ask_limit = max(static, fair) - self.OSM_TAKE_WIDTH
            bid_limit = min(static, fair) + self.OSM_TAKE_WIDTH
        else:
