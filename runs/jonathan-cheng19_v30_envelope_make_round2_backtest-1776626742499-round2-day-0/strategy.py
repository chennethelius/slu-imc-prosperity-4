...
class Trader:
    """v30: envelope-anchored MAKE — reuses v22's TAKE envelope for MAKE quotes.

    v22 TAKE uses a mean-reversion envelope:
        ask_limit = max(STATIC, fair) - TAKE_WIDTH
        bid_limit = min(STATIC, fair) + TAKE_WIDTH
    This prices the Kalman fair against the STATIC anchor: when fair < STATIC
    we still accept asks up to STATIC (expecting rise); when fair > STATIC
    we still sell into bids down to STATIC (expecting fall). v22's edge on
    CI comes primarily from this envelope.

    v27's MAKE uses raw `fair` as reference:
        baaf ← first ask > fair + ask_edge - 1
        bbbf ← first bid < fair - bid_edge + 1

    v30 applies the same envelope prior to MAKE:
        ask ref = max(STATIC, fair)
        bid ref = min(STATIC, fair)

    Intuition: if fair < STATIC, asks should expect mean-reversion up
    toward STATIC — quoting asks further from fair captures more edge
    per fill while still jumping the queue relative to the new ref.
    Symmetrically for bids when fair > STATIC.

    When fair = STATIC (dominant regime), v30 = v27 exactly. The change
    only engages when fair diverges from STATIC, which is precisely
    when mean-reversion signal is strongest.

    Zero new parameters. Regime-gated CLEAR (v26) and MAKE skew (v27)
    remain unchanged. This composes the regime gate (direction of skew)
    with envelope anchoring (location of quotes).

    Risk vs v28 lesson: queue-jumping preserved — we still compute
    baaf-1/bbbf+1, just relative to the envelope-shifted reference.
    Fill rate may drop on the stretched side, but every fill is an
    improved cost basis.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        ask_ref = max(static, fair)
        bid_ref = min(static, fair)
        baaf = min((p for p in d.sell_orders if p > ask_ref + ask_edge - 1), default=None)
        bbbf = max((p for p in d.buy_orders if p < bid_ref - bid_edge + 1), default=None)
        if baaf is not None and bbbf is not None:
            if baaf <= ask_ref + ask_edge and pos <= self.OSM_VOLUME_LIMIT:
                baaf = int(round(ask_ref + ask_edge + 1))
            if bbbf >= bid_ref - bid_edge and pos >= -self.OSM_VOLUME_LIMIT:
                bbbf = int(round(bid_ref - bid_edge - 1))
            buy_q = lim - pos - bv
            if buy_q > 0:
