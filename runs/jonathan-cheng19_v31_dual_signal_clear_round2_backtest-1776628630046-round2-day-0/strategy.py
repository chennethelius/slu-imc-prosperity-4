...
class Trader:
    """v31: dual-signal CLEAR gate — require micro AND fair to agree.

    v26 (part of v27 champion) skips CLEAR when inventory aligns with the
    STATIC mean-reversion direction. Gate uses only Kalman `fair`:
        long_favorable  = fair < STATIC
        short_favorable = fair > STATIC

    The Kalman fair is smoothed microprice. Within a tick it may still say
    "drift up" (fair < STATIC) even though the immediate order book says
    "sell pressure" (micro < fair). Those are cases where v26 skips CLEAR
    but probably shouldn't — the market's current aggressor flow disagrees
    with the model's longer-horizon reversion signal.

    v31 requires BOTH to agree before suppressing CLEAR:
        long_favorable_confirmed  = fair < STATIC AND micro >= fair
        short_favorable_confirmed = fair > STATIC AND micro <= fair

    When fair signals favorable but micro flips, CLEAR fires as v22 did —
    we unwind defensively. When both agree, we hold (v26 behavior).

    MAKE gate (v27's narrow-symmetric) is unchanged — it operates on the
    raw favorable_inv without dual confirmation. Rationale: MAKE skew
    affects edge widths (marginal), CLEAR affects whether we actively
    dump position (large effect). Stricter gate on the higher-leverage
    action.

    Zero new parameters. Uses the already-computed `micro`. Strictly a
    structural composition of two existing signals.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        long_favorable = fair < static
        short_favorable = fair > static
        long_confirmed = long_favorable and micro >= fair
        short_confirmed = short_favorable and micro <= fair
        if pos_after > 0 and not long_confirmed:
        ...
        elif pos_after < 0 and not short_confirmed:
