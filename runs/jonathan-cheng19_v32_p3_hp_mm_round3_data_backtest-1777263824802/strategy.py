"""Round 3 v32 — clean Chris-style P3 MM for HP (user spec).

HP pipeline replaced with explicit P3 three-phase MM:
  1. FAIR-VALUE TAKING: cross any ask < 10000, any bid > 10000 (free PnL)
  2. POSITION-REDUCING AT FAIR: take orders priced AT 10000 that move
     |position| toward zero (frees inventory for higher-margin MM)
  3. ALWAYS-ON PENNY JUMPING: post bid at best_bid+1 (capped at fair-1)
     and ask at best_ask-1 (capped at fair+1) every tick, both sides
     simultaneously while either has remaining capacity.

VFE keeps the v27 Kalman-MR + size>=11 informed-flow target bias.
VEV options keep the v27 Test_1-wide divergence thresholds.

Round 3 v27 — Test_1 wide VEV thresholds + v12 informed-flow + v16 HP fix.

Day-2 forensics revealed hybrid's tighter VEV divergence thresholds (Kevin's
5500=2, 5400=4, 5300=7, ...) over-traded options when the regime turned —
hybrid day-2 = 168,248 vs Test_1's wider thresholds (5500=3, 5400=5, ...) =
200,744 (+32k). Net hybrid won days 0+1 by 147k, but lost 32k on day 2.
That asymmetry is a textbook fit-to-early-data overfitting tell.

v27 keeps the validated alphas while reverting the over-tuned VEV thresholds:
   Test_1's WIDER VEV thresholds (regime-robust day-2 protection)
 + v12 VFE informed-flow signal (size>=11 print +4.5 mid in 500t, gain=10)
 + v16 HP mr_gain=1000 (smoother position scaling)

Trades less on options (lower per-day options PnL on volatile days), more
on the underliers (where the alpha is structural). Sacrifices peak-day-0
PnL for tail-day robustness.

Original v11 docstring follows for reference.
...
ZSCORE_PRODUCTS = [
    # v27: revert to Test_1's WIDER thresholds — robust to day-2 regime where
    # hybrid's tighter thresholds bled. Test_1 day-2 = 200,744 vs hybrid 168,248.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ---- HYDROGEL_PACK: clean P3-style MM (the user's spec) ----
        ...
        if hp_depth and hp_depth.buy_orders and hp_depth.sell_orders:
            ...
            # 1. FAIR VALUE TAKING: cross any ask < fair, any bid > fair (free PnL).
            for ask in sorted(hp_depth.sell_orders):
                if ask >= HP_FAIR_INT:
                    break
                avail = -hp_depth.sell_orders[ask]
                qty = min(avail, HP_LIMIT - position - bv)
                if qty <= 0:
                    break
                out.append(Order("HYDROGEL_PACK", ask, qty)); bv += qty
            for bid in sorted(hp_depth.buy_orders, reverse=True):
                if bid <= HP_FAIR_INT:
                    break
                avail = hp_depth.buy_orders[bid]
                qty = min(avail, HP_LIMIT + position - sv)
                if qty <= 0:
                    break
                out.append(Order("HYDROGEL_PACK", bid, -qty)); sv += qty

            # 2. POSITION REDUCING AT FAIR: take orders priced AT 10000 that
            #    move |position| toward zero (frees inventory for higher-margin MM).
            pos_after = position + bv - sv
            if pos_after > 0 and HP_FAIR_INT in hp_depth.buy_orders:
                avail = hp_depth.buy_orders[HP_FAIR_INT]
                qty = min(avail, pos_after, HP_LIMIT + position - sv)
                if qty > 0:
                    out.append(Order("HYDROGEL_PACK", HP_FAIR_INT, -qty)); sv += qty
            if pos_after < 0 and HP_FAIR_INT in hp_depth.sell_orders:
                avail = -hp_depth.sell_orders[HP_FAIR_INT]
                qty = min(avail, -pos_after, HP_LIMIT - position - bv)
                if qty > 0:
                    out.append(Order("HYDROGEL_PACK", HP_FAIR_INT, qty)); bv += qty

            # 3. ALWAYS-ON PENNY JUMPING: post at best_bid+1 / best_ask-1 every tick,
            #    capped at fair-1 / fair+1 so we never quote at-or-through fair.
            best_bid = max(hp_depth.buy_orders); best_ask = min(hp_depth.sell_orders)
            bid_px = min(best_bid + 1, HP_FAIR_INT - 1)
            ask_px = max(best_ask - 1, HP_FAIR_INT + 1)
            if bid_px < ask_px:
            ...
            if out:
                orders["HYDROGEL_PACK"] = out

        # ---- VFE: keep Kalman-MR + informed-flow target bias (v27) ----
