...
class Trader:
    """v34: target-seeking MAKE skew on favorable regime.

    v27 goes symmetric-narrow when favorable (bid_edge = ask_edge = 1),
    a neutral posture that relies on market noise to randomly fill both
    sides. v32 (flipped skew) was -53 vs v27 — too aggressive, pushed
    position to limit too fast.

    v34 splits the difference: when favorable, the skew seeks a NON-ZERO
    target in the regime direction. Target magnitude = OSM_VOLUME_LIMIT
    (30, already used for the queue-jump threshold — zero new constants).

        if long_favorable:  target = +OSM_VOLUME_LIMIT
        elif short_favorable: target = -OSM_VOLUME_LIMIT
        else:               target = 0

        effective_pos = pos - target
        skew_make = round(effective_pos / OSM_SKEW_UNIT)

    Below target: skew_make negative → tight bid (accumulate toward target).
    At target:    skew_make ~0     → neutral (hold).
    Above target: skew_make positive → tight ask (unwind to target).

    Unlike v32 (unconditional sign-flip), v34 auto-dampens once position
    reaches the target — no over-commitment to drift. Below limit pressure
    builds the favorable-side inventory; above limit, natural unwinding.

    Adverse regime uses v22's skew (drive toward 0). CLEAR gate (v26) and
    TAKE envelope (v22) unchanged. Zero new parameters — target reuses
    OSM_VOLUME_LIMIT, a magnitude already calibrated in the strategy.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        if long_favorable:
            target = self.OSM_VOLUME_LIMIT
        elif short_favorable:
            target = -self.OSM_VOLUME_LIMIT
        else:
            target = 0
        if favorable_inv:
            skew_make = round((pos - target) / self.OSM_SKEW_UNIT)
        else:
            skew_make = skew
        bid_edge = max(1, self.OSM_MAKE_EDGE + skew_make)
        ask_edge = max(1, self.OSM_MAKE_EDGE - skew_make)
