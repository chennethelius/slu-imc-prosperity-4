...
class Trader:
    """v36: when favorable_inv, suppress the unwind-side MAKE leg.

    v27 (CI champion 199176) on favorable_inv posts symmetric narrow
    MAKE: bid_edge = ask_edge = 1. The queue-jump override reprices to
    fair+1 / fair-1. That lets the ask at fair+1 fill easily when we're
    long-favorable — we give away favorable inventory at ~fair, capping
    the mark-to-market gain the regime gate is trying to preserve.

    The CLEAR block already steps out when favorable_inv — good — but
    the MAKE block still posts an ask a single tick wide. Natural flow
    takes those asks often; the favorable thesis leaks through MAKE.

    v36: when favorable_inv, keep the accumulation-side MAKE leg only,
    drop the unwind-side leg entirely:
        long-favorable  → post BID MAKE only (keep adding)
        short-favorable → post ASK MAKE only (keep shorting)
    Non-favorable ticks fall back to v22/v27 two-sided skew MAKE.

    The hard position cap (±80) still caps accumulation. Once fair
    reverts past STATIC the regime flips, favorable_inv becomes false,
    and the MAKE block resumes posting both sides with v22 skew driving
    unwind — and CLEAR re-engages since the gate opens. Natural self-
    correction.

    v32 widened the unwind edge; v36 removes it. v32 lost 53 — possibly
    because queue-jump still kept a close ask in place via `baaf - 1`.
    Skipping the order entirely is the hard version of that idea.

    Zero new parameters. Pepper unchanged. All other osmium logic v27.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        long_fav_inv = pos > 0 and long_favorable
        short_fav_inv = pos < 0 and short_favorable
        favorable_inv = long_fav_inv or short_fav_inv
        if favorable_inv:
        ...
        if baaf is not None and bbbf is not None:
            ...
            buy_q = lim - pos - bv
            sell_q = lim + pos - sv
            if buy_q > 0 and not short_fav_inv:
                orders.append(Order("ASH_COATED_OSMIUM", bbbf + 1, buy_q))
            if sell_q > 0 and not long_fav_inv:
