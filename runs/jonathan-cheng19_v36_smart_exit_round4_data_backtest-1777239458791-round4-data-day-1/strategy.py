"""Round 4 v36 — v35 + smart-exit on conviction reversal.

v35 holds large positions (up to 1400 delta units) waiting for reversion.
When conviction direction flips during the hold, the existing position
becomes counter to the new signal — but v35's exit logic is purely
passive (rest at fair±1). MTM swings during this lag drive the DD.

v36 adds: when conviction direction is opposite to our existing position,
actively close that position by walking the book (until pos returns to
zero or modest level, then defer to passive MM).

   Detect: prev_pos_sign != 0 and direction != 0 and prev_pos_sign != direction
   Action: walk book to flatten the offending position before adding new

Applied to VFE only (HP module unchanged). VEV positions in the delta
chain follow VFE direction → when VFE reversal-closes, VEV positions
also close passively via existing flatten logic.

Expected: faster turnover during direction flips → less time-at-risk →
smaller DD without losing PnL (the alpha is re-captured in the new
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v36 SMART EXIT: when direction reverses against our position ===
    # If we're long and new direction is bearish (mid above fair, sell signal),
    # walk bids to flatten the position more aggressively than the passive MM.
    # Conviction must be non-zero (real reversal, not noise).
    if conviction > 0 and cfg["prefix"] == "vfe":
        if pos > 30 and direction < 0:
            # Long but bearish signal — close at touch
            rem = min(pos, cfg["take_max"])
            for bid in sorted(depth.buy_orders, reverse=True):
                if bid < fair - cfg["take_offset"] or rem <= 0: break
                avail = depth.buy_orders[bid]
                qty = min(avail, rem, LIM + pos - sv)
                if qty <= 0: break
                out.append(Order(sym, bid, -qty)); sv += qty; rem -= qty
                pos_after = pos + bv - sv
        elif pos < -30 and direction > 0:
            # Short but bullish signal — cover at touch
            rem = min(-pos, cfg["take_max"])
            for ask in sorted(depth.sell_orders):
                if ask > fair + cfg["take_offset"] or rem <= 0: break
                avail = -depth.sell_orders[ask]
                qty = min(avail, rem, LIM - pos - bv)
                if qty <= 0: break
