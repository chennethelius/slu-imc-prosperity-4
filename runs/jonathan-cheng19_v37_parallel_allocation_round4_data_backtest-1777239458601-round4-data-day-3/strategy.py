"""Round 4 v37 — v35 with parallel allocation (force VEV usage on every signal).

v35's overflow-only design wastes capacity: when VFE has plenty of room,
we route ZERO to VEV and run only on VFE. The VEV alpha is only captured
when VFE primary exhausts its allocation.

v37 forces parallel allocation: split target between VFE and VEV regardless
of VFE room. Effectively doubles signal capture per cycle.

   total_target = TAKE_MAX × conviction
   vfe_target = total_target × VFE_SHARE   (60%)
   vev_target = total_target × VEV_SHARE   (40%)

This trades some VFE-side fill rate for parallel VEV exposure. Since
VEV_4000/4500 trade at intrinsic with ~same spread cost as VFE, the
trade-off should be neutral on cost, doubling total alpha capture.
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v37 PARALLEL ALLOCATION ===
    # For VFE: split target into VFE-share and VEV-share, run both.
    # For HP: full target on HP only (no VEV proxies for HP).
    if cfg["prefix"] == "vfe":
        total_target = int(round(cfg["take_max"] * conviction)) if conviction > 0 else 0
        primary_target = int(round(total_target * 0.6))   # 60% to VFE
        vev_initial_share = total_target - primary_target  # 40% to VEV chain
    else:
    ...
    # === VFE-delta EXTENSION: VEV chain (parallel + overflow) ===
    if cfg["prefix"] == "vfe" and conviction > 0:
        # Initial VEV share + any unfilled primary
        overflow = vev_initial_share + (primary_target - primary_taken)
