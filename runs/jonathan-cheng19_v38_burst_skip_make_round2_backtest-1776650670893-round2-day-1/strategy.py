...
class Trader:
    """v38: on burst ticks, skip the MAKE leg on the swept side.

    v36 (leg-drop always-on when favorable) was catastrophic -11296 because
    suppressing unwind kept position piled into regime flips. Lesson: the
    MAKE unwind leg is load-bearing.

    But there's a narrower case where skipping IS defensible: when this
    tick saw violent directional flow, posting MAKE on the trailing side
    means quoting into a momentum wave that is likely to continue one
    more tick and run us over. Post-burst fair also hasn't fully caught
    up, so our quote is mispriced relative to where true fair is heading.

    Burst definition (zero threshold):
        micro_vol  = bv_tob + av_tob       (standing quote liquidity)
        trade_vol  = sum |q| in this tick's trades
        burst      = trade_vol > micro_vol

    Direction: trade_vwap vs fair:
        buy-pressure  → aggressors lifted asks → skip our ask MAKE this tick
        sell-pressure → aggressors hit bids   → skip our bid MAKE this tick

    Skip is ONE TICK only, not persistent. Non-burst ticks use full v27
    behavior. Because bursts are rare and self-resolving, position
    accumulation risk (v36's failure) is bounded.

    Zero new numerical parameters. Everything else (TAKE, CLEAR gate,
    MAKE edges, regime gate, pepper) is v27 verbatim.
    """
    ...
    def _osmium(self, d, pos, td, trades):
        ...
        for t in trades:
            ...
            trade_vol += q
            trade_val += t.price * q
        trade_vwap = trade_val / trade_vol if trade_vol > 0 else 0.0
        ...
        burst = trade_vol > micro_vol
        skip_ask_make = burst and trade_vol > 0 and trade_vwap > fair
        skip_bid_make = burst and trade_vol > 0 and trade_vwap < fair
        ...
        if baaf is not None and bbbf is not None:
            ...
            buy_q = lim - pos - bv
            if buy_q > 0 and not skip_bid_make:
                orders.append(Order("ASH_COATED_OSMIUM", bbbf + 1, buy_q))
            sell_q = lim + pos - sv
            if sell_q > 0 and not skip_ask_make:
