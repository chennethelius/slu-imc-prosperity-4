"""Round 4 v38 — v35 with capacity-scaled take_max for VFE.

v35's take_max=70 was sized for VFE alone (limit 200). With the full
chain (4000+4500+5000+5100 + VFE), we have 1400 delta units of capacity
but still only take 70 per signal — under-utilized.

v38 scales take_max by available delta capacity:
   capacity_factor = total_room_in_delta_units / 200
   take_max_scaled = take_max_base × capacity_factor

When all VEV vouchers are empty: factor ≈ 1100/200 = 5.5
When all caps reached: factor ≈ 0, no take

This couples take size to actual room available, exploiting the chain
capacity when it's there. Capped at 3x base to avoid huge bursts.
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v38 CAPACITY-SCALED TAKE_MAX (VFE only) ===
    # take_max scales with available delta capacity across VFE+VEV chain.
    if cfg["prefix"] == "vfe" and conviction > 0:
        if direction > 0:
            # Sum room across VFE + VEV chain on the LONG side
            total_room = max(0, cfg["limit"] - pos)
            for v in VEV_DELTA_VOUCHERS:
                v_pos = state.position.get(v["symbol"], 0)
                total_room += max(0, v["soft_cap"] - v_pos)
        else:
            total_room = max(0, cfg["limit"] + pos)
            for v in VEV_DELTA_VOUCHERS:
                v_pos = state.position.get(v["symbol"], 0)
                total_room += max(0, v["soft_cap"] + v_pos)
        capacity_factor = min(3.0, max(0.5, total_room / 200.0))
        take_max_scaled = cfg["take_max"] * capacity_factor
    else:
        take_max_scaled = cfg["take_max"]

    # Conviction take in primary product
    primary_target = int(round(take_max_scaled * conviction)) if conviction > 0 else 0
    primary_taken = 0
    if conviction > 0:
