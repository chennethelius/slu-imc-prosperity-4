...
class Trader:
    """v39: widen unwind-side MAKE by 1 tick when favorable_inv.

    v27 on favorable_inv: bid_edge = ask_edge = MAKE_EDGE (= 1).
    Symmetric narrow. The unwind leg at fair+1 (for long-favorable) gets
    lifted frequently — each fill leaks favorable inventory at ~fair,
    forfeiting the MtM edge the regime gate is trying to preserve.

    v36 dropped the leg entirely: catastrophic -11296 — regime flips
    found us at limit with no runway. v32 used -skew (wide): -53.
    The right posture sits between: gradual unwind, but slower than v27.

    v39 rule (zero new constants):
        When long_favorable+long:   ask_edge = OSM_TAKE_WIDTH (=2)
        When short_favorable+short: bid_edge = OSM_TAKE_WIDTH (=2)
        The add-side stays MAKE_EDGE (=1) — keep accumulating cheap.

    OSM_TAKE_WIDTH already calibrates one-tick separation logic in TAKE;
    reusing it as the hold-side MAKE edge on favorable is structurally
    consistent. Widens unwind by exactly one tick — half-step between
    v27 (0) and v32 (skew-magnitude).

    Non-favorable ticks use v22/v27 skewed edges. TAKE, CLEAR gate,
    pepper all v27 verbatim. Zero new parameters.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        if long_fav_inv:
            bid_edge = max(1, self.OSM_MAKE_EDGE)
            ask_edge = self.OSM_TAKE_WIDTH
        elif short_fav_inv:
            bid_edge = self.OSM_TAKE_WIDTH
            ask_edge = max(1, self.OSM_MAKE_EDGE)
        else:
