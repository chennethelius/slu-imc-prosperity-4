...
class Trader:
    """v40: adaptive CLEAR_WIDTH — tighter CLEAR as fair drifts from STATIC.

    v27 (CI champion 199176) uses fixed CLEAR_WIDTH=2: posts unwind at
    fair ± 2. When fair is right at STATIC (low conviction), ±2 is fine —
    don't rush. But when fair deviates from STATIC (large |fair-STATIC|),
    the mean-reversion thesis is more confident, and unwind at ±2 may
    leave inventory hanging during the excursion.

    Structural rule, zero new constants:
        cw_dyn = max(1, CLEAR_WIDTH - round(|fair - STATIC|))

    fair = STATIC:    cw_dyn = 2 (v27).
    |fair-STATIC|=1:  cw_dyn = 1 (tighter — catch book closer to fair).
    |fair-STATIC|>=2: cw_dyn = 1 (capped at 1 via max).

    Tighter CLEAR widens the set of book orders that qualify:
        f_ask = round(fair + cw_dyn)
        sum(v for p,v in d.buy_orders.items() if p >= f_ask)
    A lower f_ask matches more bids → higher CLEAR quantity.

    Regime gate from v26 still suppresses CLEAR when favorable_inv — the
    tighter cw only fires in adverse regime where inventory needs to move.
    The MAKE branch and TAKE remain v27 verbatim.

    Hypothesis: small-deviation adverse regimes benefit most, since the
    book likely has bids 1-tick above fair but not 2-tick. Deep-deviation
    regimes change less — tight markets rarely offer bids far from fair
    regardless of cw.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        cw = max(1, self.OSM_CLEAR_WIDTH - int(round(abs(fair - static))))
        orders = []
        bv = sv = 0
