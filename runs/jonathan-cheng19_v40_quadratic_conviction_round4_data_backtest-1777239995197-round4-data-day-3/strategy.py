"""Round 4 v40 — v35 with quadratic conviction-take scaling.

User directive: "I still believe there is excess inventory space that can
be put to use, find a way to scale up conviction or something to utilize
the excess inventory space."

v35 underutilizes the chain (4 vouchers + VFE = 1400 delta units of
capacity) because target = TAKE_MAX × conviction grows linearly.
Even at max conviction, target = 70 lots — far below the chain's room.

v40 scales target QUADRATICALLY in conviction:
   target = TAKE_MAX_BASE × conviction × (1 + conviction)

At conviction = 0.3: target = 70 × 0.3 × 1.3 = 27   (≈ same as v35's 21)
At conviction = 0.7: target = 70 × 0.7 × 1.7 = 83   (vs 49 in v35, +69%)
At conviction = 1.0: target = 70 × 1.0 × 2.0 = 140  (vs 70 in v35, +100%)

Low-conviction behavior unchanged. High-conviction events get doubled
size — uses more chain capacity when the signal warrants it. Soft caps
unchanged so worst-case position growth is bounded.

Also applies to HP (HP has limit 200, no chain extension, but quadratic
scaling still amplifies high-conviction takes within HP's own room).
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v40 QUADRATIC CONVICTION SCALING ===
    # target = TAKE_MAX × conviction × (1 + conviction)
    # Doubles size at conviction=1, unchanged at low conviction.
    if conviction > 0:
        primary_target = int(round(cfg["take_max"] * conviction * (1.0 + conviction)))
    else:
        primary_target = 0
    primary_taken = 0
    if conviction > 0:
