...
class Trader:
    """v41: override favorable-regime CLEAR gate when position is deep.

    v26/v27 regime gate skips CLEAR when favorable_inv — the core reason
    v26 gained +127 over v22 (holds drift-favorable inventory through
    reversion). But the gate is binary: any favorable regime = hold,
    regardless of how much inventory we're holding.

    At |pos| > OSM_VOLUME_LIMIT (= 30) we're materially past the level
    calibrated as the queue-jump threshold — a soft \"don't accumulate
    beyond this\" boundary elsewhere in the strategy. Above this, the
    risk of being caught at the hard limit (±80) during an adverse
    regime flip outweighs the MtM upside of holding more favorable
    inventory.

    v41 rule: reuse OSM_VOLUME_LIMIT as an override on the CLEAR gate.
        force_clear = |pos| > OSM_VOLUME_LIMIT
        if pos_after > 0 and (not long_favorable or force_clear): CLEAR
        elif pos_after < 0 and (not short_favorable or force_clear): CLEAR

    Below the threshold: full v27 behavior (hold favorable).
    Above: CLEAR fires even on favorable, trimming toward threshold
    without going all the way to zero (CLEAR only takes book liquidity,
    natural self-limiting).

    Unlike v34 (target-seek MAKE, -126): v41 leaves MAKE untouched,
    only gates CLEAR. The MAKE skew drives slow regime-aware balancing;
    the CLEAR override is a hard safety valve for extreme positions.

    Zero new parameters. Pepper unchanged. TAKE, MAKE edges, regime
    detection all v27 verbatim.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        force_clear = abs(pos) > self.OSM_VOLUME_LIMIT
        if pos_after > 0 and (not long_favorable or force_clear):
        ...
        elif pos_after < 0 and (not short_favorable or force_clear):
