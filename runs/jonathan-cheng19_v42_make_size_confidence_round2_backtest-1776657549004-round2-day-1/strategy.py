...
class Trader:
    """v42: scale MAKE order size by Kalman confidence.

    Every prior variant (v27..v41) modified MAKE price or gate. None
    touched MAKE size — v27 always posts the full remaining capacity
    on both legs. That means on noisy ticks (where err_ema is high
    and Kalman innovations are large), we post large MAKE size against
    potentially mispriced fair — classic adverse-selection exposure.

    v42 scales MAKE size by confidence:
        confidence = 1 / (1 + err_ema)
        buy_q  = round((lim - pos - bv) * confidence)
        sell_q = round((lim + pos - sv) * confidence)

    err_ema is already computed in the Kalman update. Low err_ema =
    tight tracking, full size (v27). Rising err_ema = shrinking size
    toward 0. Self-normalizing: no threshold, no magic multiplier.

    Intuition: on calm ticks we harvest passive spread at full capacity.
    On volatile ticks we expose less inventory — the market is moving,
    our fair may be stale, fills here tend to be adverse. Leaves
    capacity unallocated this tick; next tick (as err_ema decays) we
    can post fuller.

    Unlike v39 (edge widening) which pushes QUOTE price away, v42
    keeps the aggressive queue-jump pricing but reduces inventory at
    risk per noisy tick. Different axis entirely.

    TAKE, CLEAR gate, pepper, regime logic all v27 verbatim.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        if baaf is not None and bbbf is not None:
            ...
            buy_q = int(round((lim - pos - bv) * confidence))
            if buy_q > 0:
                orders.append(Order("ASH_COATED_OSMIUM", bbbf + 1, buy_q))
            sell_q = int(round((lim + pos - sv) * confidence))
            if sell_q > 0:
