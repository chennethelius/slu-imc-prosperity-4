"""Round 4 v43 — v42 + OBI adverse-selection filter on MM-leg sizing.

OBI (corr -0.33 at lag 1 with mid forward returns) was tested in v32 as
a conviction blend component, with no PnL traction (lag too short for
conviction-take to capture).

v43 applies OBI differently: as an ADVERSE-SELECTION FILTER on MM-leg
sizes. Resting MM quotes get hit when book moves against us:
   - bid-heavy book (imb > 0) → mid will drop → our resting bid will get
     hit at unfavorable prices (we buy high, then mid drops). REDUCE bid.
   - ask-heavy book (imb < 0) → mid will rise → our resting ask will get
     hit at unfavorable prices (we sell low, then mid rises). REDUCE ask.

This is the standard market-maker response to flow imbalance: pull back
the side likely to be adversely selected.

   imb = (bid_vol - ask_vol) / (bid_vol + ask_vol)
   if imb > OBI_THRESH:  buy_mult *= (1 - OBI_FACTOR * imb)   # smaller bid
   if imb < -OBI_THRESH: sell_mult *= (1 + OBI_FACTOR * imb)  # smaller ask

Skip applies when MM quote is on the alpha-confirming side
(v41's conviction boost is the dominant signal there). OBI filter only
trims size on the OPPOSITE side from alpha.
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v43: OBI adverse-selection filter (continuous, both sides) ===
    ...
    if bid_vol_total + ask_vol_total > 0:
        ...
        OBI_FACTOR = 0.5
        # imb > 0 (bid-heavy → mid drops): trim bid (avoid adverse), grow ask (capture sellers)
        # imb < 0 (ask-heavy → mid rises): grow bid (capture buyers), trim ask
        bm *= max(0.2, 1.0 - OBI_FACTOR * imb)
        sm *= max(0.2, 1.0 + OBI_FACTOR * imb)
