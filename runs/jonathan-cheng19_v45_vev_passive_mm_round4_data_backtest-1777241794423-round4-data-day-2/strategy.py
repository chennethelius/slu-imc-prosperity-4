"""Round 4 v45 — v42 + always-on passive MM on ITM VEV vouchers.

Currently VEV_4000/4500 only get fills via the take-overflow path or the
flatten logic. Between those events, no VEV orders are posted — incidental
flow on those vouchers passes us by.

v45 adds always-on passive MM on the deep-ITM vouchers (VEV_4000, VEV_4500
only — these have stable intrinsic = VFE - K, no premium volatility):
   bid at min(BB+1, intrinsic - 1)
   ask at max(BA-1, intrinsic + 1)
   small symmetric size (15 lots default)
   inventory pull-to-zero applied for asymmetric sizing

Captures spread on flow not associated with our take signal. Skipped on
ATM vouchers (5000+) where premium dynamics make intrinsic an unreliable
reference.
...
def _vev_flatten_orders(state, td):
    """Passive close-out for non-zero inventory + always-on passive MM
    on deep-ITM vouchers (VEV_4000/4500 only).
    """
    out_by_sym = {}
    ...
    DEEP_ITM_STRIKES = {4000, 4500}   # always-on MM only on these
    PASSIVE_QSIZE = 15
    for v in VEV_DELTA_VOUCHERS:
        ...
        elif v_strike in DEEP_ITM_STRIKES:
            # v45: always-on passive MM when flat (deep ITM only)
            bid_px = min(v_bb + 1, intrinsic - 1)
            ask_px = max(v_ba - 1, intrinsic + 1)
            if bid_px < ask_px:
                buy_q = min(PASSIVE_QSIZE, v_limit - v_pos)
                sell_q = min(PASSIVE_QSIZE, v_limit + v_pos)
                if buy_q > 0:
                    orders.append(Order(v_sym, bid_px, buy_q))
                if sell_q > 0:
                    orders.append(Order(v_sym, ask_px, -sell_q))
