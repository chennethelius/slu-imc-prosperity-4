"""Round 4 v51 — v42 + HP adverse-selection feedback loop.

Microstructure analysis showed HP SELL fills had fwd_ret 5k = +1.44/lot
(adverse — mid rose after we sold). 358 sells in R4-d3 × ~6 lots avg ×
1.44 = ~3k MTM drag in 5k window. Long-term reversion recovers some,
but the short-term drag is real.

v51 adds closed-loop feedback: track HP own_trades from K ticks ago.
For each completed sell, check current mid vs sell price. If multiple
recent sells were adverse (mid rose after), pull back HP sell size
temporarily until conditions normalize.

   adverse_count = #recent HP sells where mid_now > sell_price + 1
   if adverse_count >= 3: hp_sell_mult *= 0.5

Independent for buy direction (track HP buys, mid drops after = adverse).

Tracked in trader_data via running sell/buy "adverse score" decayed at
each tick.
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # v51: HP adverse-selection feedback
    if cfg["prefix"] == "hp":
        buy_adv = td.get("_hp_buy_adv", 0.0)
        sell_adv = td.get("_hp_sell_adv", 0.0)
        if buy_adv > 5.0:
            bm *= max(0.3, 1.0 - 0.1 * buy_adv)
        if sell_adv > 5.0:
            sm *= max(0.3, 1.0 - 0.1 * sell_adv)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # === v51: HP adverse-selection feedback ===
        # Track recent HP own_trades and update adverse-score per direction.
        # Score is exponentially decayed; spikes when recent fills go adverse.
        hp_depth = state.order_depths.get("HYDROGEL_PACK")
        if hp_depth and hp_depth.buy_orders and hp_depth.sell_orders:
            hp_mid = (max(hp_depth.buy_orders) + min(hp_depth.sell_orders)) / 2.0
            buy_adv = td.get("_hp_buy_adv", 0.0)
            sell_adv = td.get("_hp_sell_adv", 0.0)
            buy_adv *= 0.85   # decay
            sell_adv *= 0.85
            # Examine HP own_trades from the prior tick (just landed)
            for t in (state.own_trades.get("HYDROGEL_PACK") or []):
                if t.timestamp != state.timestamp - 100:   # last tick only
                    continue
                if t.buyer == "SUBMISSION":
                    # We bought at t.price; if mid now < t.price, fill was adverse
                    diff = hp_mid - t.price
                    if diff < -1:
                        buy_adv += abs(diff)
                    ...
                    if diff > 1:
                        sell_adv += diff
            td["_hp_buy_adv"] = buy_adv
            td["_hp_sell_adv"] = sell_adv
