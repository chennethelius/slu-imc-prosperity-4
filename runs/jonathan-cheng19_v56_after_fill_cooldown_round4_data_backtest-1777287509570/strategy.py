...
VEV_DELTA_VOUCHERS = [
    # soft_cap_max: dynamic upper limit at conviction>=0.7; soft_cap is base
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # v56: after-fill cooldown — reduce same-side quote after fill
    sym = cfg["symbol"]
    last_buy_ts = td.get(f"_{sym}_lastBuy_ts", -1e9)
    last_sell_ts = td.get(f"_{sym}_lastSell_ts", -1e9)
    COOLDOWN_TICKS = 200   # 2 ticks
    if state.timestamp - last_buy_ts < COOLDOWN_TICKS:
        bm *= 0.6
    if state.timestamp - last_sell_ts < COOLDOWN_TICKS:
        sm *= 0.6
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # === v56: after-fill cooldown tracking ===
        # Update last-fill-ts per direction per product.
        for sym in ("HYDROGEL_PACK", "VELVETFRUIT_EXTRACT"):
            for t in (state.own_trades.get(sym) or []):
                if t.timestamp != state.timestamp - 100: continue
                if t.buyer == "SUBMISSION":
                    td[f"_{sym}_lastBuy_ts"] = t.timestamp
                elif t.seller == "SUBMISSION":
                    td[f"_{sym}_lastSell_ts"] = t.timestamp
