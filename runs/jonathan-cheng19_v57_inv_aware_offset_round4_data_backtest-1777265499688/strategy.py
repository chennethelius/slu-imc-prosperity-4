...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    primary_taken = 0
    # v57: inventory-aware take_offset — tighter when already loaded
    inv_pct = abs(pos) / LIM
    eff_take_offset = max(1, cfg["take_offset"] - int(round(inv_pct * 2)))
    if conviction > 0:
        if direction > 0 and pos_after < soft_cap:
            max_pay = fair + eff_take_offset; rem = primary_target
            for ask in sorted(depth.sell_orders):
            ...
            min_recv = fair - eff_take_offset; rem = primary_target
            for bid in sorted(depth.buy_orders, reverse=True):
