...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v64: active VEV chain unwind when direction REVERSES ===
    # When VFE conviction direction conflicts with existing VEV positions,
    # actively cross to close them (instead of waiting for passive flatten).
    if cfg["prefix"] == "vfe" and conviction > 0:
        ...
        for v in VEV_DELTA_VOUCHERS:
            ...
            v_offset = v.get("offset", 2)
            v_orders = []
            # If we're long and direction is now bearish, close long
            if direction < 0 and v_pos > 0:
                # Sell at best bid (cross down)
                bb = max(v_depth.buy_orders)
                if bb >= intrinsic - v_offset:   # only close if price reasonable
                    avail = v_depth.buy_orders[bb]
                    qty = min(avail, v_pos, 30)   # cap unwind rate
                    if qty > 0:
                        v_orders.append(Order(v_sym, bb, -qty))
            elif direction > 0 and v_pos < 0:
                ba = min(v_depth.sell_orders)
                if ba <= intrinsic + v_offset:
                    avail = -v_depth.sell_orders[ba]
                    qty = min(avail, -v_pos, 30)
                    if qty > 0:
                        v_orders.append(Order(v_sym, ba, qty))
            if v_orders:
                if v_sym in all_orders:
                    all_orders[v_sym].extend(v_orders)
                else:
