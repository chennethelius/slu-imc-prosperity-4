...
def _conviction_signals(depth, pos, td, market_trades, cfg):
    ...
    if market_trades:
        ...
        recent = market_trades[-cfg["informed_lookback"]:]
        n = len(recent)
        for i, t in enumerate(recent):
            buyer = (t.buyer or ""); seller = (t.seller or "")
            qty = int(t.quantity)
            # v66: recency decay — most recent gets weight 1.0, oldest 1/n
            recency_w = (i + 1) / n   # i=n-1 (most recent) → 1.0; i=0 (oldest) → 1/n
            net_inf += weights.get(buyer, 0.0) * qty * recency_w
            net_inf -= weights.get(seller, 0.0) * qty * recency_w
