...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    primary_taken = 0
    # v67: HP-asymmetric take offset (tighter sell to reduce adverse fills)
    take_offset_buy = cfg["take_offset"]
    take_offset_sell = cfg["take_offset"] - 1 if cfg["prefix"] == "hp" else cfg["take_offset"]
    if conviction > 0:
        if direction > 0 and pos_after < soft_cap:
            max_pay = fair + take_offset_buy; rem = primary_target
            for ask in sorted(depth.sell_orders):
            ...
            min_recv = fair - take_offset_sell; rem = primary_target
            for bid in sorted(depth.buy_orders, reverse=True):
