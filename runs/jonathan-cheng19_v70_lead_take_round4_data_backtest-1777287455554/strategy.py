"""Round 4 v70 — v62 + standalone lead-lag take trigger on VFE.

Cross-product price-discovery breakthrough:
   vev_5000_delta = 0.673 * vfe_delta  (R²=0.58 on R4-d3, n=9999)
   residual > +0.5: VEV overshot → fwd VFE 5-tick mean +0.265 (BUY VFE)
   residual < -0.5: VEV undershot → fwd VFE 5-tick mean -0.328 (SELL VFE)

v68 (conviction blend) and v69 (MM-skew) didn't capture this — passive
quotes can't react fast enough to sub-tick moves predicted by lead-lag.

v70: STANDALONE TAKE on VFE when residual exceeds threshold. Cross to
buy/sell at the touch immediately (before VFE catches up to the VEV-implied
move). Bypass conviction-MR entirely — this is independent alpha.

Position cap kept conservative (50 lots per residual event) to avoid
over-loading. Layered ON TOP of v62's regular conviction take/MM.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # === v70: STANDALONE LEAD-LAG TAKE on VFE ===
        # Compute VEV→VFE residual; if large, take VFE in residual direction.
        vfe_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
        vev_depth = state.order_depths.get("VEV_5000")
        if (vfe_depth and vfe_depth.buy_orders and vfe_depth.sell_orders and
            vev_depth and vev_depth.buy_orders and vev_depth.sell_orders):
            vfe_mid = (max(vfe_depth.buy_orders) + min(vfe_depth.sell_orders)) / 2.0
            vev_mid = (max(vev_depth.buy_orders) + min(vev_depth.sell_orders)) / 2.0
            prev_vfe = td.get("_lag_vfe_prev", vfe_mid)
            prev_vev = td.get("_lag_vev_prev", vev_mid)
            vfe_d = vfe_mid - prev_vfe
            vev_d = vev_mid - prev_vev
            residual = vev_d - 0.673 * vfe_d
            td["_lag_vfe_prev"] = vfe_mid
            td["_lag_vev_prev"] = vev_mid

            LAG_TAKE_THRESH = 2.5
            LAG_TAKE_MAX = 20
            vfe_pos = state.position.get("VELVETFRUIT_EXTRACT", 0)
            vfe_lag_orders = []
            if residual > LAG_TAKE_THRESH and vfe_pos < 150:
                # Predict VFE up — buy VFE
                ba = min(vfe_depth.sell_orders)
                avail = -vfe_depth.sell_orders[ba]
                qty = min(avail, LAG_TAKE_MAX, 200 - vfe_pos)
                if qty > 0:
                    vfe_lag_orders.append(Order("VELVETFRUIT_EXTRACT", int(ba), qty))
            elif residual < -LAG_TAKE_THRESH and vfe_pos > -150:
                # Predict VFE down — sell VFE
                bb = max(vfe_depth.buy_orders)
                avail = vfe_depth.buy_orders[bb]
                qty = min(avail, LAG_TAKE_MAX, 200 + vfe_pos)
                if qty > 0:
                    vfe_lag_orders.append(Order("VELVETFRUIT_EXTRACT", int(bb), -qty))
            if vfe_lag_orders:
                orders["VELVETFRUIT_EXTRACT"] = vfe_lag_orders
        ...
        for cfg in (HP_CFG, VFE_CFG):
            ...
            if depth:
                ...
                if ors:
                    if sym in orders:
                        orders[sym].extend(ors)   # don't override v70 lead-lag take
                    else:
                        orders[sym] = ors
