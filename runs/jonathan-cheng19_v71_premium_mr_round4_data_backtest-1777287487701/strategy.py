"""Round 4 v71 — v62 + VEV premium-MR module (BREAKTHROUGH attempt #2).

VEV_5000 premium = mid - max(0, vfe_mid - 5000) is highly auto-correlated
(AC lag-5 = 0.72) and mean-reverts strongly:
   premium > 3.5: fwd 5-tick change = -0.361 (reverts DOWN)
   premium < 1.5: fwd 5-tick change = +0.578 (reverts UP)

Strategy: track rolling premium mean+std via EWMA per voucher. Post
PASSIVE MM quotes biased toward premium-MR direction. Skip chain
extension on this voucher when premium is at extreme.
...
# === v71: VEV premium-MR module ===
# Track rolling mean/std of premium via EWMA, post passive MM biased to MR
PREMIUM_VOUCHERS = [
    {"symbol": "VEV_5000", "strike": 5000, "qsize": 25, "limit": 300, "soft_cap": 60},
    {"symbol": "VEV_5100", "strike": 5100, "qsize": 20, "limit": 300, "soft_cap": 50},
]


def _premium_mr_orders(state, td):
    """Posts MM quotes biased toward premium MR. Returns dict[sym, [orders]]."""
    out_by_sym = {}
    vfe_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
    if not vfe_depth or not vfe_depth.buy_orders or not vfe_depth.sell_orders:
        return out_by_sym
    ...
    for v in PREMIUM_VOUCHERS:
        sym = v["symbol"]; K = v["strike"]; LIM = v["limit"]; SOFT_CAP = v["soft_cap"]
        v_depth = state.order_depths.get(sym)
        if not v_depth or not v_depth.buy_orders or not v_depth.sell_orders: continue
        v_bb = max(v_depth.buy_orders); v_ba = min(v_depth.sell_orders)
        v_mid = (v_bb + v_ba) / 2.0
        intrinsic = max(0, vfe_mid - K)
        premium = v_mid - intrinsic

        # EWMA of premium
        m_key = f"_prem_{K}_mean"; v_key = f"_prem_{K}_var"
        prem_mean = td.get(m_key, premium)
        prem_var = td.get(v_key, 1.0)
        alpha_m = 0.005
        prem_mean = (1 - alpha_m) * prem_mean + alpha_m * premium
        prem_var = (1 - alpha_m) * prem_var + alpha_m * (premium - prem_mean) ** 2
        prem_std = max(0.5, prem_var ** 0.5)
        td[m_key] = prem_mean; td[v_key] = prem_var

        z_prem = (premium - prem_mean) / prem_std
        v_pos = state.position.get(sym, 0)

        orders = []
        # If premium HIGH: short bias (post bigger ask)
        # If premium LOW: long bias (post bigger bid)
        # DEBUG: cross to test if signal makes money even at spread cost
        if z_prem > 1.5 and v_pos > -SOFT_CAP:
            # Premium very high — cross down (hit bid) to short
            avail = v_depth.buy_orders[v_bb]
            qty = min(v["qsize"], avail, LIM + v_pos, SOFT_CAP + v_pos)
            if qty > 0:
                orders.append(Order(sym, int(v_bb), -qty))
        elif z_prem < -1.5 and v_pos < SOFT_CAP:
            # Premium very low — cross up (lift ask) to long
            avail = -v_depth.sell_orders[v_ba]
            qty = min(v["qsize"], avail, LIM - v_pos, SOFT_CAP - v_pos)
            if qty > 0:
                orders.append(Order(sym, int(v_ba), qty))

        if orders:
            out_by_sym[sym] = orders
    return out_by_sym
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # v71: premium-MR orders on VEV_5000/5100
        prem_orders = _premium_mr_orders(state, td)
        for s, ors in prem_orders.items():
            if s in orders:
                orders[s].extend(ors)
            else:
                orders[s] = ors
