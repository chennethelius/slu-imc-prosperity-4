"""Round 4 v74 — v62 + OTM voucher anchor-divergence pipeline.

v62 only trades VEV strikes 4000-5300 (via VFE chain extension). On every
day of the d3 backtest, the unused strikes saw substantial market activity:

    VEV_5400  ~115 trades/day  (v62 captured 0)
    VEV_5500  ~120 trades/day  (v62 captured 0)
    VEV_6000  ~124 trades/day  (v62 captured 0)
    VEV_6500  ~124 trades/day  (v62 captured 0)

v74 grafts an anchor-divergence pipeline (lifted from super_combined) onto
v62. For each OTM strike, it maintains a rolling-mean anchor of mid_price,
takes against |mid-anchor| > threshold after warmup, and runs an always-on
MM quote and free-cross PnL leg. Pure additive vs v62 — the existing
HP/VFE/VEV_4000-5300 logic is unchanged; v74 only writes orders for
symbols v62 ignores entirely.
...
# ============================================================================
# OTM voucher anchor-divergence pipeline (lifted from super_combined)
# Trades VEV strikes 5400/5500/6000/6500 — products v62 ignores entirely.
# ============================================================================

import math as _math

_OTM_TAKE_WIDTH = 1
_OTM_ANCHOR_WARMUP = 100
_OTM_DIVERGE_TAKE_SIZE = 30
_OTM_MAX_POS = 295  # near hard limit
...
def _otm_walk_book(depth, side, sym, ok, qty_target):
    """Walk asks (side=+1) or bids (side=-1) until ok(price) is false or qty_target hit."""
    if side > 0:
...
def _otm_full_depth_mid(depth):
    bids = list(depth.buy_orders.items())
    asks = list(depth.sell_orders.items())
    if not bids or not asks:
        return None
    bvol = sum(abs(v) for _, v in bids)
    avol = sum(abs(v) for _, v in asks)
    bw = sum(p * abs(v) for p, v in bids) / bvol if bvol else 0
    aw = sum(p * abs(v) for p, v in asks) / avol if avol else 0
    return (bw + aw) / 2


def _otm_anchor_orders(cfg, state, scratch):
    """Divergence-take + free crosses + always-on MM quote for one OTM strike."""
    sym = cfg["product"]
    ...
    bb = max(depth.buy_orders); ba = min(depth.sell_orders)
    mid = (bb + ba) / 2
    fair = _otm_full_depth_mid(depth)
    if fair is None:
        return []
    ...
    diverge = mid - anchor
    if threshold > 0 and n >= _OTM_ANCHOR_WARMUP and abs(diverge) >= threshold:
        if diverge > 0 and pos > -_OTM_MAX_POS:
            qty_target = min(_OTM_DIVERGE_TAKE_SIZE, limit + pos, pos + _OTM_MAX_POS)
            takes, filled = _otm_walk_book(depth, -1, sym, accept_any, qty_target)
            out.extend(takes); sv += filled
        elif diverge < 0 and pos < _OTM_MAX_POS:
            qty_target = min(_OTM_DIVERGE_TAKE_SIZE, limit - pos, _OTM_MAX_POS - pos)
            takes, filled = _otm_walk_book(depth, +1, sym, accept_any, qty_target)
            out.extend(takes); bv += filled

    pos_eff = pos + bv - sv
    takes, filled = _otm_walk_book(
        depth, +1, sym, lambda px: px < fair - _OTM_TAKE_WIDTH, limit - pos_eff,
    )
    out.extend(takes); bv += filled
    takes, filled = _otm_walk_book(
        depth, -1, sym, lambda px: px > fair + _OTM_TAKE_WIDTH, limit + pos_eff,
    ...
    qsize = cfg["qsize"]
    bid_px = min(int(_math.floor((fair + bb) / 2)), ba - 1)
    ask_px = max(int(_math.ceil((fair + ba) / 2)), bb + 1)
    if bid_px < ask_px:
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # OTM anchor pipeline for VEV_5400/5500/6000/6500 — strikes v62 ignores.
        otm_state = td.setdefault("_otm", {})
        for cfg in _OTM_CFGS:
            sym = cfg["product"]
            scratch = otm_state.setdefault(sym, {})
            ors = _otm_anchor_orders(cfg, state, scratch)
            if ors:
                if sym in orders:
                    orders[sym].extend(ors)
                else:
                    orders[sym] = ors
