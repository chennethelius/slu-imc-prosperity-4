"""
Round 4 v79 — v78 with Mark-aware voucher anchor pipeline.

3-day forward-mid analysis revealed counterparty alpha on the VEV strikes
that the anchor-divergence pipeline currently ignores:

  VEV_4000  Mark 14 +10.5 (n=258),  Mark 38 -9.9 (n=448)   <- huge!
  VEV_5300  Mark 01 +0.76 (n=132),  Mark 22 -0.71 (n=163)
  VEV_5400  Mark 01 +0.59 (n=263),  Mark 22 -0.56 (n=276)
  VEV_5500  Mark 01 +0.51 (n=299),  Mark 22 -0.51 (n=306)
  VEV_6000  Mark 01 +0.50 (n=317),  Mark 22 -0.50 (n=317)
  VEV_6500  Mark 01 +0.50 (n=317),  Mark 22 -0.50 (n=317)

The anchor pipeline's blind divergence-take fades these informed flows:
when Mark 14 buys VEV_4000 hard, mid moves above the lagging anchor
faster than the anchor catches up, |diverge| > threshold, we sell
into Mark 14's position. v79 adds a Mark-net gate to skip the
divergence-take when recent informed flow agrees with the price move,
i.e. don't fade what's about to keep going.

Per-voucher mark weights are added to ANCHOR_VEV_CFGS. The conviction
pipeline used by HP/VFE is untouched (already corrected in v78).

Original v78 docstring follows:

Round 4 v78 — v77 with corrected VFE counterparty weights.

3-day forward-mid analysis (5/10/20/30/50/100 tick horizons) revealed
super_combined's VFE mark_weights have wrong-sign weights on the two
most active counterparties and are missing the single most informed
counterparty entirely:

  mark        n     5t      10t     20t     30t     50t    100t   prev   new
  Mark 01    400   +2.75   +2.86   +2.54   +2.44   +2.36   +1.76   miss  +1.5
  Mark 14    490   +2.34   +2.29   +2.07   +1.78   +1.59   +0.90   -0.5  +1.5
  Mark 22    114   -0.55   -0.46   -0.09   -0.60   -0.54   +0.92   -1.0  -1.0
  Mark 49    110   -1.12   -1.32   -1.18   -1.22   -1.29   -1.19   -1.5  -1.5
  Mark 55   1343   -2.13   -2.17   -1.95   -1.86   -1.90   -1.69   +0.5  -1.5
  Mark 67    176   +1.30   +1.69   +1.37   +1.75   +1.55   +0.74   +1.5  +1.5

Sign of the realized future-mid edge stays consistent across every
horizon for every Mark, so the corrections are robust (not a
horizon-dependent artifact). Mark 14 / Mark 55 alone account for 65%
of VFE counterparty flow — fixing those sign flips is the highest-
leverage change available without altering the core conviction
pipeline.

Original v77 docstring follows:

Round 4 v77 — super_combined + VEV_6000/6500 anchor pipeline.

super_combined caps its anchor-divergence pipeline at VEV_5500. The deeper
tails (6000, 6500) see ~120 market trades/day each that nothing in the
roster captures. Adding them with the same anchor + always-on MM quote
recipe is pure additive — limit cap is still 300, threshold 2 only fires
on real deviations, and the deep-OTM strikes have minimal trend risk
(prices hover near zero for both). Worst case +0, expected case ~+300/day.

Original super_combined docstring follows:
...
VFE_CFG = {
    ...
    "base_cap_pct": 0.50, "full_cap_pct": 1.00, "hard_cap_pct": 0.95,
    "mark_weights": {
        # Corrected per 3-day forward-mid edge analysis (see file docstring).
        "Mark 01": +1.5, "Mark 14": +1.5, "Mark 67": +1.5,  # follow (informed bullish on buys)
        "Mark 22": -1.0, "Mark 49": -1.5, "Mark 55": -1.5,  # fade (consistently wrong-side)
    },
}
...
# mark_weights are signed per-counterparty conviction multipliers:
# positive = follow (their buys are bullish), negative = fade.
# mark_lookback / mark_full define the window and saturation as in
# the conviction pipeline. Empty {} disables the mark gate for the strike.
ANCHOR_VEV_CFGS = [
    # ITM strikes — v01 wins (no gating, higher base threshold)
    # VEV_4000 has BIG mark alpha (Mark 14 +10.5, Mark 38 -9.9) — strong gate.
    {"product": "VEV_4000", "limit": 300, "qsize": 30, "diverge_threshold": 25,
     "regime_mode": "off",       "use_vol_adjust": False,
     "mark_weights": {"Mark 14": +1.5, "Mark 38": -1.5},
     "mark_lookback": 10, "mark_full": 30},
    {"product": "VEV_4500", "limit": 300, "qsize": 30, "diverge_threshold": 25,
     "regime_mode": "off",       "use_vol_adjust": False,
     "mark_weights": {}},
    {"product": "VEV_5400", "limit": 300, "qsize": 30, "diverge_threshold": 5,
     "regime_mode": "off",       "use_vol_adjust": False,
     "mark_weights": {"Mark 01": +0.5, "Mark 22": -0.5},
     "mark_lookback": 12, "mark_full": 40},
    ...
    {"product": "VEV_5200", "limit": 300, "qsize": 30, "diverge_threshold": 11,
     "regime_mode": "off",       "use_vol_adjust": False,
     "mark_weights": {}},
    # Deep OTM tails — v74 found these had 100+ unfilled market trades/day.
...
def _anchor_vev_orders(cfg, state, scratch, regime_scale):
    ...
    v79: skips the divergence-take when recent counterparty flow agrees
    with the price move — i.e., don't fade what informed flow says will
    keep moving.
    """
    sym = cfg["product"]
    depth = state.order_depths.get(sym)
    ...
    # Mark-net: signed counterparty conviction over last mark_lookback trades.
    # Positive net = informed buying (price expected to keep rising).
    mark_w = cfg.get("mark_weights") or {}
    mark_net = 0.0
    if mark_w:
        mts = state.market_trades.get(sym, []) or []
        for t in mts[-cfg.get("mark_lookback", 10):]:
            q = int(t.quantity)
            mark_net += mark_w.get(t.buyer or "", 0.0) * q
            mark_net -= mark_w.get(t.seller or "", 0.0) * q
        mark_net /= max(1.0, cfg.get("mark_full", 30))
        mark_net = max(-1.0, min(1.0, mark_net))
    ...
    if threshold > 0 and n >= ANCHOR_WARMUP and abs(diverge) >= threshold:
        ...
        # Skip the fade if informed flow agrees with the price move.
        # diverge > 0 (mid above anchor) wants to sell; mark_net > 0.3 says
        # informed flow is bullish → don't fade. Mirror for the buy leg.
        skip_sell = mark_net > 0.3
        skip_buy = mark_net < -0.3
        if diverge > 0 and pos > -max_pos and not skip_sell:
        ...
        elif diverge < 0 and pos < max_pos and not skip_buy:
