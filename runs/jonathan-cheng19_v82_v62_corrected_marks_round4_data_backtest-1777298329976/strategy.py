"""Round 4 v82 — v62 with corrected VFE counterparty mark weights.

v62 IMC submission scored 32,456 (best on the dashboard); the dashboard
top contender super_combined only scores 2,504-17,009 on actual IMC
runs because its anchor-divergence pipeline depends on aggressive
fills that don't translate to the IMC server's stricter matching.

3-day forward-mid analysis showed v62's VFE mark_weights have two
sign-flipped weights and miss the most informed counterparty entirely:

  mark        n     5-100t edge   prev   v82
  Mark 01    400   +1.76 to +2.86 miss   +1.5   <- new
  Mark 14    490   +0.90 to +2.34 -0.5   +1.5   <- sign flipped
  Mark 55   1343   -1.69 to -2.17 +0.5   -1.5   <- sign flipped

This is a one-line config change to v62 (no algorithmic changes, no
anchor pipeline). Only the conviction signal's `inf_str` term is
affected — the chain-extension and HP/VFE primary trading are
identical. So fill rates on IMC should match v62's pattern.
...
VFE_CFG = {
    ...
    "base_cap_pct": 0.50, "full_cap_pct": 1.00, "hard_cap_pct": 0.95,
    "mark_weights": {
        # v82 corrections per 3-day forward-mid edge analysis:
        "Mark 01": +1.5, "Mark 14": +1.5, "Mark 67": +1.5,  # follow
        "Mark 22": -1.0, "Mark 49": -1.5, "Mark 55": -1.5,  # fade
    },
}
