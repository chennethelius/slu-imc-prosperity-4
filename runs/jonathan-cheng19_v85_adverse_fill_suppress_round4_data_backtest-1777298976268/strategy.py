"""Round 4 v85 — v62 + adverse-fill suppression on VFE/HP MM quotes.

Reverse-engineered a competing team's VFE-only PnL chart that climbs
smoothly from 0 → 10,500 across the submission window vs v62's
8,345 VFE PnL. Analysis of v62's IMC submission trade tape revealed
the gap is adverse selection:

  counterparty   filled-on-us    alpha
  Mark 14            159         +2.3 (informed bullish on buys)
  Mark 01             85         +2.4
  Mark 67             20         +1.5
  ...                            (these all LIFT our asks then price rises)
  Mark 55             81         -2.0 (anti-informed; lifting our ask is GOOD)
  Mark 22              6         mixed

Mark 14 alone filled 159 VFE quotes. At ~$10 mean adverse drift over
10 ticks per fill, that's ~$1,590 in adverse PnL we voluntarily eat
because we keep an ask out while informed flow is consuming it.

v85 adds a per-symbol Mark-aware quote-skip:
  - Track each tick's Mark trades.
  - If an informed-bullish Mark (positive mark_weight on the buy side)
    just bought N+ units within last K ticks → skip our ASK quote.
  - Mirror for ask-side toxicity / fade marks.
...
VFE_CFG = {
    ...
    "base_cap_pct": 0.50, "full_cap_pct": 1.00, "hard_cap_pct": 0.95,
    "mark_weights": {
        # v82 corrections: Mark 01 added (most informed, +2.4 alpha),
        # Mark 14 sign flipped (-0.5 → +1.5), Mark 55 sign flipped (+0.5 → -1.5).
        "Mark 01": +1.5, "Mark 14": +1.5, "Mark 67": +1.5,  # follow (informed bullish on buys)
        "Mark 22": -1.0, "Mark 49": -1.5, "Mark 55": -1.5,  # fade (anti-informed)
    },
}
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # v85: adverse-fill suppression. If an informed-bullish Mark (positive
    # mark_weight) just bought significantly within the last K ticks, our
    # ask is about to be lifted by them and we'd be selling at a price that
    # immediately drifts higher — skip the ask. Mirror for the bid side
    # against informed-bearish (negative mark_weight) Marks who just sold.
    P = cfg["prefix"]
    weights = cfg["mark_weights"]
    # Per-tick contribution: signed quantity × weight.
    tick_buy_pressure = 0.0   # > 0 means informed buying (skip ask)
    tick_sell_pressure = 0.0  # > 0 means informed selling (skip bid)
    for t in (market_trades or [])[-3:]:  # current tick's most recent trades
        q = int(t.quantity)
        wb = weights.get(t.buyer or "", 0.0)
        ws = weights.get(t.seller or "", 0.0)
        if wb > 0:  # an informed-bullish Mark BOUGHT — toxic for our ask
            tick_buy_pressure += wb * q
        if ws < 0:  # an informed-bearish Mark SOLD — toxic for our bid
            tick_sell_pressure += -ws * q
    # Maintain a 5-tick rolling decay so a single toxic trade suppresses
    # the corresponding side for ~5 ticks.
    bp = td.get(f"_{P}_bp_decay", 0.0) * 0.6 + tick_buy_pressure
    sp = td.get(f"_{P}_sp_decay", 0.0) * 0.6 + tick_sell_pressure
    td[f"_{P}_bp_decay"] = bp
    td[f"_{P}_sp_decay"] = sp
    suppress_ask = bp > 4.0  # threshold: ~one significant Mark trade
    suppress_bid = sp > 4.0
    ...
    if bid_px < ask_px:
        if bq > 0 and not suppress_bid: out.append(Order(sym, int(bid_px), bq))
        if sq > 0 and not suppress_ask: out.append(Order(sym, int(ask_px), -sq))
