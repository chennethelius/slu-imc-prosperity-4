"""Round 4 v88 — v62 + Black-Scholes IV arbitrage on VEV voucher chain.

Each VEV strike is a call option on VFE. Implied volatility is what the
market is pricing into each contract. If IVs are arranged consistently
across moneyness (clean smile), the market agrees on uncertainty. If a
strike's IV deviates from the smile, the market is mispricing that
voucher relative to its neighbors:

  IV above the smile → over-priced vol → SELL that voucher (delta-hedged)
  IV below the smile → under-priced vol → BUY that voucher (delta-hedged)

v88 runs alongside v62's existing pipelines but adds a vol-arb leg:

  1. Compute BS IV per voucher each tick from observed mid and VFE mid.
  2. Fit a quadratic smile in log-moneyness: IV ≈ a + b*m + c*m².
  3. Compute residual = IV_market - IV_fit per strike.
  4. Trade strikes whose |residual| > threshold, with size proportional
     to |residual| / threshold (proportional conviction).
  5. Delta-hedge the voucher position via VFE so the bet is on vol, not
     direction.

Constants:
  T (time-to-expiry, normalized): 1.0 — only relative IVs matter.
  r (risk-free rate): 0.
  Position cap from vol-arb: 80 per voucher (~25% of voucher limit; v62
    chain extension still uses the rest).

Original v62 docstring follows:
...
# ============================================================================
# v88: Black-Scholes IV arbitrage on VEV voucher chain
# ============================================================================

import math as _m

# Voucher chain for IV arb. Same strikes as v62 chain extension.
IV_VOUCHERS = [
    ("VEV_4000", 4000), ("VEV_4500", 4500), ("VEV_5000", 5000),
    ("VEV_5100", 5100), ("VEV_5200", 5200), ("VEV_5300", 5300),
    ("VEV_5400", 5400), ("VEV_5500", 5500),
]
IV_T = 1.0          # normalized time-to-expiry (only relative IVs matter)
IV_RES_THRESH = 0.0008   # |IV residual| above this triggers — empirically
                          # persistent mispricings are ~0.0015, smile fit precision is ~0.0003
IV_FULL_SIZE = 100   # max position per voucher from IV arb (33% of voucher limit)
IV_LOT = 12          # per-tick fill cap
IV_RES_SMOOTH = 0.20 # EWMA smoothing for residual; only fire on persistent signal
...
def _bs_call(s, k, t, sigma):
    if sigma <= 0 or t <= 0:
        return max(0.0, s - k)
    sqrt_t = _m.sqrt(t)
    d1 = (_m.log(s / k) + 0.5 * sigma * sigma * t) / (sigma * sqrt_t)
    d2 = d1 - sigma * sqrt_t
    return s * _norm_cdf(d1) - k * _norm_cdf(d2)


def _bs_iv(s, k, t, market_price):
    """Bisection IV solver. Returns sigma such that BS price = market_price."""
    intrinsic = max(0.0, s - k)
    if market_price <= intrinsic + 1e-6:
        return None  # at/below intrinsic, IV undefined
    lo, hi = 0.001, 5.0
    for _ in range(40):
        mid = 0.5 * (lo + hi)
        if _bs_call(s, k, t, mid) > market_price:
            hi = mid
        else:
            lo = mid
    return 0.5 * (lo + hi)
...
def _bs_delta(s, k, t, sigma):
    if sigma <= 0 or t <= 0:
        return 1.0 if s > k else 0.0
    d1 = (_m.log(s / k) + 0.5 * sigma * sigma * t) / (sigma * _m.sqrt(t))
    return _norm_cdf(d1)


def _fit_smile(points):
    """Least-squares fit IV = a + b*m + c*m² for points = [(moneyness, iv), ...]."""
    n = len(points)
    if n < 3:
        return None
    ...
    sy = sxy = sx2y = 0.0
    for m, iv in points:
        sx += m; sx2 += m*m; sx3 += m*m*m; sx4 += m*m*m*m
        sy += iv; sxy += m*iv; sx2y += m*m*iv
        ...
        for k in range(3):
            if k != i:
                ...
                for j in range(i, 3):
                    M[k][j] -= f * M[i][j]
                b_vec[k] -= f * b_vec[i]
    return tuple(b_vec)  # (a, b, c)


def _iv_arb_orders(state, td, all_orders):
    """Compute IV per voucher, fit smile, trade outliers, hedge delta via VFE."""
    vfe_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
    if not vfe_depth or not vfe_depth.buy_orders or not vfe_depth.sell_orders:
        return {}
    ...
    # 1. Compute IV per voucher from market mid
    points = []  # [(moneyness, iv, sym, mid, depth)]
    for sym, strike in IV_VOUCHERS:
        ...
        if mid <= 0:
            continue
        iv = _bs_iv(vfe_mid, strike, IV_T, mid)
        if iv is None:
            continue
        moneyness = _m.log(vfe_mid / strike)
        points.append((moneyness, iv, sym, strike, depth, bb, ba, mid))

    if len(points) < 4:
        return {}

    # 2. Fit smile
    coefs = _fit_smile([(m, iv) for m, iv, *_ in points])
    if coefs is None:
        return {}
    a, b, c = coefs

    # 3. Compute residuals (smoothed) & trade outliers
    out_orders = {}
    total_delta_added = 0.0
    iv_pos = td.setdefault("_iv_pos", {})
    iv_res_ewma = td.setdefault("_iv_res_ewma", {})

    annotated = []
    for m, iv, sym, strike, depth, bb, ba, mid in points:
        iv_fit = a + b*m + c*m*m
        raw_res = iv - iv_fit
        # EWMA smooth — only persistent mispricings count
        prev = iv_res_ewma.get(sym, raw_res)
        smoothed = (1 - IV_RES_SMOOTH) * prev + IV_RES_SMOOTH * raw_res
        iv_res_ewma[sym] = smoothed
        annotated.append((smoothed, sym, strike, depth, bb, ba, mid, iv))
    annotated.sort(key=lambda x: -abs(x[0]))

    for residual, sym, strike, depth, bb, ba, mid, iv in annotated:
        if abs(residual) < IV_RES_THRESH:
            continue
        # Proportional sizing: residual at threshold → IV_LOT,
        # residual at 3× threshold → IV_FULL_SIZE.
        ratio = abs(residual) / IV_RES_THRESH
        scale = min(1.0, (ratio - 1.0) / 2.0)
        target_size = max(IV_LOT, int(round(IV_FULL_SIZE * scale)))
        cur_pos = state.position.get(sym, 0)
        # Don't fight v62's chain extension orders already in `all_orders`
        existing = all_orders.get(sym, [])
        bv_e = sum(o.quantity for o in existing if o.quantity > 0)
        sv_e = sum(-o.quantity for o in existing if o.quantity < 0)

        if residual < 0:  # underpriced vol → BUY
            # IV-arb position cap on the long side: +IV_FULL_SIZE
            arb_pos = iv_pos.get(sym, 0)
            if arb_pos >= IV_FULL_SIZE:
                continue
            room = min(IV_LOT, IV_FULL_SIZE - arb_pos, 300 - cur_pos - bv_e)
            qty = min(room, abs(depth.sell_orders[ba]))
            if qty <= 0:
                continue
            ors = list(existing) + [Order(sym, ba, qty)]
            out_orders[sym] = ors
            iv_pos[sym] = arb_pos + qty
            delta = _bs_delta(state.order_depths["VELVETFRUIT_EXTRACT"] and
                              ((max(state.order_depths["VELVETFRUIT_EXTRACT"].buy_orders) +
                                min(state.order_depths["VELVETFRUIT_EXTRACT"].sell_orders))/2),
                              strike, IV_T, max(0.05, iv))
            total_delta_added += delta * qty
        else:  # overpriced vol → SELL
            arb_pos = iv_pos.get(sym, 0)
            if arb_pos <= -IV_FULL_SIZE:
                continue
            room = min(IV_LOT, arb_pos + IV_FULL_SIZE, 300 + cur_pos - sv_e)
            qty = min(room, depth.buy_orders[bb])
            if qty <= 0:
                continue
            ors = list(existing) + [Order(sym, bb, -qty)]
            out_orders[sym] = ors
            iv_pos[sym] = arb_pos - qty
            delta = _bs_delta(vfe_mid, strike, IV_T, max(0.05, iv))
            total_delta_added -= delta * qty

    # 4. Delta-hedge via VFE (offset the call delta we just added)
    # DISABLED: v88's hedge fights v62's VFE conviction take (which already
    # responds to mid-fair deviation). Keep IV arb leg pure — accept some
    # delta exposure since vouchers we trade tend to be near-money so
    # delta is moderate and v62's flatten logic will rebalance later.
    if False and abs(total_delta_added) >= 5:
        hedge_qty = -int(round(total_delta_added))  # opposite direction
        vfe_pos = state.position.get("VELVETFRUIT_EXTRACT", 0)
        existing_vfe = all_orders.get("VELVETFRUIT_EXTRACT", [])
        bv_e = sum(o.quantity for o in existing_vfe if o.quantity > 0)
        sv_e = sum(-o.quantity for o in existing_vfe if o.quantity < 0)
        if hedge_qty > 0 and vfe_pos + bv_e < 200:
            qty = min(hedge_qty, 200 - vfe_pos - bv_e, vfe_depth.sell_orders[vfe_ba])
            if qty > 0:
                out_orders["VELVETFRUIT_EXTRACT"] = list(existing_vfe) + [Order("VELVETFRUIT_EXTRACT", vfe_ba, qty)]
        elif hedge_qty < 0 and vfe_pos - sv_e > -200:
            qty = min(-hedge_qty, 200 + vfe_pos - sv_e, vfe_depth.buy_orders[vfe_bb])
            if qty > 0:
                out_orders["VELVETFRUIT_EXTRACT"] = list(existing_vfe) + [Order("VELVETFRUIT_EXTRACT", vfe_bb, -qty)]
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # v88: IV arbitrage layer on top of v62 pipelines
        iv_orders = _iv_arb_orders(state, td, orders)
        for sym, ors in iv_orders.items():
            orders[sym] = ors
