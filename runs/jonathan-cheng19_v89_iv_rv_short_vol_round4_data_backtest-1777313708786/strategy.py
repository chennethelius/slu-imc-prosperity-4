"""Round 4 v89 — v62 + short-vol via IV>RV comparison on every voucher.

Data analysis (R4 D1/D2/D3, multiple 1500-tick samples):
  ATM IV (5000-5500):  ~0.030
  OTM IV (6000):       ~0.051
  Deep OTM (6500):     ~0.078
  Realized vol (200t): ~0.007 (1000 ticks scaled)

  IV - RV is POSITIVE on every voucher, every day, every sampled tick.
  The vol surface is systematically overpriced relative to realized vol
  by 3-10x. This is the cleanest persistent edge in the chain.

v89 grafts an IV-RV short-vol pipeline onto v62:

  1. Rolling 200-tick RV from VFE log-returns, scaled to BS-T units.
  2. Per voucher per tick: BS IV from market mid.
  3. When IV - RV > threshold: short voucher (sell vol).
     When IV - RV < -threshold: long voucher (buy vol; rare).
  4. Position size proportional to |IV-RV| / threshold (Spock's
     proportional conviction principle).
  5. Aggregate voucher delta hedged via VFE so the bet is on vol,
     not direction.
  6. Caps:
       per-voucher arb cap: ±80 (v62's chain extension still uses room)
       VFE hedge cap:        ±100 (v62 conviction still uses room)

The vol-arb leg pays:
   + Theta decay (positive every tick we hold)
   + Vega convergence if IV drops toward RV
   - Gamma cost if VFE moves more than implied
Since RV is consistently 4x BELOW IV, expected gamma cost is bounded
well below theta+vega gain.
...
# ============================================================================
# v89: IV vs RV short-vol pipeline
# ============================================================================
...
VOLARB_T = 1.0
VOLARB_RV_LOOKBACK = 200       # ticks used for realized-vol estimate
VOLARB_RV_SCALE = 1000.0       # BS-T window length (matches IV's T units)
VOLARB_THRESH = 0.010          # |IV - RV| > 0.010 triggers (10 vol points)
VOLARB_FULL_SIZE = 80          # max IV-arb position per voucher
VOLARB_LOT = 8                 # per-tick fill cap
VOLARB_VFE_HEDGE_CAP = 100     # max VFE position from delta hedging
...
def _bs_call_v89(s, k, t, sigma):
    if sigma <= 0 or t <= 0:
        return max(0.0, s - k)
    sqrt_t = _m.sqrt(t)
    d1 = (_m.log(s/k) + 0.5*sigma*sigma*t) / (sigma*sqrt_t)
    d2 = d1 - sigma*sqrt_t
    return s * _norm_cdf_v89(d1) - k * _norm_cdf_v89(d2)


def _bs_iv_v89(s, k, t, market_price):
    ...
    for _ in range(35):
        mid = 0.5 * (lo + hi)
        if _bs_call_v89(s, k, t, mid) > market_price:
            hi = mid
        else:
            lo = mid
    return 0.5 * (lo + hi)
...
def _bs_delta_v89(s, k, t, sigma):
    if sigma <= 0 or t <= 0:
        return 1.0 if s > k else 0.0
    d1 = (_m.log(s/k) + 0.5*sigma*sigma*t) / (sigma*_m.sqrt(t))
    return _norm_cdf_v89(d1)


def _walk_book_v89(depth, side, sym, qty_target):
    """Walk asks (+1) or bids (-1) up to qty_target. Returns (orders, filled)."""
    if side > 0:
...
def _volarb_orders(state, td, all_orders):
    """Per voucher, take short-vol position when IV > RV + threshold.
    Aggregate delta of vol-arb position is hedged via VFE.
    """
    vfe_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
    if not vfe_depth or not vfe_depth.buy_orders or not vfe_depth.sell_orders:
        return {}
    ...
    # Update VFE mid history for realized vol
    hist = td.setdefault("_volarb_mids", [])
    hist.append(vfe_mid)
    if len(hist) > VOLARB_RV_LOOKBACK + 1:
        del hist[0]
    if len(hist) < 30:
        return {}  # need warmup

    # Compute realized vol from log returns, scaled to BS-T units
    rets = []
    for i in range(1, len(hist)):
        if hist[i-1] > 0 and hist[i] > 0:
            rets.append(_m.log(hist[i] / hist[i-1]))
    if len(rets) < 10:
        return {}
    mu = sum(rets) / len(rets)
    var = sum((r - mu) ** 2 for r in rets) / len(rets)
    rv = _m.sqrt(max(0, var)) * _m.sqrt(VOLARB_RV_SCALE)

    out_orders = {}
    iv_pos = td.setdefault("_volarb_pos", {})
    total_delta = 0.0  # NEW delta added this tick from vol-arb fills

    for sym, strike in VOLARB_VOUCHERS:
        ...
        if mid <= 0:
            continue
        iv = _bs_iv_v89(vfe_mid, strike, VOLARB_T, mid)
        if iv is None:
            continue
        gap = iv - rv
        if abs(gap) < VOLARB_THRESH:
            continue

        # Proportional sizing: at threshold, take VOLARB_LOT;
        # at 5x threshold, take VOLARB_FULL_SIZE.
        ratio = abs(gap) / VOLARB_THRESH
        scale = min(1.0, (ratio - 1.0) / 4.0)
        target_size = max(VOLARB_LOT, int(round(VOLARB_FULL_SIZE * scale)))
        ...
        delta = _bs_delta_v89(vfe_mid, strike, VOLARB_T, max(0.05, iv))

        if gap > 0:  # IV > RV → SELL VOL
            if arb_pos <= -VOLARB_FULL_SIZE:
                continue
            room = min(VOLARB_LOT, target_size, arb_pos + VOLARB_FULL_SIZE,
                       300 + cur_pos - sv_e)
        ...
        else:  # IV < RV → BUY VOL (rare)
            if arb_pos >= VOLARB_FULL_SIZE:
                continue
            room = min(VOLARB_LOT, target_size, VOLARB_FULL_SIZE - arb_pos,
                       300 - cur_pos - bv_e)
    ...
    # Delta-hedge accumulated voucher delta via VFE (cap at VFE_HEDGE_CAP).
    # Total_delta > 0 means we added long delta → short VFE to hedge.
    if abs(total_delta) >= 3:
        hedge_qty = -int(round(total_delta))
        vfe_hedge_pos = td.get("_volarb_vfe_hedge", 0)
        # Cap the VFE hedge inventory
        if hedge_qty > 0 and vfe_hedge_pos >= VOLARB_VFE_HEDGE_CAP:
            hedge_qty = 0
        elif hedge_qty < 0 and vfe_hedge_pos <= -VOLARB_VFE_HEDGE_CAP:
            hedge_qty = 0
        else:
            hedge_qty = max(min(hedge_qty, VOLARB_VFE_HEDGE_CAP - vfe_hedge_pos),
                            -VOLARB_VFE_HEDGE_CAP - vfe_hedge_pos)
        ...
        if hedge_qty != 0:
            ...
            if hedge_qty > 0:
                qty = min(hedge_qty, 200 - vfe_pos - bv_e,
                          abs(vfe_depth.sell_orders[vfe_ba]))
                if qty > 0:
                    out_orders["VELVETFRUIT_EXTRACT"] = list(existing_vfe) + [
                        Order("VELVETFRUIT_EXTRACT", vfe_ba, qty)]
                    td["_volarb_vfe_hedge"] = vfe_hedge_pos + qty
            else:
                qty = min(-hedge_qty, 200 + vfe_pos - sv_e,
                          vfe_depth.buy_orders[vfe_bb])
                if qty > 0:
                    out_orders["VELVETFRUIT_EXTRACT"] = list(existing_vfe) + [
                        Order("VELVETFRUIT_EXTRACT", vfe_bb, -qty)]
                    td["_volarb_vfe_hedge"] = vfe_hedge_pos - qty
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # v89: IV-vs-RV short-vol leg, layered on top of v62
        volarb_out = _volarb_orders(state, td, orders)
        for s, ors in volarb_out.items():
            orders[s] = ors
