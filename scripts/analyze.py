...
from _loaders import load_json, parse_activity_from_submission_log, get_pnl, get_mid
...
def analyze_activity_log(rows: list[dict]) -> dict:
    ...
    def _product_stats(entries: list[dict]) -> dict:
    ...
    result = {p: _product_stats(e) for p, e in sorted(products.items())}
