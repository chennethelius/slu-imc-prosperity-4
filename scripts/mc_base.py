"""
Monte Carlo parameter stability test — shared core.
...
Round-specific wrappers (mc_round1.py, mc_round2.py) import run_mc() with
dataset + days config.
"""
import os
import random
...
def run_backtest(strategy_path: str, dataset: str, days: tuple[str, ...]) -> tuple[float, float, float, float]:
    """Returns (pep_pnl, osm_pnl, total, avg_per_day) aggregated over the given days."""
    pep_sum = osm_sum = 0.0
    for day in days:
        r = subprocess.run(
            [BT, "--trader", strategy_path, "--dataset", dataset, f"--day={day}",
    ...
    n = len(days) or 1
    return pep_sum, osm_sum, pep_sum + osm_sum, (pep_sum + osm_sum) / n


def mc_test(strategy_name: str, strat_dir: str, dataset: str, days: tuple[str, ...]):
    ...
    try:
        ...
        with open(path, "w") as f:
            f.write(original)
        b_pep, b_osm, b_tot, b_avg = run_backtest(path, dataset, days)
        ...
        for i in range(N_SIMS):
            ...
            with open(path, "w") as f:
                f.write(patched)
            try:
                p, o, t, a = run_backtest(path, dataset, days)
                results.append(t)
...
def run_mc(dataset: str, days: tuple[str, ...], default_strategies: list[str], argv: list[str]):
    """Entry point for round-specific wrappers. argv is sys.argv[1:]."""
    strat_dir = os.path.join(REPO, "strategies", dataset)
    strategies = argv or default_strategies
    for s in strategies:
        mc_test(s, strat_dir, dataset, days)
