"""
MC overfit check for the combined (per-asset z, t, prior) variant.

Compares three strategies under identical Gaussian price perturbation:
  - baseline   (z_take.py: uniform z=1.0, t=17, static mean)
  - per_asset  (z_take_per_asset.py: per-asset z, t; static mean)
  - combined   (z_take_combined.py: per-asset z, t; per-asset prior)

Same seed across strategies and noise levels so deltas are directly
comparable. Combined stacks two layers of in-sample tuning, so the gain
should hold up under perturbation OR shrink monotonically as noise grows;
a flip-negative would mean overfit.

Usage:  python scripts/mc_z_take_combined_overfit.py [n_seeds]
"""
import csv
import random
import shutil
...
MC_R3 = BT_DIR / "datasets" / "round3_mc_combined"
MC_R4 = BT_DIR / "datasets" / "round4_mc_combined"

STRATS = {
    "baseline":  REPO / "strategies" / "round4" / "z_take.py",
    "per_asset": REPO / "strategies" / "round4" / "tmp" / "z_take_per_asset.py",
    "combined":  REPO / "strategies" / "round4" / "tmp" / "z_take_combined.py",
}
        ...
        for noise in NOISE_SDS:
            ...
            for seed in range(n_seeds):
                ...
                print(f"  seed {seed}:  base={fmt(bt[3])}  "
                      f"per_asset={fmt(pa[3])} (Δ={pa[3]-bt[3]:>+9,.0f})  "
                      f"combined={fmt(cb[3])} (Δ={cb[3]-bt[3]:>+9,.0f})",
                      flush=True)
            print()
    finally:
