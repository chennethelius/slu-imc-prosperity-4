...
class Trader:
    """v23: anti-overfit osmium — STATIC=10001 anchor on TAKE phase only.

    v22 let Kalman-dynamic fair drift into TAKE-phase limits, which
    overfit training (d0/d1) and whipsawed OOS (d-1 = 3171). Anchoring
    TAKE to OSM_FAIR_STATIC=10001 (slu-style) while keeping dynamic fair
    for CLEAR/MAKE captures long-run mean-reversion without drift risk.
    SU=20 (was 12) widens the skew unit so aggressive takes only trigger
    at larger imbalances.

    Cumulative round2 3-day PnL (QP=0.0, local backtester):
      v22 control:           248147  (dm1=3171 d0=3517 d1=3542)
      v23 (this):            248353  (dm1=3583 d0=3129 d1=3724)  +206
      dyn_take SU=20:        248275  (dm1=3497 d0=3152 d1=3709)  +128
      static_take SU=12:     248125  (dm1=3224 d0=3343 d1=3641)

    d-1 OOS: +412 vs v22. d1: +182. d0 gives up 388 but the OOS
    improvement + d1 gain dominates. Mechanism matches Kalman-overfit
    hypothesis: static TAKE reference prevents drift-induced late takes.
    """
    ...
    def _osmium(self, d, pos, td):
        ...
        ask_limit = self.OSM_FAIR_STATIC - max(0, self.OSM_TAKE_WIDTH + skew)
        bid_limit = self.OSM_FAIR_STATIC + max(0, self.OSM_TAKE_WIDTH - skew)
