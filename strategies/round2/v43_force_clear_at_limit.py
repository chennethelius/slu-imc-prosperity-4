...
class Trader:
    """v43: force CLEAR only at the hard position limit.

    v41 forced CLEAR when |pos| > OSM_VOLUME_LIMIT (30), lost 101 —
    unwinding favorable inventory at moderate positions sacrificed too
    much drift MtM. The trigger was too eager.

    v43 narrows the trigger to |pos| >= LIMIT (= 80, the hard cap):
        force_clear = abs(pos) >= lim

    At the hard cap we physically cannot accumulate more — TAKE can't
    add, MAKE bid can't fill (rejected by exchange). If we're stuck
    at ±80 in a favorable regime, v27 skips CLEAR indefinitely until
    regime flips. That maximizes drift exposure but removes the only
    active unwind path, leaving us potentially stuck with peak position
    if the flip never comes mid-session.

    v43 rule: at pos = ±80, activate CLEAR regardless of regime. Below
    the cap, full v27 behavior (hold through favorable). This is a
    strict safety valve — fires only when there is zero upside from
    holding more.

    Unlike v41 (trigger at 30), v43 lets the position grow fully into
    the drift — only when stalled at the wall do we start unwinding.

    Zero new parameters. Pepper, TAKE, MAKE, regime detection all v27.
    """
