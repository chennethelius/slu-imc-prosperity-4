"""Round 3 v31 — v30 + HP NEVER pays spread (pure passive MM).

User constraint: HP should never cross the spread. Disable the Kalman take
on HP entirely (take_max_pay = -999 → no ask crosses, no bid crosses).
Position adjusts ONLY via passive penny-jump fills. Slower convergence to
the Kalman target, but every fill happens AT the spread (we collect spread
instead of paying it).

VFE keeps its mild take_max_pay = -2 since VFE has a real mean-reversion
edge that benefits from cheap-cross fills.

Round 3 v30 — v27 + always-on tight penny-jumping for HP/VFE.

The v27 quote_edge gate (asks >= fair+3, bids <= fair-3) skipped MM on ticks
where the book sat inside that band. v30 always posts at best_bid+1 /
best_ask-1 whenever there's remaining capacity on that side. Post on BOTH
sides every tick (when not crossed and not at the cap on that side).

Same Kalman target chase, same VFE informed-flow signal, same wider VEV
divergence thresholds for day-2 robustness — only the MM quoting layer is
made strictly aggressive.
...
def kalman_mr_orders(cfg, depth, position, scratch, target_bias=0):
    ...
    # quote_size so we never use full inventory in a single quote.
    best_bid_now = max(depth.buy_orders)
    best_ask_now = min(depth.sell_orders)
    bid_px = best_bid_now + 1
    ask_px = best_ask_now - 1
    # If the book is single-tick wide, skip the side that would cross.
    if bid_px < ask_px:
        buy_q = min(quote_size, limit - position - bv)
        sell_q = min(quote_size, limit + position - sv)
        if buy_q > 0:
            orders.append(Order(product, bid_px, buy_q))
        if sell_q > 0:
            orders.append(Order(product, ask_px, -sell_q))
    else:
        # Crossed/touch market: post one tick away from the touch on each side.
        buy_q = min(quote_size, limit - position - bv)
        sell_q = min(quote_size, limit + position - sv)
        if buy_q > 0:
            orders.append(Order(product, best_bid_now, buy_q))
        if sell_q > 0:
            orders.append(Order(product, best_ask_now, -sell_q))
...
KALMAN_MR_PRODUCTS = [
    {
        ...
        "mr_gain": 1000,
        "sigma_init": 30.0,
        # v31: HP never pays spread — set take_max_pay to a value that
