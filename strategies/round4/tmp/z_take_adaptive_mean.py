"""
Round 4 — z_take_adaptive_mean: hybrid prior/empirical mean variant.

Same z-take logic as z_take.py, but the mean used in the z-score is no
longer fixed. Each tick we update a running (count, sum) of observed mids
per product (persisted in traderData), and use a Bayesian-style blend:

    effective_mean = (PRIOR_WEIGHT * static_mean + sum_obs) / (PRIOR_WEIGHT + n_obs)

At n=0 this equals the static prior (cold start, no observations yet).
As n grows the empirical mean takes over smoothly. PRIOR_WEIGHT acts as
a pseudo-count: with PRIOR_WEIGHT = 2000, the empirical mean has equal
weight after 2000 ticks (~20% of a day) and dominates after that.

State resets each day (Prosperity restarts traderData per submission day),
which is desirable — it lets the strategy re-anchor to whatever regime
day-N actually opens in, rather than locking onto a stale mid-multi-day mean.

Standard deviation stays static — sd estimation is noisy on short windows
and the static value already covers the cross-day dispersion.
"""
...
# Pseudo-count weight for the static prior. Empirical mean reaches equal
# weight after this many observed ticks. 10000 ticks = 1 day.
PRIOR_WEIGHT = 2000
...
def _z_take_orders(state, cfg, stats):
    ...
    if sd <= 0:
        return []
    eff_mean = (PRIOR_WEIGHT * static_mean + total) / (PRIOR_WEIGHT + n)
    z = (mid - eff_mean) / sd
