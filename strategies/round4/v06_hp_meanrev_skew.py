...
and slightly skew inventory. Avoid overfitting / over-tuning.

Mechanism (two parameters total):
   target = HP_MR_GAIN * (10000 - mid),  clamped to ±HP_SOFT_LIMIT
   deviation = position - target
   buy_qsize  = BASE * (1 - deviation/HP_LIMIT)
   sell_qsize = BASE * (1 + deviation/HP_LIMIT)

When mid = 9990 (10 below fair):  target = +20  → bias bigger buys
When mid = 10010 (10 above fair): target = -20  → bias bigger sells
When mid = 10000 (at fair):       target = 0    → symmetric quotes

The MR effect is BAKED INTO QUOTE SIZES, never into aggressive crosses.
We only collect spread on fills — the slight long/short bias means our
fills are weighted in the favorable direction without ever paying the
spread to chase. Inventory naturally drifts toward the MR target as fills
happen.

HP_MR_GAIN = 2 (kept low — at mid=9990, target=+20 lots = 10% of limit)
HP_SOFT_LIMIT = 60 (30% of HP_LIMIT — never goes near rail from MR alone)

V05's pure-passive HP scored 75k / Sharpe 7.10 over 3 days. v06 should
modestly raise PnL on top by leaning into the reversion.

== ORIGINAL v04 NOTES BELOW ==
...
# v06: mean-reversion target tilts position toward fair=10000.
# Two new params; both kept small to avoid overfit / drift exposure.
...
def hp_orders(state, store):
    ...
    # 5. v06: passive quoting with mean-reversion target + inventory skew.
    #    Combines two effects:
    #      (a) MR target = HP_MR_GAIN * (HP_FAIR - mid)
    #          When mid is below 10000, we want to be LONG (expect bounce up).
    #          When mid is above 10000, we want to be SHORT.
    #          Capped at ±HP_SOFT_LIMIT (30% of position cap).
    #      (b) Quote sizes pulled toward (target - position):
    #          deviation = position - target  (negative = need to buy more)
    #          buy_qsize  = BASE * (1 - deviation/HP_LIMIT)
    #          sell_qsize = BASE * (1 + deviation/HP_LIMIT)
    ...
    if bid_px < ask_px:
        ...
        deviation = pos_after_take - target  # >0 = too long vs target, <0 = too short
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # v05: VEV options DISABLED for cleanest flat-up PnL curve.
        # VEV divergence trader catches falling knives during VFE regime
        # change (round-4 day-3) — drops day-3 PnL from 170k to 49k.
        # Without VEV, we keep the consistent HP+VFE spread capture only.
