"""
Round 4 v07 — explicit four-algo stack.

Each canonical algo class gets one layer, mapped from round-4 data:

  MARKET-MAKING       Kalman-MR on HYDROGEL & VFRUIT.
                      Round-4 data: HYDROGEL spread modal=16 (92%);
                      VFRUIT modal=5 (74%) — wide structural edge.

  ARBITRAGE           Delta-1 + TV-arb on VEV_4000/4500.
                      Round-4 data: TV ≈ 0 (sd 0.8, max ±7) ⇒ voucher
                      mid is just max(0, S - K); when it deviates, that
                      gap is a near-riskless convergence trade.

  STATISTICAL ARB     Smile-fair residual on VEV_5000-5500.
                      Round-4 data: at TTE=4 the active smile fits with
                      residual SD 0.039 (real fit, not noise) and per-
                      strike biases alternate ±$1-3 — clean per-strike
                      MR signal around the model anchor.

  TREND-FOLLOWING     VFRUIT EMA crossover dampens (does NOT reverse)
                      Kalman-MR when spot is in a confirmed directional
                      move. Currently MR fights every drift; trend gate
                      cuts target by trend_dampen on the side that
                      would resist the trend, preserving MR on the
                      trend-with side.
                      Round-4 data: spot ranged 5191-5300 (109 ticks),
                      with multi-tick directional regimes between
                      reversals — fighting these full-strength bleeds
                      D2 and D3 PnL.
"""
...
def kalman_mr_orders(cfg, depth, position, scratch):
    ...
    # ----- TREND-FOLLOWING LAYER -----
    # Fast/slow EMA on mid; when fast - slow is far from zero AND aligned
    # with the side raw_target wants to oppose, dampen the target so we
    # don't fight a confirmed move.
    trend_fast_alpha = cfg.get("trend_fast_alpha", 0.05)    # ~20-tick HL
    trend_slow_alpha = cfg.get("trend_slow_alpha", 0.005)   # ~140-tick HL
    trend_threshold  = cfg.get("trend_threshold", 5.0)      # only fire on big drifts
    trend_dampen     = cfg.get("trend_dampen", 0.7)         # 0 = full kill, 1 = no effect

    fast = scratch.get("_t_f", mid)
    slow = scratch.get("_t_s", mid)
    fast += trend_fast_alpha * (mid - fast)
    slow += trend_slow_alpha * (mid - slow)
    scratch["_t_f"], scratch["_t_s"] = fast, slow
    trend = fast - slow

    # raw_target > 0 wants to BUY (price below anchor). If trend < -threshold
    # (downtrend), reducing buy size is sensible; we'd be catching a falling
    # knife. Likewise dampen sells when trend is up.
    if raw_target > 0 and trend < -trend_threshold:
        raw_target *= trend_dampen
    elif raw_target < 0 and trend > trend_threshold:
        raw_target *= trend_dampen

    target = max(-limit, min(limit, round(raw_target)))
