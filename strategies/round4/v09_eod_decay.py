"""
Round 4 v09 — v06 + end-of-day position decay (the only winning piece
from v08's four-change bundle).

After tick 8000 of any within-day clock, max_diverge_position and
max_arb_position scale linearly down to 30% of nominal by tick 9999.
On D1/D2 the next day rebuilds positions from scratch, so it costs
little; on D3 (final day) it actively unwinds before the close,
defending min-day floor.

v08 evidence that other 'improvable asset' fixes failed:
  - Penny MM on VEV_6000/6500: 0 fills (likely queue-priority issue).
  - 5400/5500 threshold raise: killed PnL on those strikes.
  - VFRUIT spread-only (mr_gain=0): cost $40k of real MR signal.
"""
...
def end_of_day_decay(timestamp, decay_start_tick=8000, floor=0.30):
    """Returns multiplier in [floor, 1.0]. After `decay_start_tick`
    within-day, scales linearly down to `floor` by tick 9999."""
    tick = (timestamp % 1_000_000) // 100
    if tick <= decay_start_tick:
        return 1.0
    progress = (tick - decay_start_tick) / (9999 - decay_start_tick)
