"""Round 4 v47 — v42 + active profit-take on VEV chain inventory.

Microstructure analysis on v42 R4-d3 fills found:
  VEV_4000 BUY: 71 fills, fwd_ret 5k +6.66/lot — gold, but max position
                only reached 87 (29% of 300 limit).
  VEV_4500 BUY: 63 fills, fwd_ret 5k +6.02/lot — gold, max position 169.
The most profitable strikes are underutilized because positions don't
exit fast enough to free capacity for the next signal.

v47 adds active profit-take on VEV inventory: when unrealized profit
per lot exceeds threshold, ACTIVELY cross to close (instead of waiting
for passive fill at intrinsic ± 1).

   pos > 0 and (current_intrinsic - avg_cost) > PROFIT_TICKS:
       cross down to v_bb (sell at best bid)
   pos < 0 and (avg_cost - current_intrinsic) > PROFIT_TICKS:
       cross up to v_ba (buy at best ask)

Track per-voucher avg_cost in trader_data, updated on each fill.
Frees capacity faster → more signal cycles → higher PnL.
...
VEV_DELTA_VOUCHERS = [
    {"symbol": "VEV_4000", "strike": 4000, "limit": 300, "soft_cap": 250, "offset": 2},
    {"symbol": "VEV_4500", "strike": 4500, "limit": 300, "soft_cap": 250, "offset": 2},
    {"symbol": "VEV_5000", "strike": 5000, "limit": 300, "soft_cap": 200, "offset": 8},
    {"symbol": "VEV_5100", "strike": 5100, "limit": 300, "soft_cap": 150, "offset": 18},
    {"symbol": "VEV_5200", "strike": 5200, "limit": 300, "soft_cap": 100, "offset": 38},
]
...
PROFIT_TAKE_TICKS = 4   # cross out when unrealized profit > 4 ticks/lot


def _update_avg_cost(td, sym, prev_pos, new_pos, fill_price, fill_qty_signed):
    """Update running avg cost basis after fill. fill_qty_signed is +qty for buy, -qty for sell."""
    key = f"_{sym}_cost"
    avg = td.get(key, fill_price)
    # If position grew on same side, weighted average
    if (prev_pos >= 0 and fill_qty_signed > 0) or (prev_pos <= 0 and fill_qty_signed < 0):
        # Adding to position
        if new_pos != 0:
            avg = (avg * prev_pos + fill_price * fill_qty_signed) / new_pos if new_pos != 0 else fill_price
    elif (prev_pos > 0 and fill_qty_signed < 0) or (prev_pos < 0 and fill_qty_signed > 0):
        # Reducing/flipping
        if abs(new_pos) <= abs(prev_pos):
            # Reducing same direction; avg unchanged
            pass
        else:
            # Flipped — new avg is fill price
            avg = fill_price
    td[key] = avg


def _vev_flatten_orders(state, td):
    """Profit-take + passive close-out for VEV inventory."""
    out_by_sym = {}
    ...
    # Update avg-cost from own_trades since last tick
    own_trades = state.own_trades
    for sym, ot_list in (own_trades or {}).items():
        for t in ot_list:
            qsign = +int(t.quantity) if t.buyer == "SUBMISSION" else -int(t.quantity)
            cost_key = f"_{sym}_cost"
            old_pos_key = f"_{sym}_oldpos"
            old_pos = td.get(old_pos_key, 0)
            new_pos = old_pos + qsign
            avg = td.get(cost_key, t.price)
            if (old_pos >= 0 and qsign > 0) or (old_pos <= 0 and qsign < 0):
                if new_pos != 0:
                    avg = (avg * old_pos + t.price * qsign) / new_pos
            elif abs(new_pos) > abs(old_pos):
                avg = t.price
            td[cost_key] = avg
            td[old_pos_key] = new_pos

    for v in VEV_DELTA_VOUCHERS:
        v_sym = v["symbol"]; v_strike = v["strike"]; v_limit = v["limit"]
        v_pos = state.position.get(v_sym, 0)
        td[f"_{v_sym}_oldpos"] = v_pos   # sync
        if v_pos == 0: continue
        v_depth = state.order_depths.get(v_sym)
        if not v_depth or not v_depth.buy_orders or not v_depth.sell_orders:
            continue
        ...
        avg_cost = td.get(f"_{v_sym}_cost", intrinsic)
        orders = []

        if v_pos > 0:
            # Profit-take: cross down (sell at v_bb) when unrealized > THRESH
            unrealized = (v_bb - avg_cost) if v_bb else 0
            if unrealized > PROFIT_TAKE_TICKS:
                # Active profit-take
                pt_qty = min(v_pos, 60, v_limit + v_pos)
                if pt_qty > 0:
                    orders.append(Order(v_sym, int(v_bb), -pt_qty))
            else:
                # Default passive flatten at intrinsic+1 (or v_ba-1)
                ask_px = max(intrinsic + 1, v_ba - 1)
                sell_q = min(v_pos, 50, v_limit + v_pos)
                if sell_q > 0:
                    orders.append(Order(v_sym, ask_px, -sell_q))
        elif v_pos < 0:
            unrealized = (avg_cost - v_ba) if v_ba else 0
            if unrealized > PROFIT_TAKE_TICKS:
                pt_qty = min(-v_pos, 60, v_limit - v_pos)
                if pt_qty > 0:
                    orders.append(Order(v_sym, int(v_ba), pt_qty))
            else:
