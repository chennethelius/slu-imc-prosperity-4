"""Round 4 v48 — v42 + chain priority flipped (VEV_4000/4500 first).

Microstructure analysis showed VEV_4000 BUYs earn +6.66/lot fwd 5k ms
and VEV_4500 BUYs +6.02/lot — far better per-lot return than VFE
(+0.88/lot). But these gold strikes only get filled via overflow when
VFE primary saturates, which rarely happens.

v48 flips priority: route conviction take to VEV_4000 first, then
VEV_4500, then VFE, then VEV_5000+. Same total volume per signal but
redistributed to highest-quality strikes.

Risk: VEV_4000/4500 are deep ITM with delta=1, so they're VFE-equivalent
in directional exposure but with potentially worse fill mechanics
(less liquidity, queue priority). Test if redistribution helps net PnL.
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === v48 PRIORITY FLIP: For VFE, route to VEV_4000/4500 FIRST, then VFE ===
    if cfg["prefix"] == "vfe" and conviction > 0:
        vfe_mid_int = int(round(mid))
        for v in VEV_DELTA_VOUCHERS[:2]:   # only 4000 and 4500 first (gold)
            if primary_taken >= primary_target: break
            ...
            v_orders = []
            v_bv = v_sv = 0
            rem = primary_target - primary_taken
    ...
    # Conviction take in primary product (after gold-VEV)
    if conviction > 0 and primary_taken < primary_target:
        rem_target = primary_target - primary_taken
        if direction > 0 and pos_after < soft_cap:
            max_pay = fair + cfg["take_offset"]; rem = rem_target
            for ask in sorted(depth.sell_orders):
                if ask > max_pay or rem <= 0: break
                avail = -depth.sell_orders[ask]
            ...
            min_recv = fair - cfg["take_offset"]; rem = rem_target
            for bid in sorted(depth.buy_orders, reverse=True):
                if bid < min_recv or rem <= 0: break
                avail = depth.buy_orders[bid]
    ...
    # Remaining overflow goes to VEV_5000/5100/5200 (lower-priority strikes)
    if cfg["prefix"] == "vfe" and conviction > 0 and primary_target > primary_taken:
        overflow = primary_target - primary_taken
        vfe_mid_int = int(round(mid))
        for v in VEV_DELTA_VOUCHERS[2:]:   # 5000, 5100, 5200
            if overflow <= 0: break
            v_sym = v["symbol"]
            v_strike = v["strike"]
            v_limit = v["limit"]
