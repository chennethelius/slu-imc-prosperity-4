...
VEV_DELTA_VOUCHERS = [
    # min_conv: required conviction for this voucher to receive overflow.
    # Deep ITM 4000/4500 always available (no premium decay risk).
    # ATM strikes (5000+) require stronger signal to overcome premium drag.
    {"symbol": "VEV_4000", "strike": 4000, "limit": 300, "soft_cap": 250, "offset": 2, "min_conv": 0.0},
    {"symbol": "VEV_4500", "strike": 4500, "limit": 300, "soft_cap": 250, "offset": 2, "min_conv": 0.0},
    {"symbol": "VEV_5000", "strike": 5000, "limit": 300, "soft_cap": 200, "offset": 8, "min_conv": 0.5},
    {"symbol": "VEV_5100", "strike": 5100, "limit": 300, "soft_cap": 150, "offset": 18, "min_conv": 0.6},
    {"symbol": "VEV_5200", "strike": 5200, "limit": 300, "soft_cap": 100, "offset": 38, "min_conv": 0.7},
]
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    # === VFE-delta EXTENSION: route overflow into VEV chain ===
    if cfg["prefix"] == "vfe" and conviction > 0 and primary_target > primary_taken:
        ...
        for v in VEV_DELTA_VOUCHERS:
            if overflow <= 0: break
            # v49: conviction gate — skip strikes whose min_conv is not met
            if conviction < v.get("min_conv", 0.0):
                continue
            v_sym = v["symbol"]
            v_strike = v["strike"]
            v_limit = v["limit"]
