"""Round 4 v50 — v42 + conviction smoothing across ticks.

Tiny optimization: conviction can flip rapidly between ticks (e.g., one
tick z just above 0.7 threshold = high conviction, next tick just below =
zero). This causes take-then-undo whipsaws when signal is borderline.

v50 smooths conviction with a small EMA:
   smoothed_conviction = α × raw + (1 − α) × prev_smoothed

α = 0.5 → conviction is 50% prior tick + 50% current. Removes single-tick
spikes without damaging multi-tick signals.

Direction is taken from current tick (not smoothed) so direction flips
remain responsive.
...
def _conviction_signals(depth, pos, td, market_trades, cfg):
    ...
    raw_conviction = 0.0 if z_str == 0.0 else cfg["w_z"] * z_str + cfg["w_ema"] * ema_str + cfg["w_inf"] * inf_str
    # v50: smooth conviction with α=0.5 EMA to reduce single-tick whipsaws
    conv_key = f"_{cfg['prefix']}_conv"
    prev = td.get(conv_key, 0.0)
    # Only smooth if direction matches the prior smoothing context;
    # otherwise reset (direction flip → fresh signal)
    prev_dir_key = f"_{cfg['prefix']}_dir"
    prev_dir = td.get(prev_dir_key, 0)
    if prev_dir == direction and prev > 0:
        conviction = 0.5 * raw_conviction + 0.5 * prev
    else:
        conviction = raw_conviction
    td[conv_key] = conviction
    td[prev_dir_key] = direction if conviction > 0 else 0
