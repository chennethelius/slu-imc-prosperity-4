"""Round 4 v68 — v62 + cross-product VEV→VFE lead-lag signal.

BREAKTHROUGH: VEV options LEAD VFE in price discovery. Empirical regression
on R4-d3 (n=9999):

   vev_5000_delta = 0.673 * vfe_delta - 0.002   (R² = 0.58)

When VEV's actual move OVERSHOOTS this expected (residual > +0.5):
   fwd VFE 5-tick mean = +0.265   → VFE will continue UP
When VEV UNDERSHOOTS (residual < -0.5):
   fwd VFE 5-tick mean = -0.328   → VFE will continue DOWN

The cross-product residual is a CLEAN directional predictor for VFE.
Orthogonal to z-score MR / EMA momentum / Mark counterparty flow.

Implementation: each tick compute residual = vev_5000_delta - (beta * vfe_delta)
where beta=0.673, alpha=0. Add as 4th VFE conviction component (w_lag=0.20).
...
# v68: cross-product lead-lag signal config
LAG_VOUCHER = "VEV_5000"     # leading product
LAG_BETA = 0.673             # vev_delta = beta * vfe_delta + alpha (per regression)
LAG_THRESH = 0.5             # |residual| above this triggers signal
LAG_FULL = 2.0               # residual saturating signal at 1.0
LAG_W = 0.20                 # weight in conviction blend


def _conviction_signals(depth, pos, td, market_trades, cfg, state=None):
    ...
    # v68: VEV→VFE lead-lag signal (VFE only)
    lag_str = 0.0
    if cfg["prefix"] == "vfe" and state is not None:
        prev_vfe = td.get("_vfe_prev_mid", mid)
        prev_vev = td.get("_vev_prev_mid", None)
        # Get current VEV_5000 mid
        vev_depth = state.order_depths.get(LAG_VOUCHER)
        if vev_depth and vev_depth.buy_orders and vev_depth.sell_orders and prev_vev is not None:
            vev_mid = (max(vev_depth.buy_orders) + min(vev_depth.sell_orders)) / 2.0
            vfe_delta_t = mid - prev_vfe
            vev_delta_t = vev_mid - prev_vev
            expected_vev = LAG_BETA * vfe_delta_t
            residual = vev_delta_t - expected_vev
            # residual > 0 → VEV overreacted → VFE will continue UP
            # residual < 0 → VEV underreacted → VFE will continue DOWN
            lag_dir = +1 if residual > 0 else -1 if residual < 0 else 0
            if lag_dir == direction and abs(residual) > LAG_THRESH:
                lag_str = min(1.0, (abs(residual) - LAG_THRESH) / (LAG_FULL - LAG_THRESH))
            # Update state
            td["_vev_prev_mid"] = vev_mid
        elif vev_depth and vev_depth.buy_orders and vev_depth.sell_orders:
            vev_mid = (max(vev_depth.buy_orders) + min(vev_depth.sell_orders)) / 2.0
            td["_vev_prev_mid"] = vev_mid
        td["_vfe_prev_mid"] = mid

    conviction = 0.0 if z_str == 0.0 else cfg["w_z"] * z_str + cfg["w_ema"] * ema_str + cfg["w_inf"] * inf_str + LAG_W * lag_str
    return {"bb": bb, "ba": ba, "mid": mid, "direction": direction, "conviction": min(1.0, conviction)}
...
def _conviction_orders_with_extension(depth, pos, td, market_trades, cfg, state, all_orders):
    ...
    """
    sig = _conviction_signals(depth, pos, td, market_trades, cfg, state)
    if sig is None:
        return []
