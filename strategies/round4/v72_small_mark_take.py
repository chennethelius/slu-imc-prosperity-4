"""Round 4 v72 — v62 + small-Mark trade-size-triggered take (BREAKTHROUGH).

Empirical (R3+R4 6 days, VFE):
   Mark 67 BUY qty 1-5: fwd VFE 5000ms mean = +4.42
   Mark 22 SELL qty 3-5: mean = +3.70

SMALL Mark trades have MUCH stronger forecast — small = initial entry,
large = late. 4-tick forecast clears 1-2 tick spread cost.

Strategy: when small Mark 67 buy or Mark 22 sell detected in recent
market_trades, cross VFE in predicted direction (fade Mark 22, follow
Mark 67).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # === v72: small-Mark take trigger on VFE ===
        # Watch recent VFE market_trades for small Mark 67 buys or Mark 22 sells
        # → cross to take VFE in predicted direction
        vfe_trades = state.market_trades.get("VELVETFRUIT_EXTRACT") or []
        vfe_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
        recent_window = vfe_trades[-5:]   # last few prints
        bullish_signal = 0
        bearish_signal = 0
        for t in recent_window:
            qty = int(t.quantity)
            buyer = t.buyer or ""; seller = t.seller or ""
            if buyer == "Mark 67" and 1 <= qty <= 5:
                bullish_signal += qty   # follow
            if seller == "Mark 22" and 3 <= qty <= 5:
                bullish_signal += qty   # fade their sells (mid will rise)

        vfe_pos = state.position.get("VELVETFRUIT_EXTRACT", 0)
        SMALL_MARK_TAKE_LIMIT = 80   # max position contribution from this signal
        if vfe_depth and vfe_depth.buy_orders and vfe_depth.sell_orders:
            if bullish_signal >= 3 and vfe_pos < SMALL_MARK_TAKE_LIMIT:
                # Take VFE asks aggressively
                ba = min(vfe_depth.sell_orders)
                avail = -vfe_depth.sell_orders[ba]
                qty = min(avail, 30, 200 - vfe_pos, SMALL_MARK_TAKE_LIMIT - vfe_pos)
                if qty > 0:
                    orders["VELVETFRUIT_EXTRACT"] = [Order("VELVETFRUIT_EXTRACT", int(ba), qty)]
