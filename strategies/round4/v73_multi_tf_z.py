...
def _conviction_signals(depth, pos, td, market_trades, cfg):
    ...
    # v73: SLOW-TIMEFRAME z (1000-tick effective window via slow EWMA)
    # When fast AND slow z agree on direction, conviction is amplified.
    # Use a slow-rolling stdev (var_alpha=0.0005, ~2000-tick half-life)
    SLOW_VAR_ALPHA = 0.0005
    var_slow = td.get(f"_{P}_var_slow", cfg["stdev_init"] ** 2)
    var_slow = (1.0 - SLOW_VAR_ALPHA) * var_slow + SLOW_VAR_ALPHA * (dev * dev)
    td[f"_{P}_var_slow"] = var_slow
    stdev_slow = max(cfg["stdev_init"] * 0.15, var_slow ** 0.5)
    z_slow = dev / stdev_slow
    z_slow_str = 0.0 if abs(z_slow) < cfg["z_min"] else min(1.0, (abs(z_slow) - cfg["z_min"]) / (cfg["z_max"] - cfg["z_min"]))
    z_slow_dir = +1 if z_slow < 0 else -1   # bullish if mid below fair
    # Boost z_str when slow z confirms (agrees with direction) — small boost
    if z_slow_dir == direction and z_slow_str > 0.3:
        z_str = min(1.0, z_str * 1.15)
