...
def _z_take(state, cfg, td):
    """Standard z-take: cross spread when |z|>=z_thresh at favorable prices."""
    sym = cfg["sym"]
    ...
    if z > 0:
        room = max(0, LIMIT + pos)  # short capacity
        if room <= 0:
            return []
        orders, _ = _walk_book(depth, -1, sym, lambda px: px >= mean, room)
        return orders
    room = max(0, LIMIT - pos)  # long capacity
    if room <= 0:
        return []
    orders, _ = _walk_book(depth, +1, sym, lambda px: px <= mean, room)
    return orders
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # 2) Snackpack basket-sum overlay (overrides z-take if both fire,
        # since basket signal is stronger)
        for pair in BASKET_PAIRS:
            basket_out = _basket_pair_take(state, pair)
            for s, ors in basket_out.items():
                orders[s] = ors
