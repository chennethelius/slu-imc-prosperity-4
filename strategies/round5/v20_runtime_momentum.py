"""
Round 5 v20 — replace hardcoded per-product trend directions with a runtime
momentum signal derived from a slow EWMA.

Why this is necessary:
v19 IMC scored 78,382 — but per-day local backtests reveal that win is
sample-specific (i.e. overfit to whichever day IMC's hidden sandbox is
running):

  strategy            day2     day3     day4    avg
  v19_uvvisor       69,879  -20,230   63,426  37,692
  mm.py             44,844   14,089   44,979  34,637

v19 LOSES 20k on day 3 because the hardcoded target_pos directions
were derived from an IMC log whose drift directions match day 4 (and
mostly match day 2), but contradict day 3. mm.py — which never makes
directional bets — is steadily positive across all three days.

So v19 is high-mean / high-variance vs mm.py's lower-mean / low-variance.
The user's overfitting concern is correct.

v20 fix — per-product runtime momentum:

  1. Maintain a slow EWMA (alpha=0.997) of mid alongside the fast EWMA.
  2. After a warmup period (300 ticks), compute slow_dev = mid - slow_ewma.
  3. If |slow_dev| > 2*book_spread, the live trend is strong enough to
     trade. Set target_pos sign by sign(slow_dev).
  4. Otherwise target_pos = 0 (pure MM, no inventory load).
  5. Magnitude preserved from CFG (5 for tier-1, 3 for tier-2, 0 pure-MM).

Effect:
  - Day 4 (drift directions match v19's hardcoded): runtime detects same
    direction --> PnL similar to v19.
  - Day 3 (directions reversed): runtime detects opposite or stays at 0
    --> avoids $20k loss, PnL closer to mm.py.
  - Average across days: dominates v19 and mm.py jointly.

Risk: warmup of 300 ticks = 3% of day; we miss small moves at start
but capture the bulk of the directional trade in the remaining 97%.
...
# v20: slow EWMA for runtime momentum / direction detection. Span ≈ 330 ticks
# (alpha=0.997). Long enough that random-walk noise averages out, short enough
# to react to a real per-day drift within the first ~10% of the day.
SLOW_EWMA_ALPHA = 0.997

# Tick count below which we use cfg["target_pos"]=0 instead of the runtime
# direction signal (slow EWMA needs to settle).
WARMUP_TICKS = 300

# How many book-spreads of slow_ewma deviation we require before declaring
# a direction. 1.0x = deviation must exceed the book spread itself. Calibrated
# from theory: with SLOW_EWMA_ALPHA=0.997 and a linear drift d ticks/iter,
# steady-state deviation = d × 332. For full-day drift +500/10000 ticks
# (d=0.05), steady-state dev ≈ 16 ticks — comparable to typical book spreads.
TREND_THRESHOLD_MULT = 1.0
TREND_THRESHOLD_FLOOR = 3  # ticks
...
CFG: dict[str, dict] = {
    ...
    "UV_VISOR_RED":                  {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": +5},
    "UV_VISOR_ORANGE":               {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": +5},
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            cfg_target_mag = abs(cfg_target)  # 0, 3, or 5 — keep magnitude, replace sign

            # Update EWMAs: fast EWMA for mean-reversion z-score, slow EWMA for
            # runtime momentum detection.
            ...
            if prev is None:
                ...
                slow_ewma_mid = SLOW_EWMA_ALPHA * prev.get("sm", fair) + (1 - SLOW_EWMA_ALPHA) * fair
            ewma_state[sym] = {"m": ewma_mid, "v": ewma_var, "sm": slow_ewma_mid}

            # Runtime direction detection. For products with cfg_target == 0
            # (pure MM), keep target=0 — they're spread-only by design. For
            # products with non-zero cfg_target, ignore the cfg sign and derive
            # direction from slow_ewma deviation.
            if cfg_target_mag == 0 or tick_count < WARMUP_TICKS:
                # Bootstrap as pure MM (no directional bias) until the slow
                # EWMA settles. Trades 3% of day as pure spread capture; then
                # runtime momentum takes over. Avoids re-introducing the
                # hardcoded-direction overfit during warmup.
                base_target = 0
            else:
                slow_dev = fair - slow_ewma_mid
                threshold = max(TREND_THRESHOLD_FLOOR, TREND_THRESHOLD_MULT * book_spread)
                # Binary momentum reading
                if slow_dev > threshold:
                    obs_signal = +1
                elif slow_dev < -threshold:
                    obs_signal = -1
                else:
                    obs_signal = 0
                # Smoothed signal: EWMA of obs_signal. Hysteresis keeps us
                # from flipping on every noise threshold crossing. Reaches
                # |0.5| only after ~70 ticks of sustained agreement, so
                # short whipsaws on random-walk noise get filtered out.
                prev_dir_state = prev.get("ds", 0.0) if prev is not None else 0.0
                dir_state = 0.99 * prev_dir_state + 0.01 * obs_signal
                ewma_state[sym]["ds"] = dir_state
                if dir_state > 0.5:
                    base_target = +cfg_target_mag
                elif dir_state < -0.5:
                    base_target = -cfg_target_mag
                else:
                    base_target = 0
        ...
        new_trader_data = json.dumps({"ewma": ewma_state, "tick": tick_count}, separators=(",", ":"))
