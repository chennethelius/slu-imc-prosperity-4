"""
Round 5 v21 — neutralize ALL directional target_pos to zero (pure MM mode).

Why:
v20 attempt at runtime momentum detection failed (-$47k avg/day local).
The runtime signal whipsaws on random-walk noise even with EWMA hysteresis.
A momentum strategy on noisy data with no real cross-day persistence is
inherently unstable.

v19 hardcoded directions are heavily overfit (day 3 = -$20k local).
Pure MM (target_pos=0 everywhere) sacrifices the directional alpha but
captures the spread on every product without directional risk.

v21 sets target_pos=0 across the entire CFG. Spread + inv_skew only.
Expected: similar to mm.py (~$30-45k/day) but on the larger v19 product
universe — possibly $40-60k/day from spread on more products.

Per-day stability: positive on every day (no directional bet to be wrong).
Lower drawdown, higher Sharpe, smoother PnL.

Tradeoff vs v19: lower IMC peak (~$50k vs $78k) but no day-3 disaster
risk. Submit alongside v19 so user can pick based on final round
...
CFG: dict[str, dict] = {
    ...
    "UV_VISOR_AMBER":              {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    "PEBBLES_M":                   {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "SLEEP_POD_SUEDE":             {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "MICROCHIP_RECTANGLE":         {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "GALAXY_SOUNDS_SOLAR_FLAMES":  {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "TRANSLATOR_GRAPHITE_MIST":    {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    ...
    "PANEL_4X4":                   {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    "PEBBLES_XL":                  {"size": 4, "min_half": 4, "inv_skew": 15, "target_pos": 0},
    "GALAXY_SOUNDS_DARK_MATTER":   {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "PANEL_2X2":                   {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    "ROBOT_IRONING":               {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    ...
    "UV_VISOR_RED":                  {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    "UV_VISOR_ORANGE":               {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    "UV_VISOR_YELLOW":               {"size": 3, "min_half": 2, "inv_skew": 12, "target_pos": 0},
    ...
    "GALAXY_SOUNDS_PLANETARY_RINGS": {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    # v16: tier-1 upgrade. Drift -398 was under-captured at tier-2 sizing.
    "PANEL_1X4":                     {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0},
    ...
    "SLEEP_POD_COTTON":              {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "PEBBLES_S":                     {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "MICROCHIP_OVAL":                {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},

    # ----- v13 ADDITIONS — 4 more trending non-CFG products -----
    "UV_VISOR_MAGENTA":              {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "PANEL_2X4":                     {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "ROBOT_MOPPING":                 {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "MICROCHIP_TRIANGLE":            {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},

    # ----- v14 ADDITIONS — 6 more trending non-CFG products -----
    "TRANSLATOR_SPACE_GRAY":         {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "SLEEP_POD_LAMB_WOOL":           {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "MICROCHIP_SQUARE":              {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "OXYGEN_SHAKE_GARLIC":           {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "OXYGEN_SHAKE_MINT":             {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "OXYGEN_SHAKE_MORNING_BREATH":   {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},

    # ----- v15 ADDITIONS — small-drift sweep -----
    "PEBBLES_XS":                    {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "TRANSLATOR_VOID_BLUE":          {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "SLEEP_POD_NYLON":               {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "MICROCHIP_CIRCLE":              {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},

    # ----- v17 ADDITIONS — final low-drift sweep -----
    "SNACKPACK_CHOCOLATE":           {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "OXYGEN_SHAKE_EVENING_BREATH":   {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "SLEEP_POD_POLYESTER":           {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "GALAXY_SOUNDS_BLACK_HOLES":     {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},

    # ----- v18 ADDITIONS — smallest-drift sweep -----
    "PEBBLES_L":                     {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "SNACKPACK_VANILLA":             {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
    "GALAXY_SOUNDS_SOLAR_WINDS":     {"size": 4, "min_half": 3, "inv_skew": 15, "target_pos": 0},
}
