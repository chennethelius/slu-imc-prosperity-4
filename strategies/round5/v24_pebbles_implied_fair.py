"""
Round 5 v24 — use PEBBLES basket invariant as implied fair value (not as
target_pos override).

PEBBLES sum = 50000 invariant gives us a hidden fair-value estimator:
  implied_fair[X] = 50000 - sum(other 4 PEBBLES mids)

This is far tighter than the book mid (sd ~2.8 vs typical book spread ~10-15).
v23's attempt to use the invariant for target_pos failed because it fought
v19's directional bets. v24 instead uses the invariant ONLY for fair-value
estimation, leaving target_pos untouched.

Effect:
  - MM quotes ARE skewed around basket-implied fair instead of microprice
  - Take-the-cross fires when book offers prices on wrong side of implied fair
  - Catches small mispricings (5-18 tick basket deviations) systematically

Expected lift over v22:
  - 280 above-2sigma ticks/day × 5 PEBBLES × small edge = ~$1-3k/day
  - Pure spread alpha — direction-agnostic, robust across days
...
# v24 PEBBLES basket fair-value blend: weight on basket-implied fair vs microprice.
# 1.0 = use only basket fair, 0.0 = use only microprice. Empirically a partial
# blend (0.5-0.8) might be safest if implied is occasionally noisy.
PEBBLES_PRODUCTS = ["PEBBLES_XS", "PEBBLES_S", "PEBBLES_M", "PEBBLES_L", "PEBBLES_XL"]
PEBBLES_BASKET_TARGET = 50000
PEBBLES_FAIR_BLEND = 0.7  # 70% implied fair, 30% microprice (hedge against missing book data)
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        # v24 PEBBLES basket-implied fair value computation. For each PEBBLES
        # product X, implied fair = 50000 - sum(other 4 PEBBLES mids). The
        # basket invariant (sd=2.8 across all 3 days) makes this estimator
        # tighter than the book microprice. We blend 70% implied / 30%
        # microprice as a hedge against missing book data on any one product.
        pebbles_microprices = {}
        ...
        if len(pebbles_microprices) == 5:
            ...
            for p in PEBBLES_PRODUCTS:
                others_sum = total - pebbles_microprices[p]
                pebbles_implied_fair[p] = PEBBLES_BASKET_TARGET - others_sum
        ...
        for sym, cfg in CFG.items():
            ...
            if fair is None:
                continue
            # v24: replace fair with blend of basket-implied fair and microprice
            # for PEBBLES products. Implied fair has sd~2.8 vs book half-spread
            # ~5-7 ticks, so it's a sharper anchor for MM quotes.
            if sym in pebbles_implied_fair:
                implied = pebbles_implied_fair[sym]
                fair = PEBBLES_FAIR_BLEND * implied + (1 - PEBBLES_FAIR_BLEND) * fair
            best_bid = max(od.buy_orders.keys())
            best_ask = min(od.sell_orders.keys())
