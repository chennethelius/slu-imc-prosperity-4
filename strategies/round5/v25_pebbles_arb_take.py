"""
Round 5 v25 — opportunistic PEBBLES basket arb via take-the-cross only.

v23/v24 attempted to use the PEBBLES sum=50000 invariant as a target_pos
override or fair-value anchor — both failed because they fought v19's
directional alpha or lagged real book moves.

v25 takes a NARROWER approach: leave MM behavior untouched, but ADD an
opportunistic take-the-cross trigger when book offers prices on the
clearly wrong side of the basket-implied fair value.

For each PEBBLES product X:
  implied_fair_X = 50000 - sum(others' microprices)

  if best_ask_X < implied_fair_X - 5:  # ask is 5+ ticks below implied
      BUY aggressively (limit qty by capacity and implied conviction)
  if best_bid_X > implied_fair_X + 5:  # bid is 5+ ticks above implied
      SELL aggressively

Threshold of 5 ticks (≈1.8x sigma=2.8) filters out noise. The 18-tick max
basket deviation gives 5-13 ticks of available edge per arb event.

Critically: this trigger ADDS to v22's existing logic. It does not replace.
So MM continues as before, and we just sweep up extra PnL when basket-
implied says a PEBBLES product is mispriced.

Expected lift: $500-2k/day from ~280 above-2sigma PEBBLES events × small
edge each.
...
# v25 PEBBLES opportunistic basket arbitrage. Trigger threshold = 5 ticks
# (≈1.8x sigma=2.8 of basket sum). Below this we don't trade the arb to
# avoid noise-driven trades that lose to spread cost.
PEBBLES_PRODUCTS = ["PEBBLES_XS", "PEBBLES_S", "PEBBLES_M", "PEBBLES_L", "PEBBLES_XL"]
PEBBLES_BASKET_TARGET = 50000
PEBBLES_ARB_THRESHOLD = 5
PEBBLES_ARB_MAX_QTY = 4  # max additional take quantity per product
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        # v25: PEBBLES basket-implied fair value for opportunistic take-the-cross.
        # Computed once per tick from microprices (sum invariant = 50000).
        pebbles_implied_fair = {}
        pebbles_microprices = {}
        for p in PEBBLES_PRODUCTS:
            ...
            mp = microprice(od_p)
            if mp is not None:
                pebbles_microprices[p] = mp
        if len(pebbles_microprices) == 5:
            total = sum(pebbles_microprices.values())
            for p in PEBBLES_PRODUCTS:
                pebbles_implied_fair[p] = PEBBLES_BASKET_TARGET - (total - pebbles_microprices[p])
        ...
        for sym, cfg in CFG.items():
            ...
            # v25: extra basket-implied take-the-cross for PEBBLES products.
            # If best_ask is meaningfully below the basket-implied fair (which
            # is sd=2.8 ticks accurate), it's mispriced cheap — buy it.
            # Symmetric on the bid side. Caps the extra take at PEBBLES_ARB_MAX_QTY
            # to bound risk if the implied fair is wrong.
            if sym in pebbles_implied_fair:
                implied = pebbles_implied_fair[sym]
                # Extra BUY when ask < implied - threshold (and we still have
                # buy capacity after MM/take logic above).
                extra_buy_capacity = max(0, buy_qty - ask_take_qty)
                if best_ask < implied - PEBBLES_ARB_THRESHOLD and extra_buy_capacity > 0:
                    avail = abs(od.sell_orders[best_ask])
                    extra_qty = min(avail, extra_buy_capacity, PEBBLES_ARB_MAX_QTY)
                    if extra_qty > 0:
                        ords.append(Order(sym, best_ask, extra_qty))
                        ask_take_qty += extra_qty
                # Extra SELL when bid > implied + threshold.
                extra_sell_capacity = max(0, sell_qty - bid_take_qty)
                if best_bid > implied + PEBBLES_ARB_THRESHOLD and extra_sell_capacity > 0:
                    avail = od.buy_orders[best_bid]
                    extra_qty = min(avail, extra_sell_capacity, PEBBLES_ARB_MAX_QTY)
                    if extra_qty > 0:
                        ords.append(Order(sym, best_bid, -extra_qty))
                        bid_take_qty += extra_qty
