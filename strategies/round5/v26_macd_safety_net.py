"""
Round 5 v26 — MACD histogram as safety net for v22's hardcoded target_pos.

Problem statement:
v19/v22 hardcoded target_pos directions are tuned to whichever day IMC
sandbox runs (drift profile matches day 4-like). On a different sample
(e.g. local day 3), those same directions LOSE $20k because momentum
runs the other way. Per-day local PnL for v22:
  day2 +72k, day3 -18k, day4 +71k.

Earlier attempts to replace cfg directions with runtime momentum (v20)
failed catastrophically (-$47k avg) because momentum signals whipsaw
on random-walk noise.

v26 design — "smart safety net" using MACD methodology:
  - Compute fast EMA (span 12, weight 0.846) and slow EMA (span 26,
    weight 0.926) of microprice per product.
  - MACD = fast_EMA - slow_EMA (momentum measure).
  - Signal line = EMA of MACD (span 9, weight 0.8) — smoothing layer.
  - Histogram = MACD - Signal (CROSSOVER selectivity, less whipsaw than
    raw deviation).
  - Threshold = book_spread * 0.5 (volatility-adjusted, no hardcoded
    constant).

  ONLY override cfg target_pos when histogram CLEARLY contradicts cfg:
    - cfg_target > 0 AND histogram < -threshold => NEUTRALIZE to 0
    - cfg_target < 0 AND histogram > +threshold => NEUTRALIZE to 0
  Never FLIP direction (avoiding v20's whipsaw failure mode).

Why this is robust to overfitting:
  - Doesn't add any hardcoded constants — threshold is volatility-derived
  - Only intervenes on STRONG, SUSTAINED disagreement (MACD histogram is
    EMA-smoothed twice)
  - Falls back to cfg target_pos in noise zone (most ticks)
  - Worst case: equivalent to v22 on days where momentum agrees

Expected per-day:
  day 2 (cfg agrees with day 4-like): histogram aligned -> no override -> v22 alpha
  day 3 (cfg disagrees): histogram triggers -> neutralize to 0 -> closer to mm.py
  day 4 (cfg matches): histogram aligned -> no override -> v22 alpha
...
# v26 v2: ULTRA-conservative regime-change detector. Initial v26 with
# standard MACD (12/26/9 scaled to 100/500/50) over-triggered during normal
# pullbacks within trending days, killing $48k of day-2 profit. The fix is
# a single very-slow EMA (span 2000) and a tight contradiction threshold:
# only fire neutralization when current mid has clearly broken the day-level
# trend line by 4+ book spreads in the wrong direction.
#
# Math: for span 2000, alpha=0.001. After 2000 ticks of linear drift +d/tick,
# steady-state lag = d * 1000 ticks worth. For day-level drift +500/10000
# (d=0.05), lag ≈ 50 ticks — comparable to threshold. Fires only on a
# *reversal* from the trend line, not a continuation.
VERYSLOW_W = 0.999    # span 2000 ticks (~20% of day)
# Threshold: deviation from very-slow EMA in opposite direction to cfg must
# exceed K * book_spread. K=4 means only extreme reversal triggers.
SAFETY_K = 4.0
# Warmup: after 1500 ticks, very-slow EMA is mostly settled (weight on
# tick 0 = 0.999^1500 ≈ 0.22).
SAFETY_WARMUP_TICKS = 1500
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        macd_state: dict[str, dict] = ts_state.get("macd", {})
        ...
        for sym, cfg in CFG.items():
            ...
            # v26 v2: ultra-conservative regime-change detector. Single
            # very-slow EMA tracks the day-level trend line. Only override
            # cfg when current mid has clearly broken that line in the
            # opposite direction by SAFETY_K * book_spread.
            ms = macd_state.get(sym)
            if ms is None:
                veryslow_ema = fair
            else:
                veryslow_ema = VERYSLOW_W * ms["vs"] + (1 - VERYSLOW_W) * fair
            macd_state[sym] = {"vs": veryslow_ema}
            ...
            if sym in SNACKPACK_PRODUCTS:
                ...
                # v26 v2 safety net: only fires after warmup AND only on
                # extreme price-vs-trend-line reversal.
                if (cfg_target != 0 and tick_count >= SAFETY_WARMUP_TICKS):
                    deviation = fair - veryslow_ema
                    threshold = SAFETY_K * book_spread
                    if cfg_target > 0 and deviation < -threshold:
                        base_target = 0  # cfg long, mid clearly below trend
                    elif cfg_target < 0 and deviation > threshold:
                        base_target = 0  # cfg short, mid clearly above trend
                    else:
