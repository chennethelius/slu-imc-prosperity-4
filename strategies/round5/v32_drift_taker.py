"""
Round 5 v32 — drift-confirmed FULL TAKER mode for maximum capture of large moves.

Per user request: "try being fully a taker of the market to maximize profits
over large price movements".

v22-v31 strategies all use PASSIVE market making — quote inside the book,
wait for fills. They miss out on the big moves because the book moves
faster than passive quotes can capture (price runs through our bid before
we get filled, for example).

v27a tried full-taker globally but bled -$522k/day because it crossed the
spread on EVERY product (including weak-drift ones where the spread cost
exceeded the move).

v32 fix: only become a taker on products where v31b's drift detection has
LOCKED IN a runtime override. These products have confirmed |drift| > 100
ticks within the first 2000 ticks — strong-drift products where the
remaining 8000 ticks of move easily covers spread cost.

For drift-confirmed products:
  - Aggressively TAKE the cross at market_ask (when target=+10) or
    market_bid (when target=-10) to load up to position_limit fast
  - Skip MM quoting entirely; we're not earning spread, we're capturing drift

For non-confirmed products: stay in v31b passive MM mode (capture spread).

Expected effect:
  - Strong-drift products fill to ±10 in just a few ticks instead of
    accumulating slowly via passive bids
  - Capture more of each day's directional move
  - On day-3-like reversals, drift override has already flipped direction,
    so taker mode loads INTO the correct direction

Original v30/v31b docstring follows:
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v32: drift-confirmed FULL TAKER mode. When the runtime drift
            # override has locked in a direction for this product, we have
            # high-confidence signal that price will continue moving. Take
            # aggressively at market prices to load up to position limit
            # fast, instead of waiting for passive fills. Spread cost is
            # paid once on entry; the remaining day's drift covers it
            # easily on confirmed-strong-drift products.
            drift_locked = (sym not in SNACKPACK_PRODUCTS
                            and ds.get("ovr") is not None)

            ask_take_qty = 0
            bid_take_qty = 0
            if drift_locked and target_pos > 0 and buy_qty > 0:
                # Drift confirms LONG: aggressively take ask up to capacity.
                avail = abs(od.sell_orders[best_ask])
                ask_take_qty = min(avail, buy_qty)
                if ask_take_qty > 0:
                    ords.append(Order(sym, best_ask, ask_take_qty))
            elif drift_locked and target_pos < 0 and sell_qty > 0:
                # Drift confirms SHORT: aggressively take bid up to capacity.
                avail = od.buy_orders[best_bid]
                bid_take_qty = min(avail, sell_qty)
                if bid_take_qty > 0:
                    ords.append(Order(sym, best_bid, -bid_take_qty))
            else:
                # Non-confirmed: standard v22 passive logic with spread-
                # boundary take-the-cross only.
            ...
            # qty must still respect capacity. Drift-confirmed products
            # ALSO place passive quotes on the unwind side (e.g. when
            # long, place sell quote at our_ask) to harvest mean reversion
            # if price overshoots.
