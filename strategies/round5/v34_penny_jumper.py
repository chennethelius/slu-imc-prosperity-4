...
CFG["UV_VISOR_YELLOW"] = {"size": 4, "min_half": 2, "inv_skew": 15, "target_pos": 0}
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v34 PENNY JUMPER: quote 1 tick INSIDE the best bid/ask whenever
            # the book is wide enough (spread > 2). This puts our quote at
            # the front of the queue, capturing flow that would otherwise
            # hit the existing best bid/ask. We capture (book_spread - 2)
            # ticks per round-trip vs the standard half-spread quote at the
            # market mid. Strong inventory skew keeps us neutral around 0.
            if book_spread > 2:
                penny_bid = best_bid + 1
                penny_ask = best_ask - 1
                # Use penny quotes when our skewed-fair-derived quote would
                # otherwise be deeper in the book (less competitive).
                std_bid = math.floor(skewed_fair - half)
                std_ask = math.ceil(skewed_fair + half)
                our_bid_px = max(std_bid, penny_bid)
                our_ask_px = min(std_ask, penny_ask)
            else:
