...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v35 TARGETED AGGRESSIVE FILLING: only after the drift override
            # has LOCKED IN a direction (high-confidence signal), bump the
            # ORDER SIZE on the LOADING side to fill inventory to ±10 fast.
            # On the unwind side, keep standard size to capture spread on
            # retracement. v33 (size=10 globally) failed MC robustness
            # because it amplified losses from misaligned signals; v35 only
            # fires when signal is CONFIRMED.
            drift_locked = (sym not in SNACKPACK_PRODUCTS
                            and ds.get("ovr") is not None)
            buy_size = cfg["size"]
            sell_size = cfg["size"]
            if drift_locked and target_pos > 0:
                buy_size = 10
            elif drift_locked and target_pos < 0:
                sell_size = 10
