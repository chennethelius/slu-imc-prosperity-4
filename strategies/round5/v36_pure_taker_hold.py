"""
Round 5 v36 — pure TAKER strategy: single entry per drift signal, HOLD to EOD.

Per user request: capture LARGE movements with minimal trading volume to
reduce transaction costs. Example: PEBBLES_XL drifted ~+1000 ticks across
a day; full long at 14,700 held to 15,700 = +$10,000 PnL on ONE trade.
Repeated MM round-tripping through that move EATS 15-tick spread per
round-trip and accumulates 50+ trades = $7,500+ in transaction cost
even when capturing the same directional move.

v36 mechanics:
  1. Drift detection (inherited from v31b): wait for warmup (2000 ticks)
     then observe |drift| from open. If >100 ticks, LOCK direction.
  2. After lock: ONE shot of cross-the-spread to load to ±10 (full pos_limit).
     Pay the spread cost ONCE (~15 ticks * 10 lots = $150 per product).
  3. HOLD ±10 for the rest of the day. No MM quotes. No re-entry. No
     trim. Inventory pinned to limit until day ends.
  4. NO TRADING on products without confirmed drift signal — those
     contribute zero PnL (no MM spread captured) but also zero loss.

Trade-offs vs v31b:
  - GIVES UP: MM spread alpha (~$30-50k/day from passive quoting)
  - GAINS: clean directional capture without transaction-cost erosion
  - Net positive only if drift detection accurately identifies enough
    strong-drift products to cover the lost MM alpha

Stress test BEFORE pushing per user instruction. No look-ahead bias:
direction is determined ONLY from observed cumulative drift since open
during the live tick stream — uses no future information.
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            if "open" not in ds:
                ds["open"] = fair
            # v36: in pure-taker mode, ALWAYS set ovr once drift confirmed,
            # regardless of cfg agreement. We don't have hardcoded directions
            # to defer to — the drift signal IS the directional decision.
            if (tick_count >= DRIFT_WARMUP_TICKS
                    and ds.get("ovr") is None
                    and sym not in SNACKPACK_PRODUCTS):
                drift = fair - ds["open"]
                if abs(drift) > DRIFT_THRESHOLD:
                    ds["ovr"] = 1 if drift > 0 else -1
            drift_state[sym] = ds

            # v36 PURE TAKER mode. Only trade when drift override is locked
            # (signal confirmed). One shot of cross-the-spread to load to
            # ±10. Then HOLD — no MM, no further orders.
            if sym in SNACKPACK_PRODUCTS:
                # SNACKPACK keeps its pair-trade overlay (small position
                # adjustments based on cointegration spreads, not directional).
                base_target = int(round(snack_target.get(sym, 0)))
            elif ds.get("ovr") is not None:
                base_target = ds["ovr"] * DRIFT_TARGET_MAG
            else:
                # No drift signal yet — DON'T TRADE. No MM, no entry.
                continue

            # Compute how much we still need to load to reach base_target.
            needed = base_target - position
            if needed == 0:
                # Already at target — HOLD. No order this tick.
                continue

            ords: list[Order] = []
            if needed > 0:
                # Need to BUY. Cross the spread at best_ask.
                buy_capacity = POS_LIMIT - position
                qty = min(needed, buy_capacity, abs(od.sell_orders.get(best_ask, 0)))
                if qty > 0:
                    ords.append(Order(sym, best_ask, qty))
            else:
                # Need to SELL. Cross the spread at best_bid.
                sell_capacity = POS_LIMIT + position
                qty = min(-needed, sell_capacity, od.buy_orders.get(best_bid, 0))
                if qty > 0:
                    ords.append(Order(sym, best_bid, -qty))
