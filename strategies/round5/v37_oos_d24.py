"""
Round 5 v37 — REVERSAL detection via Bollinger bands warmed with previous-day stats.

Per user request: "experiment with reversal detection ... try warming up the
trend indicators with the previous day's data".

PRODUCT_STATS below contains per-product mean and stddev computed from the
combined mid trajectories of days 2/3/4 (~30k ticks total per product).
This baked-in calibration lets the algorithm have MATURE statistics from
tick 0 instead of waiting 2000 ticks to warm up.

v37 logic per product (from tick 0):
  z = (current_mid - hist_mean) / hist_std
  if z >  REVERSAL_Z_HIGH (e.g. +1.5): price extreme HIGH, expect mean
       reversion DOWN -> target_pos = -10 (full short, ride drop)
  if z < -REVERSAL_Z_HIGH: price extreme LOW, expect reversion UP
       -> target_pos = +10 (full long, ride rise)
  else: stay in HYSTERESIS zone (hold previous direction or 0)

This captures user's GALAXY_SOUNDS_RINGS example: if RINGS opens at 14,700
on test day and historical mean is 10,766 (std 766), z = +5.1 -> immediate
SHORT entry. Holds short until z falls back through 0 (price reverts to
mean), capturing the ~4100-tick downward move.

Risk of overfitting: PRODUCT_STATS is derived from days 2/3/4. If the IMC
test day has a SHIFTED equilibrium (different mean), our z-score is off.
MC stress test by perturbing prices and verifying robustness BEFORE pushing.

Original v30 docstring follows:
...
PAIR_MAX_LEG_SIZE = 5      # max lots per product per pair (combined across pairs capped at POS_LIMIT)
...
# v37 REVERSAL constants. Z-score thresholds for Bollinger band entry.
# z > REVERSAL_Z_HIGH triggers SHORT (mean reversion expected).
# z < 0.5 (returning to mean) closes the position.
REVERSAL_Z_HIGH = 1.5
REVERSAL_Z_EXIT = 0.5
REVERSAL_TARGET_MAG = 10

# Per-product historical statistics from days 2/3/4 (~30k ticks per product).
# Mean and std of mid_price across all 3 days. Used to set Bollinger bands
# from tick 0 of the test day — no warmup needed.
# Stats computed from days [2, 4]
PRODUCT_STATS = {
    "GALAXY_SOUNDS_BLACK_HOLES": {"mean": 11646.4, "std": 1108.6},
    "GALAXY_SOUNDS_DARK_MATTER": {"mean": 10129.0, "std": 290.2},
    "GALAXY_SOUNDS_PLANETARY_RINGS": {"mean": 10342.4, "std": 550.4},
    "GALAXY_SOUNDS_SOLAR_FLAMES": {"mean": 11008.9, "std": 429.5},
    "GALAXY_SOUNDS_SOLAR_WINDS": {"mean": 10501.4, "std": 553.7},
    "MICROCHIP_CIRCLE": {"mean": 9373.5, "std": 561.2},
    "MICROCHIP_OVAL": {"mean": 7997.5, "std": 1823.1},
    "MICROCHIP_RECTANGLE": {"mean": 8952.0, "std": 740.2},
    "MICROCHIP_SQUARE": {"mean": 13100.0, "std": 1946.4},
    "MICROCHIP_TRIANGLE": {"mean": 9415.0, "std": 867.5},
    "OXYGEN_SHAKE_CHOCOLATE": {"mean": 9713.5, "std": 611.6},
    "OXYGEN_SHAKE_EVENING_BREATH": {"mean": 9305.7, "std": 450.5},
    "OXYGEN_SHAKE_GARLIC": {"mean": 11984.5, "std": 1146.4},
    "OXYGEN_SHAKE_MINT": {"mean": 9624.9, "std": 478.4},
    "OXYGEN_SHAKE_MORNING_BREATH": {"mean": 10045.1, "std": 593.2},
    "PANEL_1X2": {"mean": 9243.6, "std": 412.3},
    "PANEL_1X4": {"mean": 9280.1, "std": 824.0},
    "PANEL_2X2": {"mean": 9281.4, "std": 566.4},
    "PANEL_2X4": {"mean": 11294.6, "std": 727.5},
    "PANEL_4X4": {"mean": 10044.1, "std": 424.7},
    "PEBBLES_L": {"mean": 10160.4, "std": 696.4},
    "PEBBLES_M": {"mean": 10309.6, "std": 648.3},
    "PEBBLES_S": {"mean": 8799.8, "std": 975.6},
    "PEBBLES_XL": {"mean": 13107.3, "std": 2120.5},
    "PEBBLES_XS": {"mean": 7622.8, "std": 1663.4},
    "ROBOT_DISHES": {"mean": 10055.4, "std": 661.5},
    "ROBOT_IRONING": {"mean": 8606.1, "std": 824.5},
    "ROBOT_LAUNDRY": {"mean": 9651.8, "std": 661.4},
    "ROBOT_MOPPING": {"mean": 11144.7, "std": 669.4},
    "ROBOT_VACUUMING": {"mean": 9187.6, "std": 592.2},
    "SLEEP_POD_COTTON": {"mean": 11617.6, "std": 1028.6},
    "SLEEP_POD_LAMB_WOOL": {"mean": 10813.9, "std": 413.4},
    "SLEEP_POD_NYLON": {"mean": 9842.5, "std": 458.9},
    "SLEEP_POD_POLYESTER": {"mean": 11753.2, "std": 1150.6},
    "SLEEP_POD_SUEDE": {"mean": 11146.5, "std": 973.8},
    "SNACKPACK_CHOCOLATE": {"mean": 9814.9, "std": 212.9},
    "SNACKPACK_PISTACHIO": {"mean": 9500.1, "std": 206.3},
    "SNACKPACK_RASPBERRY": {"mean": 10058.0, "std": 172.4},
    "SNACKPACK_STRAWBERRY": {"mean": 10662.6, "std": 423.1},
    "SNACKPACK_VANILLA": {"mean": 10132.7, "std": 178.9},
    "TRANSLATOR_ASTRO_BLACK": {"mean": 9564.7, "std": 431.0},
    "TRANSLATOR_ECLIPSE_CHARCOAL": {"mean": 9714.9, "std": 330.9},
    "TRANSLATOR_GRAPHITE_MIST": {"mean": 10138.3, "std": 540.9},
    "TRANSLATOR_SPACE_GRAY": {"mean": 9341.8, "std": 540.0},
    "TRANSLATOR_VOID_BLUE": {"mean": 10713.4, "std": 639.2},
    "UV_VISOR_AMBER": {"mean": 8031.1, "std": 1189.8},
    "UV_VISOR_MAGENTA": {"mean": 10988.5, "std": 696.6},
    "UV_VISOR_ORANGE": {"mean": 10352.6, "std": 625.6},
    "UV_VISOR_RED": {"mean": 11282.8, "std": 578.0},
    "UV_VISOR_YELLOW": {"mean": 10731.8, "std": 715.6},
}
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v37 REVERSAL signal via baked-in historical Bollinger bands.
            # Computes z-score of current mid vs 3-day historical mean/std
            # and sets target_pos based on extreme deviation (mean reversion).
            # Hysteresis: stay in current direction within |z| ∈ (0.5, 1.5);
            # exit to neutral when |z| < 0.5; flip direction when |z| > 1.5.
            stats = PRODUCT_STATS.get(sym)
            if sym in SNACKPACK_PRODUCTS:
                base_target = int(round(snack_target.get(sym, 0)))
            elif stats is not None and stats["std"] > 0:
                z = (fair - stats["mean"]) / stats["std"]
                if z > REVERSAL_Z_HIGH:
                    base_target = -REVERSAL_TARGET_MAG  # extreme high -> short
                elif z < -REVERSAL_Z_HIGH:
                    base_target = +REVERSAL_TARGET_MAG  # extreme low  -> long
                elif abs(z) < REVERSAL_Z_EXIT:
                    base_target = 0                     # at mean -> close
                else:
                    # hysteresis zone: keep current position direction
                    if position > 0:
                        base_target = +REVERSAL_TARGET_MAG
                    elif position < 0:
                        base_target = -REVERSAL_TARGET_MAG
                    else:
                        base_target = 0
