"""
Round 5 v38 — ADAPTIVE rolling Bollinger band REVERSAL.

Per user request: keep iterating with reversal detection strategies WITHOUT
moving-average trend detection.

v37 attempted reversal via baked-in historical stats from days 2/3/4. Local
OOS validated ($187k avg) but IMC dropped to $32k — the hidden test day's
equilibrium is shifted from the training days, so historical mean is stale.

v38 fix: compute Bollinger bands ENTIRELY FROM THE CURRENT DAY's data, no
baked statistics. Rolling EWMA mean (alpha=0.995, ~200-tick lookback) and
EWMA of squared deviation as variance proxy. From the current day's
distribution, detect when mid hits an extreme:

  z = (mid - rolling_mean) / sqrt(rolling_var)

Entry:
  z > +REVERSAL_Z_HIGH (e.g. +2.0): mid is at recent extreme HIGH, expect
    mean reversion DOWN -> target = -10
  z < -REVERSAL_Z_HIGH: mid at extreme LOW -> target = +10

Exit (hysteresis):
  |z| < REVERSAL_Z_EXIT (e.g. 0.5): mid back near rolling mean, close out
  Otherwise: hold current position

Drift override REMOVED. SNACKPACK pair logic kept (separate alpha source).
v22-style MM kept as fallback for products without enough warmup.

Why this should work where v37 didn't: rolling stats adapt to whatever
distribution the live test day has. If equilibrium is +1000 vs historical,
rolling mean catches up within ~200 ticks. Reversal signal then fires on
deviations from CURRENT regime, not stale history.

Risk: in trending markets, rolling mean lags trend by ~ alpha/(1-alpha) *
drift_per_tick = 200 * 0.05 = 10 ticks. Trending products hit z > 2 in the
trend direction repeatedly — we'd short uptrends and bleed money. Need
stress testing on day-3 and day-4 (which have strong drifts in different
directions).
...
# v38 ADAPTIVE rolling Bollinger constants. EWMA-based mean and variance,
# computed entirely within the current day. No baked-in historical stats.
#
# REV_ALPHA: weight on prev EWMA. 0.995 -> ~200-tick effective lookback.
#   Long enough to be statistically meaningful, short enough to ADAPT to
#   regime shifts faster than IMC's 10000-tick test window.
# REV_WARMUP: ticks before we trust the rolling stats (warmup needs alpha^N
#   to be small). 0.995^500 = 0.082, alpha^1000 = 0.0067 (settled).
# REV_Z_HIGH: z-score threshold for entry (extreme deviation).
# REV_Z_EXIT: z-score threshold to close back to neutral.
# REV_TARGET_MAG: position magnitude when signal fires.
REV_ALPHA = 0.995
REV_WARMUP = 1000
REV_Z_HIGH = 2.0
REV_Z_EXIT = 0.5
REV_TARGET_MAG = 10
REV_MIN_STD = 1.0  # require enough variance to compute meaningful z
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v38 ROLLING-BOLLINGER REVERSAL. Maintain a SLOW EWMA mean and
            # EWMA-of-squared-deviation as variance. Both adapt to current
            # day's distribution -- no historical stats baked in.
            ...
            if prev is None:
                ...
                # Fast EWMA (used by mean-reversion overlay below).
                ewma_mid = EWMA_ALPHA * prev["m"] + (1 - EWMA_ALPHA) * fair
                ewma_var = EWMA_ALPHA * prev["v"] + (1 - EWMA_ALPHA) * (fair - ewma_mid) ** 2
                # Slow EWMA for Bollinger band reversal.
                rev_mean = REV_ALPHA * prev.get("rm", fair) + (1 - REV_ALPHA) * fair
                rev_var = REV_ALPHA * prev.get("rv", 0.0) + (1 - REV_ALPHA) * (fair - rev_mean) ** 2
            ewma_state[sym] = {"m": ewma_mid, "v": ewma_var, "rm": rev_mean, "rv": rev_var}

            # Compute z-score for reversal signal (after warmup).
            rev_std = math.sqrt(rev_var) if rev_var > 0 else 0.0
            if tick_count >= REV_WARMUP and rev_std >= REV_MIN_STD:
                z = (fair - rev_mean) / rev_std
            else:
                z = 0.0

            # Dispatch base_target. SNACKPACK keeps pair logic. Other
            # products use ROLLING BOLLINGER reversal:
            #   z > +REV_Z_HIGH: extreme high -> SHORT
            #   z < -REV_Z_HIGH: extreme low -> LONG
            #   |z| < REV_Z_EXIT: at mean -> close to neutral
            #   else: hysteresis zone, hold current direction
            if sym in SNACKPACK_PRODUCTS:
                base_target = int(round(snack_target.get(sym, 0)))
            elif z > REV_Z_HIGH:
                base_target = -REV_TARGET_MAG
            elif z < -REV_Z_HIGH:
                base_target = +REV_TARGET_MAG
            elif abs(z) < REV_Z_EXIT:
                base_target = 0
            else:
                # hysteresis zone: hold what we have
                if position > 0:
                    base_target = +REV_TARGET_MAG
                elif position < 0:
                    base_target = -REV_TARGET_MAG
