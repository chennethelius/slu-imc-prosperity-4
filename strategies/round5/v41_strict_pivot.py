"""
Round 5 v39 — pivot-confirmed reversal TAKER, no moving averages.

Per user request: detect REVERSALS without MA, capture LARGER movements,
HOLD LONGER, reduce trading fees.

v37 (historical-baked Bollinger) failed IMC because test day's equilibrium
shifted from training. v38 (rolling Bollinger) failed local because pure
reversal in trending markets fires repeatedly against trends.

v39 uses a NON-MA reversal detector: pivot-point confirmation.

  Per product, track:
    day_max      = running maximum mid since first observation
    day_min      = running minimum mid since first observation
    ticks_since_max = counter incremented each tick, RESET when day_max updates
    ticks_since_min = same for minimum

  Pivot CONFIRMED when ticks_since_max > PIVOT_CONFIRM_TICKS without a new
  high. The previous max is then declared "the peak"; any subsequent move
  is the AFTER-PEAK reversal/trend. Mirrored for min.

  ENTRY (single cross-the-spread shot):
    Peak confirmed -> SHORT to -10 at best_bid
    Trough confirmed -> LONG to +10 at best_ask

  EXIT: HOLD position to EOD. No exit logic — capture the full reversal
  move. End-of-day mark settles the PnL.

  RE-ENTRY: once a pivot has fired and we're at ±10, we don't trade again
  on that product unless position drifts (e.g., partial fills).

This minimizes trading volume (1-2 trades per product per day = ~50-100
total vs v31b's ~1200). Each trade captures a LARGE multi-thousand-tick
move. Spread cost paid ONCE per product per day.

Critical structural difference vs v37/v38:
  - No MA: pivot detection uses running max/min only (no EWMA, no SMA)
  - No baked stats: everything computed from current day's tick stream
  - Patient: waits for confirmation before entering (vs Bollinger fires
    on every threshold crossing)

MC stress test BEFORE pushing per user instruction.
...
# v39 PIVOT-CONFIRMATION constants (NO moving averages).
# Track running max/min and the tick-count since last extreme update.
# When count exceeds PIVOT_CONFIRM_TICKS, the prior extreme is "confirmed"
# as the day's pivot — enter the opposing direction.
#
# PIVOT_CONFIRM_TICKS: how long to wait without a new extreme before
#   declaring a pivot. Tradeoff: shorter -> earlier entry but more false
#   positives from noise spikes; longer -> miss more of the move.
#   500 ticks = 5% of day. After this, we have ~95% of day still to capture.
# PIVOT_MIN_RANGE: minimum (max-min) to consider — filter out flat
#   products where pivots are noise. 30 ticks ≈ 2x typical book spread.
# PIVOT_TARGET_MAG: position magnitude at full pivot entry (±10 = full
#   pos_limit).
# PIVOT_INITIAL_DELAY: minimum tick count before any pivot can fire.
#   Lets us observe early price action before declaring an extreme.
PIVOT_CONFIRM_TICKS = 1500   # v41: longer than v40's 500 to filter noise
PIVOT_MIN_RANGE = 100        # day's range must exceed this before pivot can fire
PIVOT_MIN_REVERSAL = 80      # current must be 80+ ticks PAST the extreme (real reversal)
PIVOT_TARGET_MAG = 10
PIVOT_INITIAL_DELAY = 1000   # longer warmup
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        # v39 pivot-confirmation state per product.
        pivot_state: dict[str, dict] = ts_state.get("pivot", {})
        ...
        for sym, cfg in CFG.items():
            ...
            # v39 PIVOT-CONFIRMATION reversal taker (NO moving averages).
            # Track running max/min and ticks-since-extreme. When extreme
            # has not been broken for PIVOT_CONFIRM_TICKS, declare pivot
            # and lock in opposing direction (full ±10).
            ps = pivot_state.get(sym, {"max": fair, "min": fair,
                                        "tsm": 0, "tsn": 0, "lock": 0})
            if fair > ps["max"]:
                ps["max"] = fair
                ps["tsm"] = 0
            ...
            # Lock direction once a pivot is confirmed. lock = -1 (short
            # after peak) / +1 (long after trough) / 0 (no pivot yet).
            if (ps["lock"] == 0
                    and tick_count >= PIVOT_INITIAL_DELAY
                    and (ps["max"] - ps["min"]) >= PIVOT_MIN_RANGE):
                # v41: require BOTH a long no-new-extreme period AND a
                # significant move PAST the extreme. This filters out
                # noise-driven false pivots that were destroying v40's
                # cfg-correct directions.
                if (ps["tsm"] >= PIVOT_CONFIRM_TICKS
                        and (ps["max"] - fair) >= PIVOT_MIN_REVERSAL):
                    ps["lock"] = -1
                elif (ps["tsn"] >= PIVOT_CONFIRM_TICKS
                        and (fair - ps["min"]) >= PIVOT_MIN_REVERSAL):
                    ps["lock"] = +1
            pivot_state[sym] = ps

            # v40: cfg direction as DEFAULT (v31b-style), pivot OVERRIDE
            # when fired. This avoids the v39 failure where pre-pivot
            # target=0 bled pure-MM money on wide-spread products.
            if sym in SNACKPACK_PRODUCTS:
                base_target = int(round(snack_target.get(sym, 0)))
            elif ps["lock"] != 0:
                # Pivot CONFIRMED — overrides cfg direction.
                base_target = ps["lock"] * PIVOT_TARGET_MAG
