...
# v42 EOD position decay: in the last ~20% of the day, gradually reduce
# target_pos toward 0 to lock in gains and protect against late-day
# reversals. Linear ramp from full magnitude at EOD_DECAY_START to 0 at
# EOD_DECAY_END. IMC sandbox runs 10000 ticks/day, so 8000-10000 is the
# last 20%.
EOD_DECAY_START = 8000
EOD_DECAY_END = 10000
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym, cfg in CFG.items():
            ...
            # v42 EOD POSITION DECAY: in last 20% of day, scale target_pos
            # linearly toward 0. Locks in gains and avoids late-day
            # reversal risk. Doesn't apply to SNACKPACK (pair logic).
            if (sym not in SNACKPACK_PRODUCTS
                    and tick_count > EOD_DECAY_START):
                decay_factor = max(0.0, (EOD_DECAY_END - tick_count)
                                   / (EOD_DECAY_END - EOD_DECAY_START))
                base_target = int(round(base_target * decay_factor))
