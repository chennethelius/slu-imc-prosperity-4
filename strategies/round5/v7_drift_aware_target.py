"""
Round 5 v7 — v6 + drift-detected target_pos override (don't fight strong trends).

Empirical finding from mm.py R5 IMC submission #5 (24,482 PnL):
  Among the 13 products with directional target_pos=±5, 11 won and 2 lost.
  The losers were both fighting a clear trend:
    PANEL_1X2:        target=+5  but mid dropped 357 ticks (8849 → 8492)  -> -$2,603
    ROBOT_VACUUMING:  target=-5  but mid rose 227 ticks   (8584 → 8811)  -> -$1,113
  Combined damage from trend-fade: -$3,716.

Random-walk markets give 50/50 directional outcomes, but when a clear
intra-day trend establishes itself, the static target_pos either catches
the trend (big win) or fights it (big loss). Trying to PRE-PICK target_pos
is overfitting; the clean fix is to let the strategy DETECT the trend
intra-day and zero out the directional bias when it disagrees with the
realized drift.

Mechanism:
  - Maintain a 200-tick mid-price buffer per product (already partially
    threaded for vol estimation in mm.py).
  - Each tick, compute drift = (mid_now - mid_200_ago) / 200.
  - If sign(drift) opposes sign(target_pos) AND |drift| > DRIFT_TICKS_PER_STEP
    threshold, set effective_target_pos = 0 for this tick.
  - Otherwise keep configured target_pos.

DRIFT_TICKS_PER_STEP threshold sized as fraction of 1: a 1-tick-per-step
drift over 200 ticks = 200-tick total move, comfortably distinguishable
from random-walk noise (sqrt(200) ≈ 14 std-deviations of 1-tick noise).
Picked 0.5 ticks/step = 100-tick total — clear trend without false alarms.

If observed drift is too short (buffer < 50 ticks), skip the override
and use configured target_pos.

Original v6 docstring follows:

Round 5 v6 — mm.py + 3 conservative MM additions.

mm.py (IMC baseline at 24,482) quotes 19 of 50 products. v6 adds 3
products whose spread/drift/std profile clones an existing in-strategy
analog, so we expect similar behavior without re-introducing the
adverse-selection failure modes the original author explicitly excluded.

Additions and their analogs:

  TRANSLATOR_SPACE_GRAY    (spread 8.4,  std 503,  drift 731)
    -> near-clone of TRANSLATOR_ASTRO_BLACK (spread 8.4, std 490,
       drift 911) which is in mm.py at size=6, min_half=2, inv_skew=4.
       Use the same settings.

  SLEEP_POD_LAMB_WOOL      (spread 9.4,  std 413,  drift 383)
    -> profile sits between ROBOT_LAUNDRY (spread 7.2, std 614) and
       TRANSLATOR_GRAPHITE_MIST (spread 8.9, std 500). Conservative
       size=5, min_half=3, inv_skew=4, target_pos=0.

  OXYGEN_SHAKE_EVENING_BREATH (spread 11.9, std 400, drift 242)
    -> lowest drift in the OxygenShake group, very stable. Wider
       spread => slightly higher min_half. size=5, min_half=4,
       inv_skew=5, target_pos=0.

All target_pos=0 (no directional bias) since I have no LOO-validated
edge for these products. Everything else — microprice fair, take-the-
cross, vol-aware half-spread, inv_skew machinery — is unchanged from
mm.py.
...
# v7: trend override on the static target_pos.
# Track a long-window mid buffer and zero out base target when realized
# drift over the window opposes the configured directional bias.
TREND_BUFFER_LEN = 200          # ticks of mid history per product
TREND_TICKS_THRESHOLD = 30.0    # |drift| over the window to count as "clear trend"
...
CFG: dict[str, dict] = {
    ...
    # ----- v6 ADDITIONS — pure MM (target_pos=0), settings cloned from
    # nearest-profile existing product. No LOO-validated directional edge.
    "TRANSLATOR_SPACE_GRAY":       {"size": 6, "min_half": 2, "inv_skew": 4, "target_pos":  0},
    "SLEEP_POD_LAMB_WOOL":         {"size": 5, "min_half": 3, "inv_skew": 4, "target_pos":  0},
    "OXYGEN_SHAKE_EVENING_BREATH": {"size": 5, "min_half": 4, "inv_skew": 5, "target_pos":  0},
}
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        trend_buf: dict[str, list] = ts_state.get("trend_buf", {})
        ...
        for sym, cfg in CFG.items():
            ...
            # v7 drift override: track 200-tick mid buffer, zero out base_target
            # when realized drift over the window opposes the configured bias.
            buf = trend_buf.setdefault(sym, [])
            buf.append(fair)
            if len(buf) > TREND_BUFFER_LEN:
                del buf[: len(buf) - TREND_BUFFER_LEN]
            if base_target != 0 and len(buf) >= TREND_BUFFER_LEN:
                drift = buf[-1] - buf[0]
                if abs(drift) > TREND_TICKS_THRESHOLD and (
                    (drift > 0 and base_target < 0) or (drift < 0 and base_target > 0)
                ):
                    # Trend goes against the configured directional bet — neutralize.
                    base_target = 0
        ...
        new_trader_data = json.dumps(
            {"ewma": ewma_state, "trend_buf": trend_buf}, separators=(",", ":"))
